...
export let getTypeOption = function( data ) {
    ...
    if( data.applyToRawList && data.applyToRawList.length > 0 ) {
        var revision = data.applyTo.dbValue ? data.applyToRawList[ 0 ].internalValue :
 ...
 * 
 */
export let objectsToAssign = function() {
    var objectsToAssign = [];
    objectsToAssign = _.clone( appCtxSvc.getCtx( "license.adaptedObjects" ) );
...
export let attachDetachInput = function( data, attachFlag ) {
    exports.getContext();
    var selectedObjects = exports.objectsToAssign();
...
export let detachInput = function( data ) {
    var ctx = appCtxSvc.getCtx( "licenses" );
    data.applyToRawList = ctx.applyToRawList;
    data.entireBOM = ctx.entireBOM;
...
/**
 * This service contais utility functions for Awp0AttachLicense
 * 
 * @memberof NgServices
 * @member licenseMgmtUtils
 */
app.factory( 'licenseMgmtUtils', () => exports );
