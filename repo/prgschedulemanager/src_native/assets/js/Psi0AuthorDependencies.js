...
export let pasteDependenciesObjectsInList = function( targetObject, sourceObjects ) {
    ...
    if( sourceObjects.typeList[0] !== 'Psi0PrgDelRevision' ) {
    ...
    if( targetObject && targetObject.uid === PRECEDING_LIST ) {
    ...
    } else if( targetObject && targetObject.uid === SUCCEEDING_LIST ) {
        _addInObjectList( deferred, parentData, parentData.dataProviders.psi0_succeedingList, parentData.dataProviders.psi0_precedingList, 
            parentData.dataProviders.psi0_concurrentList, sourceObjects.uidList );
    } else if( targetObject && targetObject.uid === CONCURRENT_LIST ) {
...
export let getAuthorDependencyPanel = function( commandId, location ) {
 ...
 * @returns {Promise} - Resolved when all processing is complete.
 */
export let getPrecedingList = function( ctx, data, dataProvider ) {
    ...
    //exports.setPin(data, ctx.Psi0AuthorDependencies);

    var selected = appCtxService.ctx.selectedObj ? appCtxService.ctx.selectedObj.uid : ctx.mselected[ 0 ].uid;
    ...
    soaSvc.post( 'Query-2014-11-Finder', 'performSearch', inputData ).then( function( response ) {
        // Parse the SOA data to content the correct deliverable data
        var outputData = {
            psi0_precedingSearchResults: response.searchResults,
            psi0_precedingTotalFound: response.totalFound,
            psi0_precedingTotalLoaded: response.totalLoaded
        };
...
/**
 * Do the perform search call to populate the Succeeding list based on object values
 *
 * @param {ctx} ctx - The ctx of the viewModel
 * @param {data} data - The qualified data of the viewModel
 * @param {dataProvider} dataProvider - The data provider that will be used to get the correct content
 * @returns {Promise} - Resolved when all processing is complete.
 */
export let getSucceedingList = function( ctx, data, dataProvider ) {
    // Check is data provider is null or undefined then no need to process further
    // and return from here
    if( !dataProvider ) {
        return;
    ...
    soaSvc.post( 'Query-2014-11-Finder', 'performSearch', inputData ).then( function( response ) {
        // Parse the SOA data to content the correct deliverable data
        var outputData = {
            psi0_succeedingSearchResults: response.searchResults,
            psi0_succeedingTotalFound: response.totalFound,
            psi0_succeedingTotalLoaded: response.totalLoaded
        };
        policySvc.unregister( policyId );
        deferred.resolve( outputData );
    } );
 ...
 * @returns {Promise} - Resolved when all processing is complete.
 */
export let getConcurrentList = function( ctx, data, dataProvider ) {
    // Check is data provider is null or undefined then no need to process further
    // and return from here
    if( !dataProvider ) {
        return;
    ...
    soaSvc.post( 'Query-2014-11-Finder', 'performSearch', inputData ).then( function( response ) {
        // Parse the SOA data to content the correct deliverable data
        var outputData = {
            psi0_concurrentSearchResults: response.searchResults,
            psi0_concurrentTotalFound: response.totalFound,
            psi0_concurrentTotalLoaded: response.totalLoaded
        };
        policySvc.unregister( policyId );
        deferred.resolve( outputData );
    } );
...
export let createDeliverableDependencies = function( ctx, data, dataProvider, recalcSequence ) {
    ...
    var managePrgDeliverableDepsInput = {
    ...
    return managePrgDeliverableDepsInput;
};
...
/**
 * Service to Author Dependency panel.
 *
 * @member Psi0AuthorDependencies
 * @memberof NgServices
 */
app.factory( 'Psi0AuthorDependencies', () => exports );
