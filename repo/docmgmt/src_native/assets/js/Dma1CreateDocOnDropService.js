...
/**
 * Cached reference to 'messagingService'.
 */

/**
 * Cached reference to 'pasteService'.
 */
...
/**
 * Cached reference to 'localeService'.
 */

/**
 * Cached reference to 'soa_preferenceService'.
 */

/**
 * Cached reference to 'clientDataModel'.
 */

/**
 * Cached reference to 'clientMetaModel'.
 */
            ...
            return pasteSvc.execute( targetObject, createdObj, '' )
                .then( function() {
                    ...
                    _.forEach( createDocAndAttachResponse.output[ 0 ].datasets, function( dataset ) {
                        var datasetModel = cdm.getObject( dataset.dataset.uid );
                        var objNameProp = _.get( datasetModel, 'props.object_name' );
                        var fileName = objNameProp.getDisplayValue();
                        var fileInfo = [];
                        var ticket = '';
    ...
    return new Promise( resolve => {
        cfgSvc.getCfg( 'paste' ).then( function( pasteProvider ) {
            if( pasteProvider.defaultPasteFileHandler ) {
                 ...
                 */
                _myPromise = declUtils.loadDependentModule( pasteProvider.defaultPasteFileHandler.dep, AwPromiseService.instance, AwInjectorService.instance );
 ...
 *
 * @memberof NgServices
 * @member Dma1CreateDocOnDropService
 */
app.factory( 'Dma1CreateDocOnDropService', () => exports );
