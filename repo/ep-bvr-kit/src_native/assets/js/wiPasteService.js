 ...
 */
export function pasteInto( vmo ) {
    const areObjectsCut = epCutCopyService.areObjectsCut().isCutAction;
    const saveInputWriter = saveInputWriterService.get();
    let relatedObjects = [];
    const objectToPaste = ClipboardService.instance.getContents();

    relatedObjects = objectToPaste.map( element => {
        areObjectsCut ? addMoveObjectInput( saveInputWriter, vmo, element ) : addPasteInput( saveInputWriter, vmo, element );
        return element;
    } );
    ...
    epSaveService.saveChanges( saveInputWriter, true, relatedObjects ).then( function() {
        ...
        const resource = localeService.getLoadedText( app.getBaseUrlPath() + '/i18n/InstructionsMessages' );
 ...
 */
function addPasteInput( saveInputWriter, connectedTo, cloneFrom ) {
    const newObjUid = "new_object_id" + Math.random().toString();
    const objToClone = {
        "id": newObjUid,
    ...
    addReloadSection( saveInputWriter,objToReload );
 ...
 */
export function pasteBefore( vmo ) {
    pasteBeforeAfter(vmo, true);
}
 ...
 */
function pasteBeforeAfter(vmo, isPasteBefore){
    const areObjectsCut = epCutCopyService.areObjectsCut().isCutAction;
    let relatedObjects = [];
    let newObjUids = [];
    const saveInputWriter = saveInputWriterService.get();
    ...
    if( parentObj ) {
        ...
        objectToPaste.forEach( element => {
            if( !areObjectsCut ){
                newObjUids.push( addPasteInput( saveInputWriter, parentObj, element ) );
                relatedObjects.push( vmo );
        ...
        epSaveService.saveChanges( saveInputWriter, true, relatedObjects ).then( function( ) {
            policyId && policySvc.unregister( policyId );
            eventBus.publish( "epProcessTreeTable.plTable.reload" );
            const resource = localeService.getLoadedText( app.getBaseUrlPath() + '/i18n/InstructionsMessages' );
...
function addMoveObjectInput( saveInputWriter, parentObject, objectToMove ) {
    const objectToReload = {
        "objectUid": [
            objectToMove.uid
        ]
    };
    addReloadSection( saveInputWriter, objectToReload );
