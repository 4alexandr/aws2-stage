...
const EP_VALIDATION_RESULT_LABEL_SYNC = 'epValidationResultLabelSync';
 ...
 */
function createColumn( columnName, render ) {
 ...
 */
export function createDetailColumns( dataProvider, data, render ) {
    let tableColumns = [];
    for( let column in data.columns ) {
        tableColumns.push( createColumn( data.columns[ column ], render ) );
    }
 ...
 */
export function getColumnsData( dataProvider, data ) {
 ...
 * @return {Object} - the table row contents
 */
export function getDetailColumnsData( selectedObject, dataProvider ) {
    ...
    for( let testedObject in testedObjects ) {
        if( selectedObject.inputObject[ 0 ].uid === testedObject ) {
            const additionalProperties = testedObjects[ testedObject ].additionalPropertiesMap2;
            // There will be a message for each validation test
            for( let prop in additionalProperties ) {
                let vmo = viewModelObjectService.constructViewModelObjectFromModelObject( selectedObject.inputObject[ 0 ] );
                vmo.uid = testedObject;
                let propValueArray = additionalProperties[ prop ];
                vmo.props = [];
    ...
    loadResult.totalFound = totalRows;
...
// The 'loadMorePages' event is fired twice from splmTable.createTable
// This event cause the dataProvider.nextPage action to fire which causes results to load twice
// in table.  Setting this is a workaround .
/**
 * doNothingForTree - fired as nextPageAction from data provider
 *
 * @param {String} data - view data
 */
export function doNothingForTree( data ) {
    data.totalFound = 0;
}
/**
...
/**
 * turnOffResultsSync - turn off Sync for BOMLine label sync on results table
 *
 */
export function turnOffResultsSync() {
    epValidationContextService.setEpValidationContext( EP_VALIDATION_RESULT_LABEL_SYNC, 'false' );
}
...
export function runValidationChecks() {
    ...
    epValidationContextService.setEpValidationContext( EP_VALIDATION_GET_NEW_RESULTS, 'true' );
    ...
    return epLoadService.loadObject( loadtypeInput, false ).then(
        function( response ) {
            if( response.ServiceData ) {
                ...
                epValidationContextService.setEpValidationContext( EP_VALIDATION_RESULT_LABEL_SYNC, 'true' );
            }
        }
    );
}
