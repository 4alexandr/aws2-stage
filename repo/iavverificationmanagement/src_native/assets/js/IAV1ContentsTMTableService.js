...
/**
 * @module js/IAV1ContentsTMTableService
 */
...
function _buildTreeTableStructure( parentNode, deferred, treeLoadInput ) {
    ...
    var selectedobj;
    var type = 'IAV0TestRequestRevision';
    ...
    var soaInput = {
        columnConfigInput: {
            clientName: 'AWClient',
            clientScopeURI: 'IAV1TestMethodTable'
        },
        searchInput: {
            maxToLoad: 50,
            maxToReturn: 50,
...
function processProviderResponse( treeLoadInput, searchResults, startReached, endReached ) {
    ...
    var treeLoadResult = awTableSvc.buildTreeLoadResult( treeLoadInput, vmNodes, true, startReached, endReached, null );
...
function createVMNodeUsingObjectInfo( obj, childNdx, levelNdx, parentNode ) {
    ...
    var objUid = sourceObj.uid;
    var objType = sourceObj.type;
    ...
    // get Icon for node
    if( sourceObj.props.awb0UnderlyingObjectType.dbValues[0] ) {
...
export let loadTMTreeDataForVR = function( treeLoadInput ) {
    /**
...
export let loadTreeTableColumns = function( dataProvider, data ) { // Get the column config
    ...
    let columnConfigUri = 'IAV1TestMethodTable';
...
/*
 * Corrects selection to be contents from test method tree table
 */
export let changeSelectionForTM = function( data ) {
    var dataProvider = null;
    var selectedObjects = [];
    if( data && data.dataProviders && data.dataProviders.contentsTMTableProvider ) {
        dataProvider = data.dataProviders.contentsTMTableProvider;
        selectedObjects = dataProvider.selectedObjects;
        appCtxSvc.registerCtx( 'TR_TMTableSelection', selectedObjects );
    ...
    if(data && data.dataProviders && data.dataProviders.contentsTMTableProvider && data.dataProviders.contentsTMTableProvider.selectedObjects.length !== 0){
        appCtxSvc.registerCtx( 'isChangeSelectionForTMCalled', true );
    }
...
/*
 * Corrects selection to be physical BOM
 */
export let changeSelectionForPhyBOM = function( data ) {
    var dataProvider = null;
    var selectedObjects = [];
    if( data && data.dataProviders && data.dataProviders.testEBOMTableProvider ) {
...
export let clearProviderSelectionForTM = function( data ) {
    if( data && data.dataProviders )
    {
        var dataProvider = data.dataProviders.contentsTMTableProvider;
        if( dataProvider )
        {
...
export default exports = {
    ...
    changeSelectionForTM,
    changeSelectionForPhyBOM,
    clearProviderSelectionForTM

};
app.factory( 'IAV1ContentsTMTableService', () => exports );
