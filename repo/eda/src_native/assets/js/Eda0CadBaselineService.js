...
function _processUiConfigColumns( columns ) {
    for( var idx = 0; idx < columns.length; ++idx ) {
        ...
        if( columns[ idx ].propDescriptor.propertyName === 'eda0BaselineDate' ) {
    ...
    return soaSvc.postUnchecked( 'Internal-AWS2-2019-06-Finder', 'performSearchViewModel4', soaInput ).then(
        function( response ) {
            ...
            for( var childNdx = searchObjects.length - 1; searchObjects && childNdx >= 0; childNdx-- ) {
                ...
                var vmNode = awTableSvc.createViewModelTreeNode( pobject.uid, _cadBaselineElementType, displayName, 0, childNdx, '' );
                vmNode.isLeaf = true;
...
app.factory( 'Eda0CadBaselineService', () => exports );
