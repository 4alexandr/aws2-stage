...
/**
 * Update the context with valid item revisions
 *
 * @param {Object} selections Valid selection that needs to be assigned
 */
var _updateContext = function( selections ) {

    if( selections && selections.length > 0 ) {
        var ctx = {};
        ctx.selectedObjects = selections;
        appCtxSvc.registerCtx( "workflow", ctx );
    }
};
...
export let validateAndActivateCommandPanel = function( data, ctx, selections ) {
    ...
    if( ( ctx.activeToolsAndInfoCommand && ctx.activeToolsAndInfoCommand.commandId === "Awp0ReplaceMultipleParticipants" ) ) {
        appCtxSvc.unRegisterCtx( "workflow" );
    ...
    if( outputData && outputData.finalMessage && outputData.finalMessage.length > 0 ) {
        var buttons = [ {
            ...
            },
            {
                ...
                onClick: function( $noty ) {
                    $noty.close();
                    _updateContext( outputData.validSels );
                    commandPanelService.activateCommandPanel( 'Awp0ReplaceMultipleParticipants', 'aw_toolsAndInfo' );
                }
            }
        ];
 ...
 *
 * @memberof NgServices
 * @member Awp0ReplaceMultipleParticipantCommandHandler
 */
app.factory( 'Awp0ReplaceMultipleParticipantCommandHandler', () => exports );
