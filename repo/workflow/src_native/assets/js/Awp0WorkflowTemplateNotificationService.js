...
var _populateRecipientsColumn = function( data, handlerArgumentValues, dynamicParticipants ) {
    ...
    if( data.preferences.EPM_ARG_target_user_group_list_separator && data.preferences.EPM_ARG_target_user_group_list_separator.length > 0 && data.preferences.EPM_ARG_target_user_group_list_separator[0].trim() !== '' ) {
        splitRecipientValues = recipientValue.split( data.preferences.EPM_ARG_target_user_group_list_separator[0] );
    }else{
 ...
 */
export let populateNotificationTableData = function( data, selection, dataProvider, handlerNames, dynamicParticipants ) {
    parentData = data;
    var argumentRows = [];
...
export let reloadPanel = function( selection, dynamicParticipants ) {
    ...
    exports.populateNotificationTableData( parentData, templateObject, parentData.dataProviders.notificationDataProvider,
        [ 'EPM-late-notification', 'EPM-notify', 'EPM-notify-report' ], dynamicParticipants );
};
 ...
 */
export let updateNotificationContext = function( handlerObject, dataProvider ) {
 ...
 */
export let notificationHandlerRowSelection = function( ctx ) {
    if( ctx.activeToolsAndInfoCommand && ( ctx.activeToolsAndInfoCommand.commandId === 'Awp0WorkflowAddNotificationHandler'
 ...
 *
 * @memberof NgServices
 * @member Awp0WorkflowTemplateNotificationService
 */
app.factory( 'Awp0WorkflowTemplateNotificationService', () => exports );
