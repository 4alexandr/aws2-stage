...
export let getDigitalSignatureService = function() {
    ...
    contributionService.require( 'digitalSignatureService' ).then( function( providers ) {
...
export let populateDescription = function( data, selectedObject ) {
    awp0InboxUtils.populateDescription( data, selectedObject );
};
...
var _getCurrentTaskState = function( data ) {
    var taskState;
    var actualTask;
    if( data && data.inputData && data.inputData.actionableObject ) {
        actualTask = data.inputData.actionableObject;
    }
            ...
            if( partErr.errorValues ) {
                _.forEach( partErr.errorValues, function( errVal ) {
                    ...
                    if( errorCode && errorCode === 33321 ) {
                        ...
                        if( data.preferences.WRKFLW_hide_subsequent_task_errors && data.preferences.WRKFLW_hide_subsequent_task_errors[ 0 ]
                            && data.preferences.WRKFLW_hide_subsequent_task_errors[ 0 ] === 'false' ) {
                            displayMessage = true;
...
export let populateSecondaryPanel = function( data, selection ) {
    if( selection !== null ) {
        ...
        exports.loadObjectProperties( data, modelObject );
    }
};
...
export let isPerformPanelUpdateNeeded = function( data, selection, updatedObjects ) {
    if( selection && updatedObjects && updatedObjects.length > 0 ) {
        ...
        if( modelObject && modelObject.props.awp0PerformableByMeBehavior && modelObject.props.awp0PerformableByMeBehavior.dbValues && modelObject.props.awp0PerformableByMeBehavior.dbValues[ 0 ] ) {
            ...
            // Check if previous value and new value both ar ento same and action is not in progress then only call
            // further
            if( performableByMe !== data.awp0PerformableByMeBehavior ) {
                exports.populateSecondaryPanel( data, modelObject );
            }
        }
    }
};
 ...
 * @returns Return the structure that will contain group member signoff and resource pool signoff on which
 ...
 *
 * @returns Role UID based on logged in user session
 */
...
var _getGroupMemberSignoffObject = function( groupMemberSignoffs ) {
    ...
    if( !signoffObject && groupMemberSignoffs && groupMemberSignoffs.length > 0 ) {
        if( groupMemberSignoffs[ 0 ] && groupMemberSignoffs[ 0 ].signoff ) {
...
var _getResourcePoolSignoffObject = function( resourcePoolSignoffs ) {
    ...
    if( !signoffObject && resourcePoolSignoffs && resourcePoolSignoffs.length > 0 ) {
        if( resourcePoolSignoffs[ 0 ] && resourcePoolSignoffs[ 0 ].signoff ) {
...
export let getSignoffObject = function( selection ) {
    ...
    if( allSignoffObjects.length > 0 ) {
        ...
        if( signoffOutput && signoffOutput.gropuMemberSignoffs && signoffOutput.resourcepoolSignoffs ) {
            if( signoffOutput.gropuMemberSignoffs.length <= 0 && signoffOutput.resourcepoolSignoffs.length <= 0 ) {
 ...
 *
 * @returns True or false based on server version
 */
...
export let openPerformPanel = function( data, selection, deferred ) {
    ...
    data.awp0PerformableByMeBehavior = selection.props.awp0PerformableByMeBehavior.dbValues[ 0 ];
    ...
    if( !exports.isPlatformVersionSupported() ) {
        data.isTaskPerformable = true;
        ctx.isTaskPerformable = true;
        exports.updateSelection( selection, false );
        return deferred.resolve();
    }
 ...
 */
export let updateSelection = function( selection, isPanelContext, deferred ) {
    // Check if selection is null then unregister the context and return from here
    if( !selection ) {
        appCtxSvc.unRegisterCtx( 'task_to_perform' );
        if( deferred ) {
            deferred.resolve( null );
        }
        return;
    ...
    if( modelObject ) {
        ...
        if( isPanelContext ) {
            exports.runTaskPerformHandlers( selection ).then( function () {
                performPanelService.updatePerformTaskPanelContent( selection );
                if( deferred ) {
                    deferred.resolve( null );
                }
            },
        ...
        performPanelService.populateContributedPerformTaskPanel( 'performTaskPanelConfiguration.secondaryAreaContribution', selection );
    }
};
...
app.factory( 'Awp0PerformTask', () => exports );
