 ...
 * @param {Object} data Data view model obejct
 */
export let drawNodes = function( nodeObjects, graphModel, graph, groupGraph, activeLegendView, data, selTemplate ) {
    parentData = data;
    var fixedLayoutMode = false;

    if( data && data.preferences && data.preferences.WRKFLW_use_fixed_position && data.preferences.WRKFLW_use_fixed_position[ 0 ] === 'true' ) {
        fixedLayoutMode = true;
    }

    var workflowDgmCtx = appCtxSvc.getCtx( 'workflowDgmCtx' );
    if( workflowDgmCtx && workflowDgmCtx.diagram ) {
        fixedLayoutMode = !workflowDgmCtx.diagram.isAutoLayoutOn;
    } else {
...
/**
 * Define graph node service to draw nodes on graph
 *
 * @memberof NgServices
 * @member Awp0WorkflowDesignerNodeService
 */
app.factory( 'Awp0WorkflowDesignerNodeService', () => exports );
