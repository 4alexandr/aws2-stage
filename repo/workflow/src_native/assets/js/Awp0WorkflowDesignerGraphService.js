...
var _setInitialContextFromPreference = function( data, isAutoLayout, selectionLayoutDirection ) {
    ...
    appCtxSvc.registerCtx( 'workflowDgmCtx', workflowDgmCtx );
};
...
var _isFixedLayoutMode = function( data ) {
    ...
    if( workflowDgmCtx && workflowDgmCtx.diagram && !workflowDgmCtx.diagram.isAutoLayoutOn ) {
        return true;
    } else if( !workflowDgmCtx ||  !workflowDgmCtx.diagram || !workflowDgmCtx.diagram.layoutOption ) {
        //Get the preferred layout direction from the preference if it exists and set the layout context
        if ( data && data.preferences && data.preferences.WRKFLW_preferred_diagram_layout ) {
 ...
 */
var _drawGraph = function( data, graphModel, selTemplate, node ) {
    ...
    if( returnData && returnData.nodeObjects && returnData.nodeDataMap ) {
        ...
        nodeService.drawNodes( addedNodes, graphModel, graph, groupGraph, activeLegendView, data, selTemplate );
    ...
    if( node && !_isFixedLayoutMode( data ) ) {
 ...
 */
var _getAppliedLayoutOption = function() {
    if( appCtxSvc.ctx.workflowDgmCtx && appCtxSvc.ctx.workflowDgmCtx.diagram && appCtxSvc.ctx.workflowDgmCtx.diagram.layoutOption ) {
        return appCtxSvc.ctx.workflowDgmCtx.diagram.layoutOption;
    } else if( !appCtxSvc.ctx.workflowDgmCtx ||  !appCtxSvc.ctx.workflowDgmCtx.diagram || !appCtxSvc.ctx.workflowDgmCtx.diagram.layoutOption ) {
...
export let drawGraph = function( data, node, modelObject, a_graphModel ) {
    ...
    if( appCtxSvc.ctx &&  appCtxSvc.ctx.startEditGraphLayout ) {
        ...
        _setInitialContextFromPreference( data, isPrevAutoLayout, previousLayout );
        appCtxSvc.unRegisterCtx( 'startEditGraphLayout' );
    }

    var isFixedLayout = _isFixedLayoutMode( data );
    var layout = _getAppliedLayoutOption( data );
    if ( isFixedLayout ) {
        exports.drawGraphFixedLayout( data, node, modelObject, a_graphModel );
        return;
    }
    ...
    _drawGraph( data, graphModel, selTemplate, node );
    ...
    var defaultLayout = _getAppliedLayoutOption();
    if( graphModel && graphModel.graphControl ) {
        _applyGraphLayout( graphModel );
    ...
    if( appCtxSvc.ctx.selected ) {
        selectionService.setSourceObject( appCtxSvc.ctx.selected );
        workflowDgmCtx = appCtxSvc.getCtx( 'workflowDgmCtx' );
        if( workflowDgmCtx ) {
            workflowDgmCtx.selectedObject = selTemplate;
            if( !workflowDgmCtx.diagram ) {
                workflowDgmCtx.diagram = {};
            }
            workflowDgmCtx.diagram.isAutoLayoutOn = true;
            workflowDgmCtx.diagram.layoutOption = defaultLayout;
            appCtxSvc.updateCtx( 'workflowDgmCtx', workflowDgmCtx );
        } else {
            workflowDgmCtx = {
                selectedObject: selTemplate,
                diagram: {
                    isAutoLayoutOn : true,
    ...
    if( graphModel && appCtxSvc.ctx &&  appCtxSvc.ctx.openTaskPalette && appCtxSvc.ctx.graph
        ...
        exports.showTaskLegendPalettePanel( workflowDgmCtx, graphModel, appCtxSvc.ctx.graph.legendState );
    }

    // Set the default selection on graph. If any sc_uid present in URL that node need to be selected by default.
    selectionService.setDefaultGraphSelection( appCtxSvc.ctx.selected, appCtxSvc.ctx.state.params );
};
 ...
 * @param {Object} node Node object whose children need to be loaded
 */
export let drawGraphFixedLayout = function( data, node, modelObject, a_graphModel ) {
    var graphModel = data.graphModel;
    ...
    var selectedSubTask;
    if( appCtxSvc.ctx.workflowDesignerSubTaskTemplateSelection ) {
        selectedSubTask = appCtxSvc.ctx.workflowDesignerSubTaskTemplateSelection;
    }
    ...
    if( selectedSubTask ) {
        ...
        appCtxSvc.unRegisterCtx( 'workflowDesignerSubTaskTemplateSelection' );
    ...
    if( selTemplate ) {
        ...
        if( workflowDgmCtx ) {
            ...
            workflowDgmCtx.diagram.isAutoLayoutOn = false;
            workflowDgmCtx.diagram.layoutOption = 'FixedLayout';
            appCtxSvc.registerCtx( 'workflowDgmCtx', workflowDgmCtx );
        } else {
            workflowDgmCtx = {
                selectedObject: selTemplate,
    ...
    // Set the default selection on graph. If any sc_uid present in URL that node need to be selected by default.
    selectionService.setDefaultGraphSelection( selTemplate, appCtxSvc.ctx.state.params );
};
...
/**
 * Set the layout as fixed position
 * @param {*} data the data
 */
export let setAutoLayout = function( data ) {
    var workflowDgmCtx = appCtxSvc.getCtx( 'workflowDgmCtx' );
    if( workflowDgmCtx ) {
        workflowDgmCtx.useFixedLayout = false;
        appCtxSvc.updateCtx( 'workflowDgmCtx', workflowDgmCtx );
    }
    exports.updateWorkflowDesignerPage( data, null );
};

/**
 * Method to get whether workflow task legend panel Mode is On
 * @param {Object} workflowDgmCtx Context object
 * @return {boolean} true when workflow task legend panel Mode is On, false otherwise
 */
var _isLegendPanelActive = function( workflowDgmCtx ) {
    if( workflowDgmCtx && workflowDgmCtx.taskLegendPaletteMode ) {
        return workflowDgmCtx.taskLegendPaletteMode;
    }
    return false;
};

/**
 * Set the layout as auto layout
 *  @param {*} data the data
 */
export let setFixedLayout = function( data ) {
    var workflowDgmCtx = appCtxSvc.getCtx( 'workflowDgmCtx' );
    if( workflowDgmCtx ) {
        workflowDgmCtx.useFixedLayout = true;
        appCtxSvc.updateCtx( 'workflowDgmCtx', workflowDgmCtx );
    }

    // If the legend panel is open, close it (disable editing in fixed mode for now)
    var taskLegendPaletteMode = !_isLegendPanelActive( workflowDgmCtx );
    if( workflowDgmCtx && !taskLegendPaletteMode ) {
        workflowDgmCtx.taskLegendPaletteMode = taskLegendPaletteMode;
        appCtxSvc.updateCtx( 'workflowDgmCtx', workflowDgmCtx );
    }
    exports.updateWorkflowDesignerPage( data, null );
};
...
export let expandChildren = function( graphModel, node, data ) {
    ...
    if( node && isFixedLayout && node.modelObject ) {
        ...
        appCtxSvc.registerCtx( 'workflowDesignerSubTaskTemplateSelection', node.modelObject );
        // Fire this event to reset the connection if already selected in legend panel
        eventBus.publish( 'workflowDesigner.resetConnectionMode' );
        return;
...
export let showTaskRelatedInfoPanel = function( tabIdToShow, toggleCase ) {
    ...
    if( toggleCase ) {
        appCtxSvc.ctx.hideLegendPanel = true;
        if( tabIdToShow !== null ) {
            tabIdToShow = null;
        } else {
 ...
 */
var _setLegendPanelContainerHeight = function() {
    var taskLegendPaletteMode = _isLegendPanelActive( appCtxSvc.ctx.workflowDgmCtx );
 ...
 */
export let fitGraphVisibility = function( graphContext ) {
    ...
    _setLegendPanelContainerHeight();
 ...
 */
export let showTaskLegendPalettePanel = function( workflowDgmCtx, graphModel, legendState ) {
    var taskLegendPaletteMode = !_isLegendPanelActive( workflowDgmCtx );
    if( workflowDgmCtx ) {
        workflowDgmCtx.taskLegendPaletteMode = taskLegendPaletteMode;
        appCtxSvc.updateCtx( 'workflowDgmCtx', workflowDgmCtx );
        ...
        if( selectedNodes && selectedNodes.length === 1 ) {
            ...
            if( workflowDgmCtx ) {
                ...
                if( groupGraph.isGroup( selectedNodes[ 0 ] ) ) {
                    workflowDgmCtx.isGroupSelectedNode = true;
                }
            }
        }
    }
 ...
 *
 * @memberof NgServices
 * @member Awp0WorkflowDesignerGraphService
 */
app.factory( 'Awp0WorkflowDesignerGraphService', () => exports );
