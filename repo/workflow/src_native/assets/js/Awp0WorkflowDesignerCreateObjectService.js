...
export let onNodeEditCommitted = function( eventData ) {
    ...
    var templateId = 'WorkflowDummyNodeTemplate';
    if( node && node.style ) {
        templateId = node.style.templateId;
    }
    var isDummy = templateId === 'WorkflowDummyNodeTemplate';
...
var removeDummyNodeOnError = function( graphModel, node ) {
    ...
    // remove dummy node
    if( templateId === 'WorkflowDummyNodeTemplate' ) {
 ...
 */
export let addEdgeOnGraph = function( graphModel, createEdgeResponse ) {
...
export let addNodeOnGraph = function( graphModel, createNodeResponse, data ) {
    ...
    try {
        ...
        if( appCtxSvc.ctx.workflowDgmCtx && appCtxSvc.ctx.workflowDgmCtx.diagram && !appCtxSvc.ctx.workflowDgmCtx.diagram.isAutoLayoutOn ) {
            ...
            eventBus.publish( 'workflowDesigner.CreateNodeUpdateEvent', { createInput: input, openObject: openObject } );
 ...
 *
 * @memberof NgServices
 * @member Awp0WorkflowDesignerDataService
 */
app.factory( 'Awp0WorkflowDesignerCreateObjectService', () => exports );
