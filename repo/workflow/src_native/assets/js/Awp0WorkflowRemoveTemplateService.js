 ...
 * @param {Array} selectedNodes Selected edges array that need to be removed
 * @param {Object} rootProcessTemplate Root process template object
 * @returns {Array} Model object array that can be deleted
 */
export let getRemoveNodeInput = function( ctx, selectedNodes ) {
    ...
    // Update the context with nodes that will be modified
    if( ctx && ctx.workflowDgmCtx ) {
        ctx.workflowDgmCtx.removeGraphItems = nodesToBeDeleted;
    }
 ...
 */
var _getEdgesToBeremoved = function( selectedEdges ) {
 ...
 * @param {Array} selectedEdges Selected edges array that need to be removed
 */
export let removeGraphNodeEdges = function( selectedNodes, selectedEdges ) {
    var allPromises = [];
    ...
    edgesToBeDeleted = _getEdgesToBeremoved( selectedEdges );
 ...
 * @return {Array} SOA input with removed edge information
 */
export let getRemoveEdgeInput = function( ctx, selectedEdges, rootProcessTemplate ) {
    var soaInput = [];
    var edgesToBeDeleted = [];
    if( selectedEdges && selectedEdges.length > 0 && rootProcessTemplate ) {
        ...
        // Update the context with nodes that will be modified
        if( ctx && ctx.workflowDgmCtx ) {
            ctx.workflowDgmCtx.removeGraphItems = edgesToBeDeleted;
        }
 ...
 * @param {Array} objectstoRemove Objects that need to be removed
 */
export let removeGraphElements = function( objectstoRemove ) {
    ...
    if( objectstoRemove && objectstoRemove.length > 0 ) {
        ...
        layoutService.applyGraphLayout( graphModel, true, false );
    }
};
...
/**
 * Define remove element methods
 *
 * @memberof NgServices
 * @member Awp0WorkflowRemoveTemplateService
 */
app.factory( 'Awp0WorkflowRemoveTemplateService', () => exports );
