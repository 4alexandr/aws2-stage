...
export let populatePanelData = function( data ) {
    ...
    var templateDispProp = data.workflowProcessTemplates;
    if( selectedTaskTemplateType === 'task' ) {
        templateDispProp = data.workflowTaskTemplates;
    }

    templateDispProp.dbValue = '';
    templateDispProp.uiValue = '';
    ...
    if( selectedTaskTemplateType === 'workflow' && templatesObjects && noneTemplateList && noneTemplateList.length > 0 ) {
        ...
        if( templatesObjects.length > 0 && templatesObjects[ 0 ] ) {
            templateDispProp.dbValue = templatesObjects[ 0 ];
            templateDispProp.uiValue = templatesObjects[ 0 ].propDisplayValue;
        }
        ...
        if( defaultTaskTemplate && defaultTaskTemplate.propInternalValue ) {
            templateDispProp.dbValue = defaultTaskTemplate.propInternalValue;
            templateDispProp.uiValue = defaultTaskTemplate.propDisplayValue;
            ...
            if( data.taskTemplateObjectTypes ) {
                exports.populateValidTaskTemplateTypes(  data, defaultTaskTemplate.propInternalValue, data.taskTemplateObjectTypes );
            }
        }
    }

    if( selectedTaskTemplateType === 'task' ) {
        data.taskTemplates = templatesObjects;
    } else {
        data.processTemplates = templatesObjects;
    }
...
export let populateValidTaskTemplateTypes = function( data, selectedTaskTemplateProp, templateTypes ) {
    ...
    data.validTaskTemplateTypes = taskTypeList;
 ...
 *
 * @memberof NgServices
 * @member Awp0CreateWorkflowTemplate
 */
app.factory( 'Awp0CreateWorkflowTemplate', () => exports );
