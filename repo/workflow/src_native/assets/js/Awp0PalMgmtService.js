...
export let createPalInputStructure = function( data ) {
    ...
    var palDesc = data.palDescription.dbValues;
    if( palDesc && !palDesc[ 0 ] ) {
        palDesc = [ '' ];
    }
 ...
 */
export let isGroupAdminOrSysAdmin = function( ctx ) {
    if( !ctx.userSession.props.fnd0groupmember || !ctx.userSession.props.fnd0groupmember.dbValue ) {
        return false;
    }
    var deferred = AwPromiseService.instance.defer();
    var groupMember = ctx.userSession.props.fnd0groupmember.dbValue;
    var groupName = ctx.userSession.props.group_name.dbValue;
...
/**
 * Get the PAL object that need to be deleted
 * @param {Object} ctx Context object
 *
 * @returns {Object} PAL object that need to be deleted
 */
export let getPALObjectToDelete = function( ctx ) {
...
app.factory( 'Awp0PalMgmtService', () => exports );
