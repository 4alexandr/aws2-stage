...
var _getConditionTaskUpdateSOAInput = function( epmSetConditionHandler, data, inputData ) {
    ...
    var queryScope = data.queryScope.dbValue;
    var includeReplica = data.includeRepliaceTarget.dbValue;
    var queryName = data.savedQueries.dbValue;
    var queryType = data.queryAganist.dbValue;
    var conditionName = data.savedConditions.uiValue;
    ...
    if( data.queryScope && data.queryScope.isEditable && epmSetConditionHandler ) {
        ...
        if( data.queryCondition.dbValue === 'query' && queryType === 'sub-process' && ( data.queryAganist.value === 'task' || data.queryAganist.value === 'target' ) ||
            data.queryAganist.value === 'sub-process' && ( queryType === 'task' || queryType === 'target' ) ) {
    ...
    if( queryName !== '' && data.queryCondition.dbValue === 'query' ) {
    ...
    if( conditionName !== '' && data.queryCondition.dbValue === 'condition' ) {
 ...
 */
export let saveTemplateProperties = function( refreshPanel ) {
...
/**
 * App Factory for Template properties editing
 */
...
app.factory( 'Awp0TemplatePropertiesEditService', () => exports );
