...
export let workflowTaskAssignmentRendererFn = function( vmo, tableElem, column, rowElem ) {
    ...
    if( cellDBValue && cellDBValue[ 0 ] && cellDispValue ) {
        ...
        if( cellImg ) {
            var iconImageDiv = _t.util.createElement( 'div', _t.Const.CLASS_GRID_CELL_IMAGE );
        ...
        if( cellDBValue[ 0 ]  === 'unstaffedUID' && vmo.assignmentObject && vmo.assignmentObject.isProfileAssignment ) {
            let imagePath = app.getBaseUrlPath() + '/image/indicatorRequired16.svg';
            ...
            if( celRequiredImg ) {
                var requiredImageDiv = _t.util.createElement( 'div', _t.Const.CLASS_GRID_CELL_IMAGE );
...
app.factory( 'workflowTaskAssignmentCellRenderer', () => exports );
