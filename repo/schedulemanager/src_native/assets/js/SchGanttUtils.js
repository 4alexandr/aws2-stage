...
import ganttEventHandler from 'js/SMGantt/uiSchGanttEventHandler';
...
export let isFinishDateScheduleForTask = function( id ) {
    let result = false;
    let task = dataSource.instance.getTask( id );
    let answer = null;
    let schedule = null;
    if( task ) {
...
export let onTaskDrag = function( id, startDate, endDate, mode ) {
    let taskObject = dataSource.instance.getTask( id );
...
export let reorderTasks = function( moveRequest ) {
    ...
    if( prevSibling &&
        ( prevSibling.props.task_type.dbValues[ '0' ] === '2' || prevSibling.props.task_type.dbValues[ '0' ] === '6' ) ) {
        var prevSiblingIndex = ganttManager.getGanttInstance().getTaskIndex( moveRequest.prevSibling.uid );
        ganttManager.getGanttInstance().moveTask( moveRequest.task.uid, prevSiblingIndex + 1,
            moveRequest.newParent.uid );
    }
...
export let resetGanttConfigForBaseline = function() {
    ganttManager.getGanttInstance().config.task_height = dataSource.instance.hasBaseline() ? 12 : 'full';
...
export let getOriginalTasksForBaseline = function( ctx, eventMap ) {
    var originalTasks = [];
    if( eventMap && eventMap.loadBaselineEvent && eventMap.loadBaselineEvent.hasBaselineChanged && eventMap.loadBaselineEvent.hasBaselineChanged === true ) {
        originalTasks = dataSource.instance.getTaskObjectArray();
        return originalTasks;
    }
    // In case of first page, add the schedule summary task.
    if( ctx.smGanttCtx.searchStartIndex <= 0 ) {
        var schSummaryTaskBO = cdm.getObject( dataSource.instance.getSourceObject().props.fnd0SummaryTask.dbValues[ 0 ] );
        originalTasks.push( { type: schSummaryTaskBO.type, uid: schSummaryTaskBO.uid } );
    }
    if( ctx.searchResponseInfo ) {
        var results = ctx.searchResponseInfo.searchResults;
        if( results ) {
            results.forEach( function( businessObject ) {
                if( businessObject.modelType.typeHierarchyArray.indexOf( 'ScheduleTask' ) > -1 ) {
                    originalTasks.push( { type: businessObject.type, uid: businessObject.uid } );
                }
            } );
        }
    }
...
/**
 * This function will populated the proxy Map.
 *
 * @param {Object} proxies - The proxy tasks.
 * @return {Object} proxyMap - A Map of the reference task UID and its List of Proxy.
 */
export let populateProxyMap = function( proxies ) {
    var proxyMap = {};
    if( proxies ) {
        proxies.forEach( function( proxy ) {
            var refTaskProp = proxy.props.fnd0ref.dbValues[ 0 ];
            var refTask = cdm.getObject( refTaskProp );
            if( refTask ) {
                var refProxies = proxyMap[ refTask.uid ];
                if( !refProxies ) {
                    refProxies = [];
                }
                if( refProxies.indexOf( proxy ) < 0 ) {
                    refProxies.push( proxy );
                }
                proxyMap[ refTask.uid ] = refProxies;
            }
        } );
    }
    return proxyMap;
...
export let createNewTask = function( selectedTaskUID, createdObject, data ) {
    ...
    if( parentTask && dataSource.instance.isPaginationCompletedForParent( parentTask.uid ) && !prevSiblingUID && ganttManager.getGanttInstance().hasChild( parentUID ) ) {
        ...
        let ganttTasks = dataSource.instance.constructGanttTasks( schTasksList, parentTasksList, data.ganttColumns, null );
 ...
 */
export let toggleGanttSummary = function() {
    var xrtContext = ctxService.getCtx( 'ActiveWorkspace:xrtContext' );
    var showGanttSummary = true;
    if( !xrtContext ) {
        xrtContext = {
            ganttWithDetails: true
        };
        ctxService.registerCtx( 'ActiveWorkspace:xrtContext', xrtContext );
    } else {
        showGanttSummary = !xrtContext.ganttWithDetails;
    }

    ctxService.ctx.activeSplit = showGanttSummary;
    xrtContext.ganttWithDetails = showGanttSummary;
    ctxService.updateCtx( 'ActiveWorkspace:xrtContext', xrtContext );

    var selectedTask = cdm.getObject( getSelectedTaskID() );

    if( showGanttSummary && selectedTask ) {
        ctxService.updateCtx( 'splitXrtViewModel', selectedTask );

        // Update the selection after the split view model is loaded.
        var selectionEvent = eventBus.subscribe( 'appCtx.register', function( eventData ) {
            if( eventData.name === 'xrtSummaryContextObject' ) {
                if( eventData.value && eventData.value.uid === selectedTask.uid ) {
                    eventBus.unsubscribe( selectionEvent );
                    if( ctxService.ctx.smGanttCtx && dataSource.instance.getSourceObject() ) {
                        selectionSvc.updateSelection( selectedTask, dataSource.instance.getSourceObject() );
                    }
                }
            }
        } );
    } else if( showGanttSummary ) {
        ctxService.updateCtx( 'splitXrtViewModel', dataSource.instance.getSourceObject() );
    } else {
        ctxService.updateCtx( 'splitXrtViewModel', null );
    }

    ganttSummaryEvents();

    ganttEventHandler.updateGanttHeight();
};

export let ganttSummaryEvents = function() {
    var locationChangeEvent = eventBus.subscribe( '$locationChangeStart', function() {
        ganttSummaryReset( true );
    } );
    _events.push( locationChangeEvent );

    var selectionChangeEvent = eventBus.subscribe( 'primaryWorkArea.selectionChangeEvent', function() {
        ganttSummaryReset( true );
    } );
    _events.push( selectionChangeEvent );

    var arrangeEvent = eventBus.subscribe( 'columnArrange', function( eventData ) {
        let isGantt = true;
        if( eventData && eventData.objectSetUri !== 'Saw1GanttColumns') {
            isGantt = false;
        }
        ganttSummaryReset( isGantt );
    } );
    _events.push( arrangeEvent );

    var objSelectionChangeEvent = eventBus.subscribe( 'objectSet.selectionChangeEvent', function( eventData ) {
        var xrtContextObject = ctxService.getCtx( 'xrtPageContext' );
        if ( xrtContextObject.primaryXrtPageID === 'tc_xrt_Gantt' && xrtContextObject.secondaryXrtPageID ) {
            if ( eventData.source && eventData.source === 'primaryWorkArea' ) {
                let taskUid = ganttManager.getGanttInstance().getSelectedId();
                let task = cdm.getObject( taskUid );
                if ( !task ) {
                    task = ctxService.ctx.locationContext.modelObject;
                }
                var selectedObjects = eventData.selected;

                if ( selectedObjects.length === 0 ) {
                    selectionSvc.updateSelection( task, dataSource.instance.getSourceObject() );
                } else {
                    ctxService.ctx.pselected = task;
                }
            }
        }
    } );
    _events.push( objSelectionChangeEvent );
};

export let ganttSummaryReset = function( isGantt ) {
    var xrtContext = ctxService.getCtx( 'ActiveWorkspace:xrtContext' );
    if( xrtContext && xrtContext.ganttWithDetails === true ) {
        xrtContext.ganttWithDetails = false;
        ctxService.updateCtx( 'ActiveWorkspace:xrtContext', xrtContext );
        if( isGantt ) {
            ctxService.updateCtx( 'splitXrtViewModel', null );
        }
...
export let unsubscribeGanttSummaryEvents = function() {
    for( var i = 0; i < _events.length; i++ ) {
        eventBus.unsubscribe( _events[i] );
    }
    _events = [];
};

export let showGanttSummary = function() {
    if( ctxService.ctx.activeSplit ) {
        var xrtContext = ctxService.getCtx( 'ActiveWorkspace:xrtContext' );
        if( !xrtContext ) {
            xrtContext = {
                ganttWithDetails: true
            ...
            xrtContext.ganttWithDetails = true;
        }
        ctxService.registerCtx( 'ActiveWorkspace:xrtContext', xrtContext );

        let ctxx =  ctxService.ctx;
        var contextObj = ctxService.ctx.locationContext.modelObject;
        if ( contextObj ) {
            ctxService.updateCtx( 'splitXrtViewModel', contextObj );
        }
        ganttSummaryEvents();
    }
};
 ...
 *
 * @memberof NgServices
 * @member SchGanttUtils
 */
app.factory( 'SchGanttUtils', () => exports );
