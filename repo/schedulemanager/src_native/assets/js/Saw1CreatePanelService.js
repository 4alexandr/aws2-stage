 ...
 */
export let getScheduleTasks = function( data ) {
    var input = [];
    var inputData;
...
var getScheduleCustomProperties = function( data, defaultProperties, viewModelProperties ) {
    ...
    if( data.preferences.AWC_SM_Unit_Of_Time_Measure && data.preferences.AWC_SM_Unit_Of_Time_Measure[ 0 ] ) {
        attribute.push( {
            key: 'saw1UnitOfTimeMeasure',
            value: data.preferences.AWC_SM_Unit_Of_Time_Measure[ 0 ],
            type: 1
        } );
    }
 ...
 */
export let getTypedAttributesContainer = function( data, defaultProperties ) {
    var customProperties = [];
    var viewModelProperties = [];
    for( var i in data.vmo.props ) {
        if( data.vmo.props.hasOwnProperty( i ) ) {
            viewModelProperties.push( data.vmo.props[ i ] );
        }
    }
 ...
 * @return {array} customProperties - Custom properties array
 */
export let getScheduleTypedAttributesContainer = function( data, defaultProperties ) {
    var customProperties = [];
    var viewModelProperties = [];
    for( var i in data.vmo.props ) {
        if( data.vmo.props.hasOwnProperty( i ) ) {
 ...
 */
export let getMoveTaskContainer = function( data, targetTask ) {
    {
        var moveRequests = [];

        for( var index = 0; index < data.sourceObjects.length; index++ ) {
            var selectedTaskObj = data.sourceObjects[ index ];
 ...
 * @param {object} data - data containing information for master schedule and ref task.
...
export let createNewProxyTaskContainersFromTab = function( data, ctx ) {
    ...
    let selectedObjs = [];
    if( data.selectedTab && data.selectedTab.panelId === 'Saw1CreateProxyTaskSub' ) {
        selectedObjs = data.dataProviders.scheduleTasksProvider.selectedObjects;
    } else if ( data.selectedTab && data.selectedTab.panelId === 'paletteTabPageSub' ) {
        if( ctx.getClipboardProvider.selectedObjects.length > 0 ) {
            for ( let index = 0; index < ctx.getClipboardProvider.selectedObjects.length; index++ ) {
                if ( isScheduleTask( ctx.getClipboardProvider.selectedObjects[index] ) ) {
                    selectedObjs.push( ctx.getClipboardProvider.selectedObjects[index] );
                }
            }
        }

        if( ctx.getFavoriteProvider.selectedObjects.length > 0 ) {
            for ( let index = 0; index < ctx.getFavoriteProvider.selectedObjects.length; index++ ) {
                if ( isScheduleTask( ctx.getFavoriteProvider.selectedObjects[index])) {
                    selectedObjs.push(ctx.getFavoriteProvider.selectedObjects[index]);
                }
            }
        }

        if( ctx.getRecentObjsProvider.selectedObjects.length > 0 ) {
            for ( let index = 0; index < ctx.getRecentObjsProvider.selectedObjects.length; index++ ) {
                if ( isScheduleTask( ctx.getRecentObjsProvider.selectedObjects[index])) {
                    selectedObjs.push(ctx.getRecentObjsProvider.selectedObjects[index]);
                }
            }
        }
    ...
    for ( let index1 = 0; index1 < selectedObjs.length; index1++ ) {
        proxyTaskContainers.push( {
            schedule: data.masterSchedule,
            sublevels: 0,
            taskTag: selectedObjs[ index1 ],
            refTag: data.refTask
        } );
    }
 ...
 */
export let getCreateSchedulePanel = function( commandId, location, paletteAndSearchData ) {
    var Object = 'Object';
...
/**
 * Set context data for proxy task creating.
 * @param {Object} selectedObj - schedule or schedule task selected.
 * @param {object} data  - view model data caching context data.
 */
export let setProxyTaskContext = function( selectedObj, data ) {
    if ( isSchedule( selectedObj ) || isScheduleTask( selectedObj ) ) {
        var schedule = getSchedule( selectedObj );

        data.masterSchedule = schedule;
        data.refTask = getScheduleTask( selectedObj );
    }
};
 ...
 */
export let getCreateSchTaskProperties = function( commandId, location ) {
    var Object = 'Object';
 ...
 */
export let getNewSchedules = function( data, defaultProperties ) {
    ...
    for( i = 0; i < viewModelProperties.length; i++ ) {
        var defaultAttribute = false;
        var j = 0;
    ...
    attribute.type = data.objCreateInfo.createType;
    attribute.priority = 3;
    attribute.notificationsEnabled = true;
    attribute.typedAttributesContainer = [ {
        type: 'ScheduleType',
        attributes: exports.getScheduleTypedAttributesContainer( data, defaultProperties )
    } ];
...
export let populateDefaultValuesForSchedule = function( data ) {
    ...
    if( timeZoneProp && data.preferences.SiteTimeZone && data.preferences.SiteTimeZone.length > 0 ) {
        var timeZone = data.preferences.SiteTimeZone[ 0 ];
...
app.factory( 'Saw1CreatePanelService', () => exports );
