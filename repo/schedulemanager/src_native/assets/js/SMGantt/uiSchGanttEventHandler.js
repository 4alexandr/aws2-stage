...
export let registerGanttEvents = function( dataProcessor, data ) {
    ...
    var beforeTaskDragEvent = ganttManager.getGanttInstance().attachEvent( 'onBeforeTaskDrag',
        function( id, mode, e ) {
            ...
            if( mode === 'resize' ) {
                ...
                if( !isFinishDateSchedule && element === 'gantt_task_drag task_right' || isFinishDateSchedule && element === 'gantt_task_drag task_left' ) {
                    canDrag = true;
                }
            } else {
                ...
                if( taskExists === true ) {
                    ...
                    // Allow dragging of Standard, Milestone, Phase and Gate task type
                    if( task.taskType === 0 || task.taskType === 1 || task.taskType === 3 || task.taskType === 4 ) {
                        canDrag = true;
                    }
                }
            }
    ...
    var onTaskClick = ganttManager.getGanttInstance().attachEvent( 'onTaskClick', function( id, e ) {
        ...
        if( appCtx.ctx.activeSplit === true ) {
            var selectedTasks = ganttManager.getGanttInstance().getSelectedTasks();
            var task = cdm.getObject( selectedTasks[0] );
            if( selectedTasks && selectedTasks.length > 1 ) {
                appCtx.updateCtx( 'splitXrtViewModel',  null );
            } else if( !task ||  selectedTasks.length === 1 && appCtx.ctx.mselected.length === 1 && task.uid === appCtx.ctx.mselected.uid  ) {
                appCtx.updateCtx( 'splitXrtViewModel',  _dataSource.getSourceObject() );
            } else {
                appCtx.updateCtx( 'splitXrtViewModel',  task );
            }
        }
        if( nTaskClicks === 0 ) {
            var prevSelectedId = ganttManager.getGanttInstance().getSelectedId();

            setTimeout( function() {
                if( nTaskClicks === 1 && prevSelectedId === id ) {
                    if( $( e.target ).closest( '.gantt_tree_icon, .ganttInlineEditor' ).length <= 0 ) {
                        ganttManager.getGanttInstance().unselectTask( id );
                        nTaskClicks = 0;
                        return false;
                    }
                }
    ...
    var taskDblClickEvent = ganttManager.getGanttInstance().attachEvent( 'onTaskDblClick', function( id, e ) {
        if( $( e.target ).closest( '.ganttInlineEditor' ).length <= 0 && $( e.target ).closest( '.gantt_cell' ).length > 0 ) {
            event.stopPropagation();
            eventBus.publish( 'gantCellDblClicked', event );
        } else {
            return true;
        }
    } );
    _events.push( taskDblClickEvent );
...
export let registerAWEvents = function( timelineDataSource ) {
    var subLocationChangeEvent = eventBus.subscribe( 'GanttXRT.contentUnloaded', function( eventData ) {
        _dataSource.cleanup();
    } );
...
splitterUpdate = eventBus.subscribe( 'aw-splitter-update', function( eventData ) {
    ...
    var smGanttEle = document.getElementsByClassName( 'smgantt' );
    if( smGanttEle ) {
        var updatedHeight = eventData.area1.clientHeight - 65;
        ganttHeightInPercentage = updatedHeight / height;
        if( appCtx.ctx.xrtPageContext.secondaryXrtPageID && appCtx.ctx.xrtPageContext.secondaryXrtPageID === 'tc_xrt_Gantt' ) {
            updatedHeight = eventData.area1.clientHeight - 51; //offset for list in summary view
        }
        smGanttEle[ 0 ].style.height = updatedHeight + 'px';
        var smGanttParent = smGanttEle[ 0 ].parentElement;
        if( smGanttParent && smGanttParent.parentElement ) {
            smGanttParent.parentElement.style.flexBasis = 'auto';
        }
        var splitGanttElement = document.getElementById( 'splitGantt' );
        var element = $( splitGanttElement );
        element[ 0 ].style.height = eventData.area2.clientHeight + 'px';
    }
...
/**
 * This will compute the available height and update it for Gantt.
 */
export let updateGanttHeight = function() {
    var height = exports.getComputedHeight();
    var ganttElement = document.getElementsByClassName( 'smgantt' );
    if( ganttElement ) {
        var splitGanttElement = document.getElementById( 'splitGantt' );
        if( appCtx.ctx.activeSplit && splitGanttElement ) {
            if( !ganttHeightInPercentage ) {
                ganttHeightInPercentage = 0.6;
            }
            var ganttHeight = Math.round( ganttHeightInPercentage * height );

            ganttElement[ 0 ].style.height = ganttHeight - 30 + 'px';
            var ganttParentElement = ganttElement[ 0 ].parentElement;
            if( ganttParentElement && ganttParentElement.parentElement ) {
                ganttParentElement.parentElement.style.flexBasis = 'auto';
            }
            var element = $( splitGanttElement );
            element[ 0 ].style.height = height - ganttHeight + 'px';
        } else {
            ganttElement[ 0 ].style.height = height + 'px';
        }
    }
    ganttManager.getGanttInstance().setSizes();
    ganttManager.getGanttInstance().render();
...
export let getComputedHeight = function() {
    ...
    //XRT tabs will be part of sublocationContent only when the Program is selected and Gantt is opened in summary
    // instead of directly opening Schedule and navigating to Gantt sublocation. So reduce that height as well.
    var xrtTabHeight = 0;
    if( appCtx.ctx.xrtPageContext.secondaryXrtPageID && appCtx.ctx.xrtPageContext.secondaryXrtPageID === 'tc_xrt_Gantt' ) {
        var xrtTabs = document.getElementsByClassName( 'aw-xrt-tabsContainer' )[ 0 ];

        xrtTabHeight = xrtTabs ? xrtTabs.clientHeight : 0;
        if( isNaN( xrtTabHeight ) ) {
            xrtTabHeight = 0;
        }
    }
    var filterBreadCrumb = document.getElementsByName( 'Saw1GanttBreadcrumbPanel' );
    var filterHeight = filterBreadCrumb ? filterBreadCrumb[ 0 ] ? filterBreadCrumb[ 0 ].clientHeight + 20 : 0 : 0;
    if( isNaN( filterHeight ) ) {
        filterHeight = 0;
    }
    var columnHeight = 19 + 80; //115 to offset  for margins at top and bottom
    var height = subLocHeight - columnHeight - xrtTabHeight - filterHeight;
    if( height < 200 ) {
        height = 200;
    }
    return height;
