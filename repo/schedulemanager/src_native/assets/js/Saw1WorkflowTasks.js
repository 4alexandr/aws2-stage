...
export let openUserPanel = function( data, isFromSectionSelected ) {
    ...
    var panelTitle = data.i18n.PrivilegedUser;

    if( !isFromSectionSelected ) {
        panelTitle = data.i18n.WorkflowOwner;
    }
    var destPanelId = 'Saw1ResourcePoolGanttWrapper';
    var context = {
 ...
 * @param {object} isFromSectionSelected - the current selection object
 */
export let IsPrivilegedOrWorkflowOwner = function( ctx, data ) {
    if( data.selectedTab.panelId === 'Saw1ResourcePoolGantt' ) {
        data.priviledgeUser = data.dataProviders.getResourcePool.selectedObjects[ 0 ].uid;

        var resourcePoolModelObject = cdm.getObject( data.priviledgeUser );
        data.priviledgeUser = [ viewModelObjectService.constructViewModelObjectFromModelObject(
            resourcePoolModelObject, 'EDIT' ) ];
        data.dataProviders.getAssignedPrivilegedUser.viewModelCollection.loadedVMObjects = data.priviledgeUser;

        eventBus.publish( 'priviledgeUser.assignUsers' );
    } else {
        if( data.isFromSectionSelected === true ) {
            data.priviledgeUser = data.dataProviders.userPerformSearch.selectedObjects[ 0 ].props.user.dbValues[ 0 ];
            var priviledgeUserModelObject = cdm.getObject( data.priviledgeUser );
            data.priviledgeUser = [ viewModelObjectService.constructViewModelObjectFromModelObject(
                priviledgeUserModelObject, 'EDIT' ) ];
            data.dataProviders.getAssignedPrivilegedUser.viewModelCollection.loadedVMObjects = data.priviledgeUser;

            eventBus.publish( 'priviledgeUser.assignUsers' );
        } else {
            data.workflowOwner = data.dataProviders.userPerformSearch.selectedObjects[ 0 ].props.user.dbValues[ 0 ];
            var workflowOwnerModelObject = cdm.getObject( data.workflowOwner );
            data.workflowOwner = [ viewModelObjectService.constructViewModelObjectFromModelObject(
                workflowOwnerModelObject, 'EDIT' ) ];
            data.dataProviders.getAssignedWorkflowOwner.viewModelCollection.loadedVMObjects = data.workflowOwner;
            eventBus.publish( 'workflowOwner.assignUsers' );
        }
    }
};
...
export let updateTasks = function( schedule, ctx, data ) {
    ...
    if( data.workflowOwner !== undefined ) {
        workflowOwner = data.workflowOwner[ 0 ].uid;
    }
    if( data.priviledgeUser !== undefined ) {
        priviledgeUser = data.priviledgeUser[ 0 ].uid;
    }
...
var getExternalTemplates = function() {
    var ctx = appCtxService.ctx;
    var templateValues = ctx.preferences !== undefined && ctx.preferences.ProgramWorkFlowTemplates !== undefined ? appCtxService.ctx.preferences.ProgramWorkFlowTemplates : '';
 ...
 *
 * @memberof NgServices
 * @member Saw1WorkflowTasks
 */
app.factory( 'Saw1WorkflowTasks', () => exports );
