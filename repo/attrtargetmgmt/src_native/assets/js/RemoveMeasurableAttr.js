...
export let createInputForRemoveParameters = function( isDeleteParameter ) {
    ...
    if( paramProjectSelectedParameters ) {
        ...
        inputs = [ { clientId: 'AW_Client_RemoveParameter', parent: selectedParent, selectedParams: _.get( appCtxSvc, 'ctx.mselected', [] ) } ];
        //in case of Ace
    } else if( selectedParamsFromPwa.length > 0 ) {
...
var getInputInProjectOrGroup = function( parentOfInterests ) {
    ...
    _.forEach( parentOfInterests, function( parent ) {
        ...
        var attachedAttributesToParent = paramProjectParameters.map( function( parameter ) {
            return cdm.getObject( _.get( parameter, 'props.att1SourceAttribute.dbValues[0]', undefined ) );
        } );
...
export let createInputForValueFileCut = function() {
    ...
    if( selectedProxyCountInPWA === 1 && mselected.length > 0 && mselected[0].modelType.typeHierarchyArray.indexOf( 'Dataset' ) > -1  ) {
            parent = cdm.getObject( _.get( selectedProxyParameters[0], 'props.att1SourceAttribute.dbValues[0]', undefined ) ); //Parameter selected in Project tree PWA
...
var getInputForParameterSelectionFromTree = function() {
    ...
    if( selectedProxyParameters.length > 0 ) {
        _.forEach( selectedProxyParameters, function( parameter ) {
            ...
            var attributeObject = cdm.getObject( _.get( parameter, 'props.att1SourceAttribute.dbValues[0]', undefined ) );
...
export let displayCutParametersMessage = function( data ) {
    ...
    var msg = '';
    if( selectedParams.length === 1 && data.cutParameterFrameworkSoaResponse.ServiceData.deleted && data.cutParameterFrameworkSoaResponse.ServiceData.deleted.length === 1 ) {
    ...
    } else if( selectedParams.length > 1 && data.cutParameterFrameworkSoaResponse.ServiceData.deleted && data.cutParameterFrameworkSoaResponse.ServiceData.deleted.length > 1 ) {
...
app.factory( 'RemoveMeasurableAttr', () => exports );
