 ...
 * getOpenedObject
 */
function getOpenedObject( inInfoPanel ) {
    ...
    if( inInfoPanel ) {
        ...
        openedObject = locationCtx.modelObject;
    }
...
export let isAttrOpenedAndOverridden = function( eventMap ) {

    var activeToolAndInfoCmd = appCtxSvc.getCtx( "activeToolsAndInfoCommand" );
    var inInfoPanel = ( activeToolAndInfoCmd && activeToolAndInfoCmd.commandId === "Awp0ObjectInfo" );

    var openedObject = getOpenedObject( inInfoPanel );

    if( !cmdMapSvc.isInstanceOf( 'Att0MeasurableAttribute', openedObject.modelType ) ) {
        return;
    }
...
app.factory( 'Att1MeasurableAttributeService', () => exports );
