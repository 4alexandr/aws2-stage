            ...
            if( cmm.isInstanceOf( 'Att0MeasurableAttribute', response.output[ i ].inputObject.modelType ) &&
                ...
                dms.getProperties(
                    [ response.output[ i ].relationshipData[ j ].relationshipObjects[ 0 ].otherSideObject.uid ], [
                        'object_name', 'original_file_name'
                    ] );
                goalFile
                    .push( cdm
...
export let openFileSelectDialog = function( vmo ) {
    ...
    appCtxSvc.registerCtx( 'datasetFileName', temp[ 0 ] );
    appCtxSvc.registerCtx( 'datasetFileExt', temp[ 1 ] );
    appCtxSvc.registerCtx( 'datasetFileFullName', datasetFileFullName );
    elements.fmsFile.click();
};
...
/**
 * Returns the Att1DatasetService instance
 *
 * @member Att1DatasetService00
 */
...
app.factory( 'Att1DatasetService', () => exports );
