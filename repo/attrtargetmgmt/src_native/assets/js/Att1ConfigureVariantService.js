...
var exports = {};
export let initializeConsumerAppsData = function() {
    var hasVariantConfigContext = paramMgmtSvc.isVariantConfigurationContextAttached();
    if( hasVariantConfigContext ) {
        _.set( appCtxSvc, 'ctx.parammgmtctx.hasVariantConfigContext', hasVariantConfigContext );
        _.set( appCtxSvc, 'ctx.variantConditionContext.consumerAppsLoadDataInProgress', true );
    }
};

export let loadParameterVariants = function( hasVariantConfigContext, isSelectionChange ) {
    if( hasVariantConfigContext ) {
        var locationContextTypeHierarchyArray = _.get( appCtxSvc, 'ctx.locationContext.modelObject.modelType.typeHierarchyArray', [] );
        if( locationContextTypeHierarchyArray.indexOf( 'Att0ParamProject' ) > -1 || locationContextTypeHierarchyArray.indexOf( 'Att0ParamGroup' ) > -1 ) {
            ...
            if( !selected.modelType || selected.modelType.typeHierarchyArray.indexOf( 'Att1AttributeAlignmentProxy' ) <= -1 ) {
                selectedProxies = appCtxSvc.getCtx( 'parammgmtctx.selectedProxyParams' );
            }
...
export let setVariantConfigContext = function() {
    ...
    //also check if configuratorContext Object is Attached or Not
        if( hasVariantConfigContext ) {
            ...
            _.set( appCtxSvc, 'ctx.parammgmtctx.hasVariantConfigContext', false );
        }
};

/**
 * sets the parameter context also attach the config perspective with this context
 */
export let registerParameterCtx = function() {
    var parammgmtctx = appCtxSvc.getCtx( 'parammgmtctx', parammgmtctx );
    var openedProject = paramMgmtSvc.getOpenedParamProject();

    if( !parammgmtctx ) {
        appCtxSvc.registerCtx( 'parammgmtctx', {
            configPerspective: {},
            paramProject: openedProject,
            selectedParams: [],
            selectedParamGroups: [],
            selected: appCtxSvc.getCtx( 'locationContext' ).modelObject,
            mselected: [ appCtxSvc.getCtx( 'locationContext' ).modelObject ]
        } );
    } else {
        parammgmtctx.configPerspective = {};
        parammgmtctx.paramProject = openedProject;
        parammgmtctx.selectedParams = [];
        parammgmtctx.selectedParamGroups = [];
        parammgmtctx.selected = appCtxSvc.getCtx( 'locationContext' ).modelObject;
        parammgmtctx.mselected = [ appCtxSvc.getCtx( 'locationContext' ).modelObject ];
        appCtxSvc.updatePartialCtx( 'parammgmtctx', parammgmtctx );
    }
    if( paramMgmtSvc.isVariantConfigurationContextAttached() ) {
        _.set( appCtxSvc, 'ctx.parammgmtctx.hasVariantConfigContext', true );
    }
    paramMgmtSvc.resetParentAccess();
};
...
/**
 * Unregister the parameter context
 */
export let unregisterParameterCtx = function() {
    appCtxSvc.unRegisterCtx( 'parammgmtctx' );
    appCtxSvc.unRegisterCtx( 'openedARObject' );
    // it is used by Relation Browser
...
export let cacheConfigPerspective = function( response ) {
    if( response.configPerspective ) {
        var parammgmtctx = appCtxSvc.getCtx( 'parammgmtctx' );
        parammgmtctx.configPerspective = response.configPerspective;
        appCtxSvc.updateCtx( 'parammgmtctx', parammgmtctx );
...
export default exports = {
    ...
    registerParameterCtx,
    clearPWASelection,
    unregisterParameterCtx,
    getSelectionForVariantContext,
    getCreatedVariantRule,
    cacheConfigPerspective,
    getConfigurationMode
};
app.factory( 'Att1ConfigureVariantService', () => exports );
