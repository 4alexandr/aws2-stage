...
export function getConfiguredObject( uid ) {
    ...
    return loadObjects( [ uid ] ).then(
        ( response ) => {
            return loadConfiguredObject( response.modelObjects[ uid ] );
        }
    );
}
 ...
 */
export function loadObjects( uids ) {
    return ngpSoaSvc.executeSoa( 'Core-2007-09-DataManagement', 'loadObjects', { uids } );
}
 ...
 * @return {promise} a promise object
 */
export function ensureObjectsLoaded( uids ) {
