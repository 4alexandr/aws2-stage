...
import ngpHeaderSvc from 'js/services/ngpHeaderService';
...
function resetNgpContext() {
    ...
    eventBus.publish( 'ngp.scopeObjectCleared' );
}
...
function isLoadingCurrentScopeObject() {
    ...
    if( modelObject ) {
        return modelObject.uid === AwStateService.instance.params.uid;
    }
    return false;
}
...
function initNGP() {
    ...
    ngpHeaderSvc.init();
    ngpStorageSvc.init();
    eventListeners.push( eventBus.subscribe( 'LOCATION_CHANGE_COMPLETE', loadModel ) );
    eventListeners.push( eventBus.subscribe( 'navigateToObjectFromHostedContent', navigationFromHostedContent ) );
    revisionRuleChangeEventListener = eventBus.subscribe( 'context.awGlobalRevisionChange', onRevisionRuleChange );
        ...
        if( !isLoadingCurrentScopeObject() ) {
            ...
            ngpLoadSvc.getConfiguredObject( state.params.uid ).then(
                ( configuredObj ) => {
                    ...
                    ngpHeaderSvc.updateHeader( configuredObj );
                    if( state.params.uid !== configuredObj.uid ) {
                        state.go( state.current.name, { uid: configuredObj.uid }, { inherit: false, reload: false, notify: false, location: 'replace' } );
                    }
            ...
            ngpLoadSvc.refreshObjects( [ scopeObject ] );
        }
