...
export function getProductEffectivityValue( modelObject ) {
    if( modelObject ) {
        if( modelObject.props[ ngpPropConst.MANUFACTURING_MODEL ] ) {
    ...
    console.warn( 'Tried to load product effectivity value with null model object or property mdl0model_object not loaded' );
    return new Promise( ( resolve ) => {
        resolve( '' );
    } );
}
