...
export const reloadRevisedObjects = function( eventdata ) {
    for( let i = 0; i < eventdata.revisedObjects.length; i++ ) {
        ...
        soaSvc.post( 'Internal-ActiveWorkspaceBom-2020-05-OccurrenceManagement', 'updateContentBasedOnRevision', inputData );
    }
};

/**
 * It will store the selected objects in provided key path
 *
 * @param {String} selectedObjects grid id of table which will be reloded.
 * @param {String} key Context key path.
 * 
 */
export const updateSelectedObjectUids = function( key, selectedObjects ) {
    let selectedUids = [];
    _.forEach( selectedObjects, function( obj ) {
        selectedUids.push( obj.uid );
    } );
    appCtxSvc.updatePartialCtx( key, selectedUids );
};
