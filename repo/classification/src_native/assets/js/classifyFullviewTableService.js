...
function createPolymorphicPropForInstance( data, cardinalAttr, vmObject, columnInfo, columnNdx ) {
    ...
    var instVmObject = instance.polymorphicTypeProperty.vmos[ 0 ];
        ...
        AwRootScopeService.instance.$watch( function() {
            ...
            function( newValue, oldValue ) {
                if( newValue !== oldValue ) {
                    ...
                    if( kInd !== -1 ) {
                        instance.polymorphicTypeProperty.vmos[ 0 ].dbValue = vmProp.dbValue;
                        var context = {
                            owningAttribute: instance,
                            cardinalAttribute: cardinalAttr,
                            property: instance.polymorphicTypeProperty.vmos
                        };
...
function _getRowsForCardInstances( data, cardinalAttr, columnConfig ) {
    ...
    for( var i = 0; i < cardinalAttr.instances.length; i++ ) {
        ...
        for( var rowNdx = 0; rowNdx < instObject.children.length; rowNdx++ ) {
            ...
            if( child.vmos ) {
                vmObject = instObject.children[ rowNdx ].vmos[ 0 ];
            ...
            if( !found ) {
                for( var columnNdx = 0; columnNdx < columnConfig.length; columnNdx++ ) {
                    ...
                    if( columnInfo.name !== instObject.name && columnInfo.name !== PROP_COLUMN ) {
                        if( rowInd === -1 ) {
                            instVmObject = ngModule.copy( vmObject );
...
function _getCardinalWithPolyRows( data, cardinalAttr ) {
    ...
    var vmObject = cardinalAttr.polymorphicTypeProperty.vmos[ 0 ];
    if( data.searchResults ) {
 ...
 */
export let loadFlatTableData = function( data, dataProvider, decoration, attribute ) {
    if( !decoration ) {
        decoration = '';
    }
    if( !attribute ) {
        attribute = exports.getCardinalBlock( data.attr_anno );
    }
    ...
    return deferred.promise.then( function() {
        if( data.editPropInProgress || data.panelMode === 0 ) {
...
export let setTableEditable = function( data, dataProvider ) {
    if( data.editPropInProgress || data.editClassInProgress || data.panelMode === 0 || data.panelMode === 1 ) {
        // splm table event
        var context = {
            state: 'starting',
            dataSource: dataProvider
        };
        if ( data.eventData && data.eventData.saveAndExitOperation === true ) {
            context.state = 'canceling';
        }
 ...
 * @returns {Object} data
 */
export let reorderInstances = function( data, attribute ) {
    // exports.updateInstanceData( data, attribute );
    var columns = data.dataProviders.PropDataProvider.columnConfig.columns;
    var tmpArr = [];
    _.forEach( attribute.instances, function( instance ) {
        //find instance in columns
        var ind = _.findIndex( columns, function( col ) {
            return instance.name === col.name;
        } );
 ...
 * @param {Object} attribute - attribute
 */
export let updateInstanceData = function( data, attribute ) {
    ...
    _.forEach( rows, function( row, idx ) {
        _.forEach( attribute.instances, function( instance ) {
            ...
            if( ind !== -1 && row.props[ name ] ) {
                if( attribute.tableView ) {
                    children[ ind ].vmos[ 0 ].dbValue = row.props[ name ].dbValue;
                    children[ ind ].vmos[ 0 ].displayValues = row.props[ name ].displayValues;
                    children[ ind ].vmos[ 0 ].uiValue = row.props[ name ].uiValue;
                } else {
                    row.props[ name ].dbValue = children[ ind ].vmos[ 0 ].dbValue;
                    row.props[ name ].displayValues = children[ ind ].vmos[ 0 ].displayValues;
                    row.props[ name ].uiValue = children[ ind ].vmos[ 0 ].uiValue;
                }
            }
        } );
    } );
 ...
 */
app.factory( 'classifyFullviewTableService', () => exports );
