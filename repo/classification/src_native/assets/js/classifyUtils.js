...
export let CLEARED_WIDGET_DATE_CHROME = '-62135665438000';
...
ClsUtilValueMap.prototype.getRootLevelPropertiesFromAttributesForLocation = function( attributeArray ) {
    ...
    _.forEach( attributeArray, function( attribute ) {
        ...
        if( attribute.type !== 'Block' ) {
            var attributeVmo = attribute.vmos[ 0 ];
        }
    } );
};
...
ClsUtilValueMap.prototype.getRootLevelPropertiesFromAttributes = function( attributeArray, unitSystem ) {
    ...
    _.forEach( attributeArray, ( attribute ) => {
        if( attribute.type !== 'Block' ) {
            ...
            if( exports.isNullOrEmpty( attributeVmo.displayValues ) && !exports.isNullOrEmpty( attributeVmo.dbValue ) && attributeVmo.valueUpdated ) {
            ...
            if ( attribute.vmos.length > COMPLEXVMOMINIMUMLENGTH ) { //Complex
                for ( var i = 0; i < attribute.vmos.length; i++ ) {
                    if ( i !== 1 && i !== 2 ) {
                        ...
                        values.push( exports.addComplexValuesToOutPut( attribute, values, attribute.vmos[i], i ) );
                    }
                }
                var numEmpty = 0;
                ...
                attributeVmo.displayValues.forEach( ( item, valIdx ) => {
                    ...
                    if( dbValue && attributeVmo.type === 'DATE' || attributeVmo.type === exports.DATE_ARRAY ) {
                        ...
                        if( dbValue === exports.CLEARED_WIDGET_DATE_CHROME ||
                            ...
                            if( clsDateFormat === exports.FORMAT_WITH_MMM_DATE ) {
                                var dateNum = parseInt( dbValue );
                                var date = new Date( dateNum );
                                var dateStr = date.toString();
                                dbValue = AwFilterService.instance( 'date' )( dbValue, clsDateFormat );
                                if( dateStr !== '' ) {
                                    var engMonth = dateStr.substring( 4, 7 );
                                    dbValue = dbValue.substring( 0, 3 ) + engMonth +
                                        dbValue.substring( 6 );
                                }
                            } else {
                                dbValue = AwFilterService.instance( 'date' )( dbValue, clsDateFormat );
                        ...
                        if( attributeVmo && attributeVmo.displayValues && attributeVmo.displayValues[ valIdx ] ) {
                            ...
                            if( attribute.vmos && attribute.vmos[ 2 ] && attribute.vmos[ 2 ].names && attribute.vmos[ 2 ].uiValue ) {
                                var selectedUnitDef = 0;
                                var storageUnitDef = 0;
                                var delimiter = ' |~| ';
                                var displayValueString = '';
                                _.forEach( attribute.vmos[ 2 ].unitDefs, function( unit ) {
                                    if( unit.displayName === attribute.vmos[ 2 ].uiValue ) {
                                        selectedUnitDef = unit;
                                    }
                                    if( unit.displayName === attribute.vmos[ 2 ].uiOriginalValue ) {
                                        storageUnitDef = unit;
                                    }
                                } );
                                ...
                                propertyValueObj.displayValue = displayValueString; //attribute.vmos[2].uiValue;
...
function parseValues( values, attributeVmo, attribute, rootLevelProperties ) {
    ...
    if ( hasEmptyVals && attribute.vmos.length > COMPLEXVMOMINIMUMLENGTH ) {
        var skip = false;
        for ( var i = 0; i < values.length; i++ ) {
            ...
            if( attribute.cardinalController ) {
                for( var instanceInx in attribute.instances ) {
                    ...
                    if( attribute.tableView && data.searchResults ) {
                        exports.getAttributeValuesFromTable( data, attribute, instance );
                    }
                    var instanceObject = {
 ...
 */
export let getAttributeValuesFromTable = function( data, attribute, instance ) {
    _.forEach( instance.children, function( child ) {
        ...
        if( index !== -1 ) {
            ...
            if( vmProp.props[ instance.name ].dbValue ) {
                child.vmos[ 0 ].dbValue = vmProp.props[ instance.name ].dbValue;
            }
            child.vmos[ 0 ].displayValues = vmProp.props[ instance.name ].displayValues;
            child.vmos[ 0 ].uiValue = vmProp.props[ instance.name ].uiValue;
        }
    } );
};
...
export let convertValue = function( valueToConvert, unitSystem, numberToString ) {
    ...
    switch ( formatType ) {
        ...
        case 4:
            // Boolean
            var tempValue = valueToConvert !== '' ? valueToConvert === 'true' || valueToConvert === '1' : '';
    ...
    if( serverDateValue && !_.isNumber( serverDateValue ) && serverDateValue !== '' ) {
        ...
        switch ( formatTypeIndex ) {
            ...
            case 4:
                ...
                date = inputServerDate.substring( 0, 2 );
                month = inputServerDate.substring( 3, 5 );
                year = inputServerDate.substring( 6, 10 );
                hours = inputServerDate.substring( 11, 13 );
                minutes = inputServerDate.substring( 14, 16 );
                seconds = inputServerDate.substring( 17, 19 );
            ...
            case 5:
                ...
                date = inputServerDate.substring( 0, 2 );
                month = inputServerDate.substring( 3, 5 );
                year = inputServerDate.substring( 6, 8 );
                hours = inputServerDate.substring( 9, 11 );
                minutes = inputServerDate.substring( 12, 14 );
                seconds = inputServerDate.substring( 15, 17 );
            ...
            case 11:
                ...
                month = inputServerDate.substring( 0, 2 );
                date = inputServerDate.substring( 3, 5 );
                year = inputServerDate.substring( 6, 8 );
                hours = inputServerDate.substring( 9, 11 );
                minutes = inputServerDate.substring( 12, 14 );
                seconds = inputServerDate.substring( 15, 17 );
            ...
            case 14:
                ...
                year = inputServerDate.substring( 0, 4 );
                month = inputServerDate.substring( 5, 7 );
                date = inputServerDate.substring( 8, 10 );
                // 10th position is T
                hours = inputServerDate.substring( 11, 13 );
                minutes = inputServerDate.substring( 14, 16 );
                seconds = inputServerDate.substring( 17, 19 );
...
export let getAttributeValuesFromInstance = function( formattedCardBlock, cardinalIndex, attributeId ) {
    ...
    if( formattedCardBlock.hasBlockChildren && formattedCardBlock.instances.length > 0 && formattedCardBlock.cardinalController ) {
        ...
        if( !exports.isNullOrEmpty( attribute ) ) {
            outputValues.dbValue = attribute.vmos[ 0 ].dbValue;
            outputValues.displayValues = attribute.vmos[ 0 ].displayValues;
        }
    }
 ...
 */
export let resetReleases = function( prop ) {
    getReleasesExpanded( prop, true );
};
...
export let createReleaseList = function( data ) {
    ...
    if ( data.eventMap )
    {
        ...
        {
            getReleasesExpanded( prop, false );
        }
        else
        {
            getReleasesExpanded( prop, true );
 ...
 *
 * @memberof NgServices
 * @member classifyUtils
 */
app.factory( 'classifyUtils', () => exports );
