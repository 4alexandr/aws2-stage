...
export let setCtxAddElementInputParentElementToSelectedElement = function() {
    ...
    addElementInput.parentElement = viewModelObjectService
        .createViewModelObject(  appCtxService.ctx.aceActiveContext.context.topElement );
...
export let shouldFetchColumnConfig = function() {
    var isParentElementIsOpenedElementToAdd = appCtxService.ctx.aceActiveContext.context.openedElement.uid === appCtxService.ctx.selected.uid;
    return isParentElementIsOpenedElementToAdd;
};

/**
 * Process input passed by consumer to the add element
 */
export let mrmProcessAddElementInput = function() {
...
export let populateRequestPref = function() {
    ...
    toParams.depthFirst = [ 'false' ];
    if( stateSvc.params.fd ) {
        toParams.depthFirst[ 0 ] = stateSvc.params.fd;
        toParams.fd = [ stateSvc.params.fd ];
    }

    toParams.restoreAutoSavedSession = [ 'true' ];

    toParams.showUntracedParts = [ 'false' ];
    if( stateSvc.params.showuntraced_part ) {
        toParams.showUntracedParts[ 0 ] = stateSvc.params.showuntraced_part;
    }

    toParams.fetchClientScopeUriProperties = [ 'false' ];
    var viewMode = appCtxService.ctx.ViewModeContext.ViewModeContext;
    if( viewMode === 'TableView' || viewMode === 'TableSummaryView' ) {
        toParams.fetchClientScopeUriProperties[ 0 ] = 'true';
    }
    ...
    toParams.nested = [ 'false' ];
    if( appCtxService.ctx.StructureDisplayModeContext &&
        appCtxService.ctx.StructureDisplayModeContext.StructureDisplayModeContext === 'NestedDisplay' ) {
        toParams.nested[ 0 ] = 'true';
    }
    ...
    if( appCtxService.ctx.aceActiveContext.context.isShowConnection === true ) {
        toParams.includeConnections = [ 'true' ];
    }
    return toParams;
};
...
export let getNewlyAddedSwcChildElements = function( data ) {
    var products = data.addToBookMarkResponse.addedProductsInfo.map( function( productInfo ) {
...
app.factory( 'MrmAddElementService', () => exports );
