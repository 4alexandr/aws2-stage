...
IsMentorQuerySupportedQueryHandler.prototype.handleQuery = function( inputMessage ) {
    ...
    if( !_.isEmpty( dataObjects ) ) {
        ...
        if( queryId ) {
            var isHandled = Boolean( getQueryHandler( queryId ) );
            if( isHandled ) {
 ...
 */
function getQueryHandler( queryId ) {
    ...
    if( handlerInfo ) {
        ...
        if( handlerSvc ) {
            ...
            logger.warn( 'hostMentorQueryService' + ':' + 'Invalid handler constructor' + '\n' +
                JSON.stringify( handlerInfo, null, 2 ) );
        } else {
            logger.warn( 'hostMentorQueryService' + ':' + 'Handler service not found' + '\n' +
                JSON.stringify( handlerInfo, null, 2 ) );
        }
    }
...
app.factory( 'hostMentorQueryService', () => exports );
