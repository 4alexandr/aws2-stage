...
export default class RMParamToReqHandler extends CKEDITOR5.Plugin {
    init() {
...
function executeBalloonPopupHandler( editor, data ) {
    ...
    {
        displayName: localTextBundle.mapExistingParameter,
        internalName: localTextBundle.mapExistingParameter
    } ];
    var eventData = {
        ...
        callback: function( response ) {
            ...
            } else if ( response === localTextBundle.mapExistingParameter ) {
...
function createAsLink( textContent, paramValue, editor ) {
    ...
    model.change( writer => {
        ...
        for ( const range of ranges ) {
            writer.setAttribute( 'paramId', paramValue, range );
        }
    } );
}
