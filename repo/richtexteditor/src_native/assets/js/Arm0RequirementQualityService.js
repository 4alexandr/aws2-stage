...
var _getReuseWebServicePath = function( data ) {
    if( data.preferences.AWC_REQ_Reuse_URL ) {
        var reuseWebServicePath = data.preferences.AWC_REQ_Reuse_URL[ 0 ];
...
export let prePopulateDataForReuseAPI = function( data, option ) {
    if( appCtxSvc.ctx.requirementCtx ) {
...
export let showReqQualityData = function( data ) {
    ckeditorOperations.showReqQualityData( data, _reConnecting );
    _reConnecting = false;
};
...
export let GetPatternGroups = function( data ) {
    ...
    $.ajax( {
        ...
        data: { sessionId: data.ReuseSessionId },
        async: false,
        type: 'POST',
        dataType: 'xml',
        success: function( xml ) {
            ...
            var groups = xml.getElementsByTagName( 'AuthoringPatternGroup' );
            var nullObjectAdded = false;
            for( var i = 0; i < groups.length; i++ ) {
                var obj = {
                    propDisplayValue: groups[ String( i ) ].getElementsByTagName( 'Name' )[ 0 ].textContent,
                    propInternalValue: groups[ String( i ) ].getElementsByTagName( 'Code' )[ 0 ].textContent
                };
    ...
    if( data.patternGroupCombo.dbValue !== '-1' ) {
        $.ajax( {
            ...
            success: function( xml ) {
                ...
                var groups = xml.getElementsByTagName( 'AuthoringPattern' );
                var nullObjectAdded = false;
                for( var i = 1; i < groups.length; i++ ) {
                    ...
                    if( groups[ String( i ) ].getElementsByTagName( 'Examples' ) ) {
                        for( var j = 0; j < groups[ String( i ) ].getElementsByTagName( 'Examples' ).length; j++ ) {
                            obj.Examples.push( groups[ String( i ) ].getElementsByTagName( 'Examples' )[ String( j ) ].textContent );
                        }
                    }
                    nullObjectAdded = !nullObjectAdded ? obj.propInternalValue === '-1' : nullObjectAdded;
...
export let updateNextPossibleTerms = function( patternGroupCode, nextPossibleTerms, matchingState ) {
    if ( patternGroupCode !== '-1' && nextPossibleTerms.length > 0 &&
    matchingState && matchingState.textContent !== 'NotMatched' ) {
        nextPossibleTerms = nextPossibleTerms[0].children;
       var autoCompleteItems = [];
       var i = 0;
       for ( i = 0; i < nextPossibleTerms.length; i++ ) {
       var option = {};
           option.id = String( ' ' + nextPossibleTerms[i].textContent );
...
function updateMatchedPath( xml, rows ) {
    ...
    var found = false;
    var matchedPath = '';
    var matchinState = xml.getElementsByTagName( 'MatchingState' );
    for( i = 0; i < rows.length; i++ ) {
        var row = rows[i];
        if( matchinState && ( matchinState[0].textContent === 'Matched' || matchinState[0].textContent === 'AfterOrBeforeMatch' ) ) {
        matchedPath = xml.getElementsByTagName( 'MatchedPath' )[0].textContent;
    ...
    $.ajax( {
        ...
        success: function( xml ) {
            if( xml.getElementsByTagName( 'Success' )[ 0 ].textContent === 'true' ) {
                if( patternGroupCode !== '-1' && xml.getElementsByTagName( 'MatchingState' )[ 0 ] !== undefined ) {
                    var nextPossibleTerms = xml.getElementsByTagName( 'NextPossibleTerms' );
                    var nextPossibleElements = xml.getElementsByTagName( 'NextPossibleElements' );

                    updateNextPossibleTerms( patternGroupCode, nextPossibleTerms, xml.getElementsByTagName( 'MatchingState' )[ 0 ] );
                    ...
                    if( nextPossibleElements.length > 0 ) {
                        ...
                        for( itPosibleElements = 0; itPosibleElements < nextPossibleElements.length; itPosibleElements++ ) {
                            row = {};
                            var currentMatchedPath = nextPossibleElements[ itPosibleElements ].getElementsByTagName( 'MatchedPath' );
                            if( currentMatchedPath.length > 0 ) {
                                 row.matchedPath = currentMatchedPath[ 0 ].textContent;
                            }
                            row.nextValidRestriction = nextPossibleElements[ itPosibleElements ].getElementsByTagName( 'NextValidRestriction' )[ 0 ].textContent;
                            row.ShortestPathForSelectedPattern = nextPossibleElements[ itPosibleElements ].getElementsByTagName( 'ShortestPathForSelectedPattern' )[0].textContent.trim();
                            if( nextPossibleElements[ itPosibleElements ].getElementsByTagName( 'RestOfShortestPathForSelectedPattern' ).length > 0 ) {
                                 row.restOfShortestPathForSelectedPattern = nextPossibleElements[ itPosibleElements ].getElementsByTagName( 'RestOfShortestPathForSelectedPattern' )[ 0 ].textContent;
                            }
                    ...
                    var matched_info = xml.getElementsByTagName( 'MatchedPatterns' );
                    var matching_info = xml.getElementsByTagName( 'MatchingPatterns' );

                    var matchingPatternsState = [];

                    if( matched_info.length > 0 ) {
                        matched_info = matched_info[ 0 ].children;
                        for( itMatching = 0; itMatching < matched_info.length; itMatching++ ) {
                            row = {};
                            row.status = 'Matched';
                            row.weight = matched_info[ itMatching ].getElementsByTagName( 'Weight' )[ 0 ].textContent;
                            row.patternName = matched_info[ itMatching ].getElementsByTagName( 'Name' )[ 0 ].textContent;

                            matchingPatternsState.push( row );
                        }
                    }

                    if( matching_info.length > 0 ) {
                        matching_info = matching_info[ 0 ].children;

                        for( itMatching = 0; itMatching < matching_info.length; itMatching++ ) {
                            row = {};
                            row.status = 'Matching';
                            row.weight = matching_info[ itMatching ].getElementsByTagName( 'Weight' )[ 0 ].textContent;
                            row.patternName = matching_info[ itMatching ].getElementsByTagName( 'Name' )[ 0 ].textContent;
                ...
                var globalQualityId = xml.getElementsByTagName( 'GlobalQualityId' )[ 0 ].textContent;
                var FeaturesByMetric = xml.getElementsByTagName( 'AuthoringMetric' );
                var itMetrics = 0;
                //for each Metric...
                for( itMetrics = 0; itMetrics < FeaturesByMetric.length; itMetrics++ ) {
                    var metric = FeaturesByMetric[ String( itMetrics ) ];

                    row = {};

                    row.summary = metric.getElementsByTagName( 'Summary' )[ 0 ].textContent;
                    row.metricName = metric.getElementsByTagName( 'MetricName' )[ 0 ].textContent;
                    row.metricValue = metric.getElementsByTagName( 'AbsoluteValue' )[ 0 ].textContent;
                    row.metricId = metric.getElementsByTagName( 'MetricPerTypeId' )[ 0 ].textContent;
                    row.weight = metric.getElementsByTagName( 'Weight' )[ 0 ].textContent;
                    row.qualityLevel = metric.getElementsByTagName( 'QualityLevel' )[ 0 ].textContent;
                    row.mandatory = metric.getElementsByTagName( 'AffectsOverallQuality' )[ 0 ].textContent;
                    if( metric.getElementsByTagName( 'FeaturesAsString' ).length > 0 ) {
                    row.featuresAsString = metric.getElementsByTagName( 'FeaturesAsString' )[ 0 ].textContent;
                    }
                    // Find instances from FeaturesAsDoubleDict or FeaturesAsDict
                    var instances = [];
                    var featuresAsDict = undefined;
                    if( metric.getElementsByTagName( 'FeaturesAsDict' ) && metric.getElementsByTagName( 'FeaturesAsDict' )[0] ) {
                        featuresAsDict = metric.getElementsByTagName( 'FeaturesAsDict' )[0];
                    }
                    if( metric.getElementsByTagName( 'FeaturesAsDoubleDict' ) && metric.getElementsByTagName( 'FeaturesAsDoubleDict' )[0] ) {
                        featuresAsDict = metric.getElementsByTagName( 'FeaturesAsDoubleDict' )[0];
                    }
                    if ( featuresAsDict !== undefined ) {
                        var features = featuresAsDict.getElementsByTagName( 'SerializableFeature' );
                        var itInstances = 0;
                        var itFeatures = 0;
                        for ( itFeatures = 0; itFeatures < features.length; itFeatures++ ) {
                            var instancesElements = features[itFeatures].getElementsByTagName( 'Instances' )[0].getElementsByTagName( 'string' );

                            for ( itInstances = 0; itInstances < instancesElements.length; itInstances++ ) {
                                if( !_contains( instances, instancesElements[itInstances].textContent ) ) {
                                    instances.push( instancesElements[itInstances].textContent );
                                }
                            }
                        }
                    }
...
var _createMetricViewModelObject = function( metric ) {
    ...
    properties.mandatory = _createProp( 'mandatory', metric.mandatory.toLowerCase() === 'true' ? 'True' : 'False', 'STRING', 'Mandatory' );
...
app.factory( 'Arm0RequirementQualityService', () => exports );
