...
function findObjIdCallback( node ) {
    ...
    if( parentNode && parentNode.attributes && parentNode.attributes.revisionid && parentNode.attributes.revisionid.value ) {
...
function setViewerContainer() {
    var reqMainPanel;
    if( appCtxSvc.ctx && appCtxSvc.ctx.occmgmtContext ) {
        reqMainPanel = document.getElementsByClassName( 'aw-requirements-mainPanel' );
    }else{
        reqMainPanel = document.getElementsByClassName( 'requirement' );
    }
    viewerContainer = reqMainPanel.length > 0 ? reqMainPanel[ 0 ] : frameWindow.frameElement;
    if( viewerContainer ) {
