...
const handledKeyCodes = [
    CKEDITOR5.keyCodes.arrowup,
    CKEDITOR5.keyCodes.arrowdown,
    CKEDITOR5.keyCodes.enter,
    CKEDITOR5.keyCodes.tab,
    CKEDITOR5.keyCodes.space,
    CKEDITOR5.keyCodes.esc
];
 ...
 */
export default class MentionUI extends CKEDITOR5.Plugin {
    /**
     * @inheritDoc
     */
    ...
    constructor( editor ) {
         ...
         * @method
         */
        this._requestFeedDebounced = CKEDITOR5.debounce( this._requestFeed, 100 );
    ...
    init() {
         ...
         */
        this._balloon = editor.plugins.get( CKEDITOR5.ContextualBalloon );
        ...
        editor.editing.view.document.on( 'keydown', ( evt, data ) => {
            if ( isHandledKey( data.keyCode ) && this._isUIVisible ) {
                ...
                if ( data.keyCode === CKEDITOR5.keyCodes.arrowdown ) {
                    this._mentionsView.selectNext();
                }

                if ( data.keyCode === CKEDITOR5.keyCodes.arrowup ) {
                    this._mentionsView.selectPrevious();
                }

                if ( data.keyCode === CKEDITOR5.keyCodes.enter || data.keyCode === CKEDITOR5.keyCodes.tab ) {
                    this._mentionsView.executeSelected();
                }

                if ( data.keyCode === CKEDITOR5.keyCodes.esc ) {
        ...
        for ( const mentionDescription of feeds ) {
            ...
            if ( !isValidMentionMarker( marker ) ) {
                 ...
                 */
                throw new CKEDITOR5.CKEditorError(
    ...
    _createMentionView() {
        ...
        this._items = new CKEDITOR5.Collection();
    ...
    _requestFeed( marker, feedText ) {
        ...
        feedResponse
            ...
            .catch( error => {
                 ...
                 */
                console.warn( CKEDITOR5.attachLinkToDocumentation( 'mention-feed-callback-error: Could not obtain mention autocomplete feed.' ) );
            } );
    }
    ...
    _setupTextWatcherForFeed( marker, minimumCharacters ) {
        ...
        const watcher = new CKEDITOR5.TextWatcher( editor.model, createTestCallback( marker, minimumCharacters ) );
    ...
    _renderItem( item, marker ) {
        ...
        if ( !view ) {
            const buttonView = new CKEDITOR5.ButtonView( editor.locale );
    ...
    _getBalloonPanelPositionData( mentionMarker, preferredPosition ) {
        ...
        return {
            target: () => {
                ...
                const rangeRects = CKEDITOR5.Rect.getDomRangeRects( domConverter.viewRangeToDom( viewRange ) );
...
export function createRegExp( marker, minimumCharacters ) {
    ...
    const pattern = `([${ marker }])([${ mentionCharacters }]${ numberOfCharacters })$`;
