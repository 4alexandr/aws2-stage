...
function subscribeEventToEnableTOCCommand(  ) {
    eventToEnableTOCCommand =  eventBus.subscribe( 'ckeditor.clickedInsideBodyText', function( ) {
        var eventData = {
            enableTOC: true
        };
        var bodyTextElement = document.getElementsByClassName( 'aw-requirement-bodytext' )[0];
        var isContentNonEditable = false;
        if(bodyTextElement && (bodyTextElement.getAttribute( 'contenttype' ) === 'READONLY' || bodyTextElement.getAttribute( 'contenteditable' ) === false) ) {
    ...
    for ( var i = 0; i < widgets.length; i++ ) {
        ...
        if( widget.getAttribute( 'revisionid' ) === id ) {
            ...
            editor.editing.view.change( writer => {
                ...
                updateAceSelectiononClickOfHeader( objectUid );
            } );
            openInNewTab = false;
            break;
        }
