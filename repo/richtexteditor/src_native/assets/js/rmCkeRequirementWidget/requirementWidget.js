...
export default class RequirementWidget extends CKEDITOR5.Plugin {
    init() {
        editor = this.editor;
        ...
        */
        editor.model.document.on( 'change', () => {
            ...
            for ( const block of blocks ) {
                RequirementWidgetUtil.disableCmdForReadOnlyObj( block, editor );
            }
        } );
    ...
    _defineSchema() {
        ...
        schema.register( 'requirement', {
            allowWhere: '$block',
            allowAttributes:[ 'class', 'isspecification', 'hastracelink', 'id', 'lmd', 'objecttype', 'itemtype',
                              'parentid', 'parenttype', 'siblingid', 'siblingtype', 'parentitemtype', 'contenteditable', 'revisionid',
                              'top_line', 'style', 'checkedoutby', 'checkedouttime' ],
            isObject: true,
            isBlock: true
        } );
        ...
        schema.register( 'requirementHeader', {
            allowIn: 'requirement',
            allowAttributes:[ 'class', 'contenttype', 'selected', 'contenteditable', 'requirementNamePrefix' ],
            isLimit: true
        } );
        ...
        schema.register( 'requirementBodyText', {
            ...
            allowAttributes:[ 'class', 'contenttype', 'contenteditable', 'isdirty', 'isBasedon', 'isderived', 'masterreqname', 'masterReqUid', 'basedOnMasterReqName', 'basedonmasterreqid' ],
            allowContentOf: '$root',
            isLimit: true
        } );
        schema.register( 'requirementProperty', {
            allowIn: [ 'requirementContent', 'paragraph' ],
            allowAttributes:[ 'class', 'isDirty', 'isdirty', 'isModifible', 'isEditable', 'iseditable', 'propertyType', 'propertytype',
        ...
        schema.register( 'requirementLovProperty', {
            ...
            allowAttributes:[ 'class', 'isDirty', 'isdirty', 'isModifible', 'isEditable', 'iseditable', 'propertyType', 'propertytype',
            'internalName', 'title', 'contenteditable', 'isRequired', 'selectOptions', 'selected' ],
            allowContentOf: '$root',
            isLimit: true
        } );
        ...
        schema.register( 'tracelinkicon', {
            allowIn: 'requirementMarker',
            allowAttributes: [ 'imgSrc', 'class', 'tracelinkCount', 'title' ]
        } );
    ...
    _defineDerivedClassConversion( conversion ) {
        ...
        conversion.for( 'downcast' ).elementToElement( {
            ...
            view: ( modelElement, viewWriter ) => {
                var classes = modelElement.getAttribute( 'class' );
                return viewWriter.createContainerElement( 'div', { class: classes } );
            }
        } );
    }
    _defineTOCDIVConversion( conversion ) {
        conversion.for( 'upcast' ).add( dispatcher => {
            dispatcher.on( 'element:div', ( evt, data, conversionApi ) => {
                const matcher = new CKEDITOR5.Matcher( {
                    name: 'div',
        ...
        conversion.for( 'downcast' ).add( dispatcher => {
            dispatcher.on( 'insert:tocWidget', ( evt, data, conversionApi ) => {
                ...
                var tocWidgetElement = CKEDITOR5.toWidget( viewtocWidget, conversionApi.writer );
    ...
    _defineRequirementConversion( conversion ) {
        ...
        conversion.for( 'editingDowncast' ).elementToElement( {
            ...
            view: ( modelElement, viewWriter ) => {
                ...
                return CKEDITOR5.toWidget( viewRequirement, viewWriter );
            }
        } );
    ...
    _defineMarkerConversion( conversion ) {
        ...
        conversion.for( 'downcast' ).elementToElement( {
            ...
            view: ( modelElement, viewWriter ) => renderToEditing( viewWriter, this.editor, modelElement )
        } );
        conversion.for( 'upcast' ).elementToElement( {
            model: 'addelementicon',
            view: 'addelementicon'
        ...
        conversion.for( 'downcast' ).elementToElement( {
            ...
            view: ( modelElement, viewWriter ) => renderToEditingRemoveIcon( viewWriter, this.editor, modelElement )
        } );
        conversion.for( 'upcast' ).elementToElement( {
            model: 'removeelementicon',
        ...
        conversion.for( 'downcast' ).elementToElement( {
            model: 'checkedout',
            view: ( modelElement, viewWriter ) => renderForCheckout( viewWriter, this.editor, modelElement )
        } );
        conversion.for( 'upcast' ).elementToElement( {
            model: 'checkedout',
            view: 'checkedout'
        ...
        conversion.for( 'downcast' ).elementToElement( {
            model: 'iconImage',
            view: ( modelElement, viewWriter ) => renderForIcon( viewWriter, this.editor, modelElement )
    ...
    _defineTracelinkIconConversion() {
            ...
            dispatcher.on( 'element:tracelinkicon', ( evt, data, conversionApi ) => {
                ...
                if ( viewDiv ) {
                    ...
                    for ( let child of viewDiv.getChildren() ) {
                        ...
                        } else if ( child.name === 'span' && child.childCount > 0 ) {
                            child = child.getChild( 0 );
                        }
                        if ( child.hasClass( 'aw-base-icon' ) ) {
                            srcImg = child.getAttribute( 'src' );
                        }
                    }
                    conversionApi.writer.setAttribute( 'class', clsStrings, tracelinkIcon );
                    conversionApi.writer.setAttribute( 'imgSrc', srcImg, tracelinkIcon );
        ...
        conversion.for( 'downcast' ).add( dispatcher => dispatcher.on( 'insert:tracelinkicon', ( evt, data, conversionApi ) => {
            ...
            if ( tracelinkIcon ) {
                ...
                conversionApi.consumable.consume( tracelinkIcon, 'attribute:imgSrc:tracelinkicon' );
                ...
                classes += ' aw-ckeditor-marker-element';
                ...
                const traceLinkSpan = viewWriter.createContainerElement( 'span' );

                var imgSrc = tracelinkIcon.getAttribute( 'imgSrc' );


                const imgElement = viewWriter.createUIElement( 'img', { class: 'aw-base-icon', src: imgSrc }, function( domDocument ) {
                    const domElement = this.toDomElement( domDocument );
                    domElement.addEventListener( 'click', ( event ) => {
                        var target = event.currentTarget ? event.currentTarget : event.srcElement;
                        var nestedWidget = getNestedEditable( target );
                ...
                viewWriter.insert( viewWriter.createPositionAt( tracelinkIconElement, 0 ), [ traceLinkSpan, tlCountElement ] );
            ...
            .add( dispatcher => dispatcher.on( 'attribute:tracelinkCount', ( evt, data, conversionApi ) => {
                ...
                var srcImg = myModelElement.getAttribute( 'imgSrc' );
                ...
                for ( let child of viewElement.getChildren() ) {
                    ...
                    if ( child.hasClass( 'aw-base-icon' ) ) {
                        conversionApi.writer.setAttribute( 'src', srcImg, child );
                    }
                }
        ...
        conversion.for( 'downcast' ).add( dispatcher => dispatcher.on( 'insert:requirementHeader', ( evt, data, conversionApi ) => {
            ...
            if ( requirementHeader ) {
                ...
                const headerNonEditable = viewWriter.createUIElement( 'span', { class: 'aw-requirement-headerId aw-requirement-headerNonEditable' }, function( domDocument ) {
                    const domElement = this.toDomElement( domDocument );
                    domElement.innerHTML = paragraphNumber;
                    return domElement;
                } );
                ...
                if ( requirementHeader.getChild( 0 ) ) {
                    ...
                    const headerWidgetEditable = CKEDITOR5.toWidgetEditable( headerEditable, conversionApi.writer );
            ...
            var paragraphNumber = myModelElement.getAttribute( 'requirementNamePrefix' );


            var hrEle = viewElement.parent;

            if ( hrEle.childCount > 1 ) {
                viewWriter.remove( hrEle.getChild( 0 ) );

                const headerNonEditable = viewWriter.createUIElement( 'span', { class: 'aw-requirement-headerNonEditable' }, function( domDocument ) {
                    const domElement = this.toDomElement( domDocument );
                    domElement.innerHTML = paragraphNumber;
                    return domElement;
                } );

                viewWriter.insert( viewWriter.createPositionAt( hrEle, 0 ), headerNonEditable );
            }
        } ) );
    }
        ...
        conversion.for( 'downcast' ).add( dispatcher => {
            dispatcher.on( 'insert:requirementBodyText', ( evt, data, conversionApi ) => {
                ...
                if ( requirementBodyText.getAttribute( 'contenttype' ) === 'READONLY' || requirementBodyText.getAttribute( 'contenteditable' ) === 'false' ) {
                    ...
                    bodyTextWidget = CKEDITOR5.toWidget( viewBodyText, conversionApi.writer );
                } else {
                    viewBodyText = viewWriter.createEditableElement( 'div', requirementBodyText.getAttributes() );
                    bodyTextWidget = CKEDITOR5.toWidgetEditable( viewBodyText, conversionApi.writer );
                }
        ...
        conversion.for( 'downcast' ).add( dispatcher => {
            dispatcher.on( 'insert:requirementProperty', ( evt, data, conversionApi ) => {
                ...
                if ( requirementProperty.hasAttribute( 'contenteditable' )
                    ...
                    propertyWidget = CKEDITOR5.toWidgetEditable( viewPropertyEle, conversionApi.writer );
                } else {
                    viewPropertyEle = viewWriter.createContainerElement( 'span', requirementProperty.getAttributes() );
                    propertyWidget = CKEDITOR5.toWidget( viewPropertyEle, conversionApi.writer );
                }
    ...
    const viewTypeIcon = viewWriter.createUIElement( 'img', modelElement.getAttributes(), function( domDocument, modelElement ) {
        ...
        if ( modelEle.parent.name === 'typeicon' ) {
            domElement.addEventListener( 'click', ( event ) => {
                ...
                if ( nestedWidget.id.indexOf( 'RM::NEW::' ) === 0 || editor.isHtmlSpecTemplate === true ) {
                    // Create panel definition for menu
                    var menuList = [];
            ...
            domElement.addEventListener( 'contextmenu', ( event ) => {
                ...
                if ( nestedWidget.id.indexOf( 'RM::NEW::' ) !== 0 ) {
                    var bodyTextElement = nestedWidget.getElementsByClassName( 'aw-requirement-bodytext' );
...
function renderToEditing( viewWriter, editor, modelElement ) {
    const viewMarker = viewWriter.createUIElement( 'addelementicon', { class: 'aw-ckeditor-marker-element aw-ckeditor-linkAction' }, function( domDocument, modelElement ) {
        const domElement = this.toDomElement( domDocument );

        domElement.innerHTML = editor.addIconImgElement;
            ...
            if ( addOptions.length > 1 ) {
                ...
                var timer = setTimeout( function() {
                    if ( !prevent ) {
                        ...
                        var actionList = [ {
                            displayName: addOptions[0],
                            internalName: addOptions[0]
                        },
                        {
                            displayName: addOptions[1],
                            internalName: addOptions[1]
                        } ];
                        var eventData = {
                            sourceObject: {
                                uid: element.id
...
function renderToEditingRemoveIcon( viewWriter, editor, modelElement ) {
    ...
    return viewWriter.createUIElement( 'removeelementicon', { class: 'aw-ckeditor-marker-element aw-ckeditor-linkAction' }, function( domDocument, modelElement ) {
        const domElement = this.toDomElement( domDocument );
        domElement.innerHTML = editor.removeIconImgElement;
        domElement.addEventListener( 'click', ( event ) => {
...
function getTypeMenu( element, event, editor, viewTypeIcon, viewWriter, domElement ) {
    ...
    var objectTypesWithIcon = RequirementWidgetUtil.getAllowedTypesFromGlobalTypeMap( editor, parentItemType )
            && RequirementWidgetUtil.getAllowedTypesFromGlobalTypeMap( editor, parentItemType ).allowedTypesWithIcon;
