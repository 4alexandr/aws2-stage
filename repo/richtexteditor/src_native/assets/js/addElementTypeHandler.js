 ...
 */
export let _getDisplayableTypes = function( selectedObject ) {
...
export let handelGetDisplayableTypesEvent = function( eventData ) {
    exports._getDisplayableTypes( eventData.selected ).then( function( allowedTypesInfo ) {
        ...
        eventData.editor.config.objectTypeGlobalMap.push( {
            ...
            allowedTypesWithIcon: allowedTypesInfo.objectTypesWithIcon,
...
export let processTemplatesBeforeEditing = function( data ) {
    var deferred = AwPromiseService.instance.defer();
    var mselection = appCtxSvc.getCtx( 'mselected' )[ 0 ];
    var type = mselection.type;
    var input = {
    ...
    var inputExportToApp3 = {
        input: input
    };
    soaSvc.post( 'Internal-ActiveWorkspaceReqMgmt-2017-06-ImportExport', 'getDisplayableTypes', inputExportToApp3 )
        .then( function( response ) {
            return response;
        }, function( data ) {
            console.log( 'getDisplayableTypes Whoops ' + data );
        } ).then(
            function( response2 ) {
                if( response2 ) {
                    var templateMap = [];
                    var displayableTypes = [];
                    for( var ii = 0; ii < response2.specElementTypes.length; ii++ ) {
                        var realTypeName = response2.specElementTypes[ ii ].realTypeName;
                        var typeTemplate = '';
                        for( var jj = 0; jj < response2.rootTypes.length; jj++ ) {
                            if( realTypeName === response2.rootTypes[ jj ].realTypeName ) {
                                typeTemplate = response2.rootTypes[ jj ].displayTypeName;
                                break;
                            }
                        }
                        templateMap.push( {
                    ...
                    if( templateMap.length > 0 ) {
                        ...
                        if( templateObjectBeingProcessed !== null ) {
                            ckeditorOperations.setCKEditorSafeTemplate( data.editorProps.id, templateObjectBeingProcessed, templateMap, appCtxSvc.ctx);
                        }
                    }
                }
            } );
...
export default exports = {
    ...
    processTemplatesBeforeEditing
};
/**
 * Service Utility
 *
 * @memberof NgServices
 * @member addElementTypeHandler
 */
app.factory( 'addElementTypeHandler', () => exports );
