 ...
 */
export let showWidePanelEditorPopup = function( popupData, calcHeight ) {
        ...
        popupService.show( popupData ).then( popupRef => {
            ...
            var sideNavEventSub = eventBus.subscribe( 'awsidenav.openClose', function( eventData ) {
                if ( eventData && eventData.id === 'aw_toolsAndInfo' ) {
                    exports.checkAndCloseWidePanel();
                }
            } );
 ...
 *
 */
export let unRegisterCtxclosePopup = function() {
    _.defer( function() {
   ...
   if( !eventInsertOle ) {
       eventInsertOle = eventBus.subscribe( 'requirementDocumentation.InsertOLEInCKEditor', function(
           eventData ) {
            ...
            _data.form = eventData.form;

            var fileName = eventData.file.name;
            _data.fileExtensions = fileName.split( '.' ).pop();

           eventBus.publish( 'Arm0SingleRequirementWidePanelEditor.getDatasetTypesWithDefaultRelation' );
       }, 'Arm0RequirementDocumentation' );
   }
   ...
   if( !eventInsertImage ) {
       eventInsertImage = eventBus.subscribe( 'requirementDocumentation.InsertImageInCKEditor', function(
           ...
           if( reqUtils.stringEndsWith( fileName.toUpperCase(), '.gif'.toUpperCase() ) || reqUtils.stringEndsWith( fileName.toUpperCase(), '.png'.toUpperCase() ) ||
               ...
               _data.datasetInfo = datasetInfo;

               eventBus.publish( 'Arm0SingleRequirementWidePanelEditor.InsertObjInCKEditorWidePanel' );
           } else {
...
/**
 * Initialize CkeditrOperations based on ckeditor version,
 * CkeditorUtils for ckeditor4 and Ckeditor5Utils for ckeditor5
 */
var initCkeditorOperation = function(  ) {
...
var _preprocessContentsAndSetToEditor = function( data ) {
    ...
    if ( divElement ) {
        ...
        if ( bodyTextEle && bodyTextEle.length > 0 && bodyTextEle[0].innerHTML && bodyTextEle[0].innerHTML.trim() === '' ) {
    ...
    _setCKEditorContent( data.editorProps.id, contentDiv.outerHTML );
};
...
export let getInputValues = function( data ) {
    ...
    if ( appCtxService.ctx.Arm0Requirements.Editor === 'CKEDITOR_4' ) {
        if ( markupText ) {
...
/**
 * This is Custom Wide Panle for Requirement Body text editing.
 *
 * @memberof NgServices
 * @member Arm0SingleRequirementWidePanelEditor
 */
app.factory( 'Arm0SingleRequirementWidePanelEditor', () => exports );
