...
var _documentationUnloadedEventListener = null;
...
export let initContent = function( data, ctx ) {
    ...
    if ( !_documentationUnloadedEventListener ) {
        _documentationUnloadedEventListener = eventBus.subscribe( 'Arm0Documentation.contentUnloaded',
            function() {
    ...
    if ( data.content ) {
        ...
        if ( transientFileReadTickets ) {
            ...
            if ( !data.editMode ) {
                ...
                if ( headerDivElement && _isHTMLObjTmplRevision( ctx ) ) {
     ...
     */
    function handleDocumentationUnloaded() {
 ...
 */
export let getSpecificationSegmentInput = function() {
    var inputCtxt = reqACEUtils.getInputContext();
    var selectedObjs = appCtxService.getCtx( 'selected' );
    ...
    if ( setContentInput !== null && setContentInput.length > 0 ) {
        for ( var i = 0; i < setContentInput.length; i++ ) {
            ...
            if ( uid !== null ) {
                ...
                var specSegmentContent = { uid: obj.uid, type: obj.type };
 ...
 *
 * @memberof NgServices
 * @member Arm0RequirementDocumentation
 */
app.factory( 'Arm0RequirementDocumentation', () => exports );
