 ...
 *
 * @module js/Vm1RemoveLocationCommandHandler
 */
import app from 'app';
...
export let execute = function( vmo ) {
    if( vmo && vmo.uid ) {
        _selectedLocation = vmo;
        Vm1CreateVendorSvc.removeLocationFromData( vmo );
        eventBus.publish( 'Vm1RemoveLocationCommand.remove' );
    }
};

/**
 * Remove Location from create vendor panel .Called when clicked on the remove cell.
 * @param {data} data - The qualified data of the viewModel
 */
export let removeVendorLocation = function( data ) {
    var removeLocationUid = [];
    removeLocationUid.push( _selectedLocation.uid );
    var memberModelObjects = data.dataProviders.getVendorLocationsList.viewModelCollection.loadedVMObjects;

    var modelObjects = $.grep( memberModelObjects, function( eachObject ) {
        return $.inArray( eachObject.uid, removeLocationUid ) === -1;
    } );
    data.dataProviders.getVendorLocationsList.update( modelObjects );
};
 ...
 */
app.factory( 'Vm1RemoveLocationCommandHandler', () => exports );
