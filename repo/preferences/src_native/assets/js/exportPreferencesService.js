...
export let populateExportPreferencesPanel = function( data ) {
    ...
    data.defaultFileName = 'Preferences_' + month + t.getDate() + t.getFullYear() + '.xml';
    data.fnd0FileName.dbValue = data.defaultFileName;
    data.fnd0FileName.uiValue = data.defaultFileName;

    // Build location list
    data.locationList = prefService.getLocations( 'export' );

    // Build the product area list
    data.productAreaList = prefService.getProductAreaListToExport();
};
 ...
 *
 * @memberof NgServices
 * @member exportPreferencesService
 *
 */
app.factory( 'exportPreferencesService', () => exports );
