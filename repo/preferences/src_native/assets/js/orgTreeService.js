...
export let preserveSelection = function( ctx, data ) {
    if( ctx.selectedTreeNode ) {
        ...
        // clear search string if it still exists
...
export let treeNodeSelected = function( data, ctx, currentNode ) {
    ...
    if( prefEditHandler !== null ) {
        ...
        prefEditHandler.leaveConfirmation().then( function() {
            if( ctx.tcadmconsole.preferences.editInProgress ) {
                ctx.tcadmconsole.preferences.editInProgress = false;
            }
            handleTreeNodeSelection( data, ctx, currentNode );
        } );
    } else {
        handleTreeNodeSelection( data, ctx, currentNode );
    }
...
var handleTreeNodeSelection = function( data, ctx, currentNode ) {
    if( currentNode && ctx.tcadmconsole.preferences.deselectingPreference !== true ) {
        //if a node has been selected and it's NOT a deselection, reset the primary workArea and get preferences
        ctx.parents = [];
        ...
        ctx.tcadmconsole.preferences.deselectingPreference = true;
        ...
        ctx.tcadmconsole.preferences.deselectingPreference = false;
    }
};
 ...
 * @param {Object} ctx context
 */
var _buildSiteLevelTreeStructure = function( deferred, treeLoadInput, ctx ) {
    var children1 = [];
    ...
    //var vmNode = _createViewModelTreeNode( siteObject, 1, 0, ctx.treeLoadInput.parentNode.displayName, ctx.treeLoadInput.parentNode.id, '', '' );
    ...
    ctx.tcadmconsole.preferences.orgTreeData = {};
    ctx.tcadmconsole.preferences.orgTreeData[ children1[ 0 ].type ] = {
        children: [],
        hier: {},
        fullExpansion: false,
        node: children1[ 0 ]
...
var _buildGroupsAndSubgroups = function( deferred, treeLoadInput, ctx, expandingNode ) {
    ...
    if( !_.isUndefined( children ) && children.length > 0 && ( ctx.tcadmconsole.preferences.filteringOrgTree === true || //
        ...
        var rootPathNodes = [];
        ctx.treeLoadInput.pageSize = ctx.tcadmconsole.preferences.orgTreeData[ treeLoadInput.parentNode.hierarchy ].children.length;
        ...
        ctx.tcadmconsole.preferences.orgTreeData[ treeLoadInput.parentNode.hierarchy ].children.forEach( function( child ) {
        ...
        var treeLoadResult = awTableService.buildTreeLoadResult( ctx.treeLoadInput, ctx.tcadmconsole.preferences.orgTreeData[ treeLoadInput.parentNode.hierarchy ].children, false, true, true, null );
        ...
        return soaService.postUnchecked( 'Internal-Core-2007-01-DataManagement', 'getOrganizationInformation', //
            ...
            .then( function( response ) {
                ...
                ctx.tcadmconsole.preferences.orgTreeData[ treeLoadInput.parentNode.hierarchy ] = _getInitialObject( treeLoadInput.parentNode );
                ...
                for( var i = 0; i < children.length; i++ ) {
                    ...
                    if( ctx.tcadmconsole.preferences.isUserGroupAdmin === true ) {
                    ...
                    if( addNode === true ) {
                        ...
                        if( children[ i ].parent.className === 'unknownClass' ) {
                            ...
                            data = _addGroupToHierarchy( ctx.tcadmconsole.preferences.orgTreeData.Site, children[ i ].group.props.object_string.dbValues[ 0 ], //
                                children[ i ], index, groupsToAdd );
                            vmNode = data.vmNode;
                            parent = data.parent;

                            // add node to orgTreeData.Site
                            ctx.tcadmconsole.preferences.orgTreeData.Site.children.push( vmNode );
                            ctx.tcadmconsole.preferences.orgTreeData.Site.hier[ vmNode.hierarchy ] = parent;
                        } else {
                            // this is a subgroup
                            var subgroupName = children[ i ].group.props.object_string.dbValues[ 0 ];
                            var groups = subgroupName.split( '.' );
                            var ndx = groups.length - 1;
                            var parentNode = ctx.tcadmconsole.preferences.orgTreeData.Site;
                            var id = 'Site';
                ...
                ctx.tcadmconsole.preferences.orgTreeData.Site.fullExpansion = true;

                // alphabetize children by displayName
                ctx.tcadmconsole.preferences.orgTreeData.Site.children = _.sortBy( ctx.tcadmconsole.preferences.orgTreeData.Site.children, [ 'displayName' ] );
                ctx.tcadmconsole.preferences.orgTreeData.Site.children.forEach( function( element, index ) {
                    element.childNdx = index;
                    element.isLeaf = false;
                } );

                var rootPathNodes = [];
                ctx.treeLoadInput.pageSize = ctx.tcadmconsole.preferences.orgTreeData.Site.children.length;
                var treeLoadResult = awTableService.buildTreeLoadResult(
                    ctx.treeLoadInput, ctx.tcadmconsole.preferences.orgTreeData.Site.children, false, true, true, null );
            ...
            .then( function( response ) {
                ...
                if( response.groupElementMap ) {
                    ...
                    for( var i = 0; i < groupMap.length; i++ ) {
                        ...
                        if( treeLoadInput.parentNode.type === 'Group' ) {
                            if( groupInMap.group.props.object_string.dbValues[ 0 ] === treeLoadInput.parentNode.fullName && ( ctx.tcadmconsole.preferences.isUserGroupAdmin === true && //
                                    _.includes( groupInMap.group.props.object_string.dbValues[ 0 ], cdm.getUserSession().props.group.uiValues[ 0 ] ) || //
                                    ctx.tcadmconsole.preferences.isUserGroupAdmin === false ) ) {
                ...
                for( var j = 0; j < children.length; j++ ) {
                    ...
                    if( !_.some( expandingNode.children, [ 'id', id ] ) ) {
                        var vmNode = _createViewModelTreeNode( children[ j ], ctx.treeLoadInput.parentNode.levelNdx + 1, j, ctx.treeLoadInput.parentNode.displayName, //
                ...
                ctx.tcadmconsole.preferences.orgTreeData = _setParentNodeInHierarchy( treeLoadInput.parentNode, ctx.tcadmconsole.preferences.orgTreeData, expandingNode );
...
function _buildTreeTableStructure( deferred, treeLoadInput, ctx ) {
    ...
    if( treeLoadInput.parentNode.levelNdx === -1 ) {
        _buildSiteLevelTreeStructure( deferred, treeLoadInput, ctx );
    } else if( treeLoadInput.parentNode.levelNdx === 0 ) {
        // Get the parent node from the context if it is available
        expandingNode = _getParentNodeInHierarchy( treeLoadInput.parentNode, ctx.tcadmconsole.preferences.orgTreeData );
 ...
 *
 * @memberof NgServices
 * @member orgTreeService
 *
 * @param {*} soaService soa service
 * @param {*} $q functions
 * @param {*} adminPreferenceUserUtil admin user utils service
 ...
 * @param {*} editHandlerService edit handler service
 *
 *@return {*} exports
 */
app.factory( 'orgTreeService', () => exports );
