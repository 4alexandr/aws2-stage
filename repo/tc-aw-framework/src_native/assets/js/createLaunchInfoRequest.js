 ...
 */
export let launchProduct = function( openInHost, isTohostInVis, productLaunchInfo ) {
    var idInfos = [];
    ...
    if( !_.isEmpty( idInfos ) ) {
        _createLaunchInfo( idInfos, openInHost ).catch( function( err ) {
 ...
 */
export let launchObject = function( openInHost, isTohostInVis, selectedObjects ) {
    var idInfos = [];
    ...
    if( !_.isEmpty( idInfos ) ) {
        _createLaunchInfo( idInfos, openInHost ).then( function( result ) {
            if( _debug_logLaunchActivity ) {
 ...
 */
function _createLaunchInfo( idInfos, openInHost ) {
    if( _debug_logLaunchActivity ) {
    ...
    return frameAdapterSvc.createLaunchInfo( idInfos, serverInfo ).then( function( response ) {
        ...
        return AwPromiseService.instance.reject( 'Could not generate VVI file that can be downloaded.' );
    } );
}
