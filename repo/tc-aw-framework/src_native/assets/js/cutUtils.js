...
export let getDeleteObjectsPartialErrors = function (response){
    ...
    {
        let difference = appCtxService.ctx.mselected.filter(x => !response.deleted.includes(x.uid));
        return difference.length;
    }

    return appCtxService.ctx.mselected.length;
};
...
let inputStructForRegularObjs = function( relationInfoObj ) {
    ...
    if( relationInfoObj ) {
        ...
        let unlinkAlwaysInput = containerInput !== '';
        deleteInput = {
