...
export let getVisiblePages = function( xrtResponseData ) {
    ...
    var xrtRet = _.filter( tempPages, function( pg ) {
        return pg.visible;
    } );
 ...
 */
var _prepareSOAInput = function( type, targetPage, vmo, objectType ) {
    var clientContexts = {};
    if( appCtxSvc.getCtx( 'locationContext' ) ) {
        clientContexts = {
    ...
    var xrtCtxt = JSON.stringify( appCtxSvc.getCtx( xrtContextString ) );
    if( xrtCtxt && _.isString( xrtCtxt ) ) {
        clientContexts[ xrtContextString ] = xrtCtxt;
    }

    // Append operation specific contexts
    if( type === 'SAVEAS' || type === 'REVISE' ) {
        var saveAsReviseCtxt = appCtxSvc.getCtx( 'SaveAsReviseWorkSpace' );
        if( saveAsReviseCtxt ) {
            Object.keys( saveAsReviseCtxt ).forEach( function( key ) {
                var val = saveAsReviseCtxt[ key ];
                if( _.isString( val ) ) {
 ...
 * @return the promise.
 */
export let getDeclStyleSheet = function( type, targetPage, vmo, objectType ) {
    var inputData = _prepareSOAInput( type, targetPage, vmo, objectType );
    if( !inputData ) {
        var deferred = AwPromiseService.instance.defer();
        return deferred.promise;
    }
    ...
    if( visibleContributedSubLocations ) {
        ...
        contributedTabs = visibleContributedSubLocations.map( function( subLoc, index ) {
            ...
            if( typeof label === 'string' ) {
                ...
                loadPromises.push( localeSvc.getLocalizedText( app.getBaseUrlPath() + label.source, label.key )
 ...
 * @memberof NgServices
 */
app.factory( 'xrtParserService', () => exports );
