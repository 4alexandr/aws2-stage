 ...
 */
export let getShowConfiguredRev = function() {
    var showConfiguredRev = 'false';
    var addObject = appCtxService.ctx.addObject;
 ...
 */
export let evaluateObjsBasedOnConfiguredRevRule = function( objectsToEvaluateForExactRelation ) {
    let evaluatedObjsToRelate = [];
    if( appCtxService && appCtxService.ctx && appCtxService.ctx.addObject ) {
        let addObjectContext = appCtxService.ctx.addObject;
        if( addObjectContext ) {
...
/**
 *
 * @member awConfiguredRevService
 * @memberof NgServices
 */
app.factory( 'awConfiguredRevService', () => exports );
