...
function _useHostConfiguration_2014_07() { //eslint-disable-line complexity
    ...
    return hostCoreSvc2.createRequestHostConfigProxy().callHostMethodAsync().then( function( hostConfig ) { //eslint-disable-line complexity
        ...
        /**
         * Set the UiOption values from the configuration fields.
         */
        var settings = hostConfig.getSettings();
...
function _onInitialization() {
    ...
    if( promise ) {
        return promise.then( function( uiOptions ) {
            ...
            /**
             * Check if we need to load any extra ('host' specific) module(s).
             */
            if( _uiOptions[ hostConfigKeys.HOST_TYPE ] === hostConfigValues.HOST_TYPE_NX ) {
                declUtils.loadDependentModule( 'js/hostedNxUtils', AwPromiseService.instance, AwInjectorService.instance )
                    .then( function( nxHosted ) {
                        nxHosted.initializeNX();
                    } );
            }
 ...
 * Register service.
 *
 * @member hostConfigService
 * @memberof NgServices
 */
app.factory( 'hostConfigService', () => exports );
