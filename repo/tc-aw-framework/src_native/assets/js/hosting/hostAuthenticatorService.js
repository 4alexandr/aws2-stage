...
export default class HostAuthenticatorService extends AwBaseService {
    constructor() {
        super();
    ...
    clearTimeoutFunc( reasonMsg ) {
        if( this._hostingStartupTimeoutHdlr ) {
            if( this._debug_logHandShakeActivity ) {
                hostSupportSvc.log( 'hostAuthenticatorService: ' +
                    'Clearing startup timeout function - clearTimeout: ' + reasonMsg );
            }
     ...
     *
     * @param {String} discriminator - ID used to relate to the result of this specific call.
     *
    ...
    static getSecureToken( discriminator ) {
        ...
        return soaSvc.postUnchecked( 'Internal-Core-2014-11-Session', 'getSecurityToken', input )
            .then( ( responseData ) => {
                //var userName = answer['userName'];
                var secureToken = responseData.out;

                if( secureToken && contextSvc ) {
                    var role = contextSvc.getUserRole();
                    var group = contextSvc.getUserGroup();
                    var userName = contextSvc.getUserName();
                    var userLocale = contextSvc.getUserLocale();
                    ...
                    $.get( `${browserUtils.getBaseURL()}getSessionVars` ).done( function() {
                        hostSupportSvc.setSoaSupportEnabled( true );
                        return sessionSvc.signIn( userName, secureToken, group, role, userLocale, discriminator, true ).then(
                                () => {
                                    hostSupportSvc.setSoaSupportEnabled( false );
                                    return dms.getTCSessionInfo().then( () => {
                                        hostSupportSvc.setSoaSupportEnabled( true );
                                    } );
                                } )
                            .then( () => {
    ...
    /**
     * @returns {Promise} Resolved with TRUE if the User session is active.
     */
    static getIsUserSessionActive() {
        if( !hostSessionSvc.isHostSessionAccessible() ) {
            return AwPromiseService.instance.resolve( false );
        }

        return hostSessionSvc.getHostSessionUserInfo().then( ( userName ) => {
    ...
    canIProcess() {
        ...
        this._configuredDeferral.promise.then(
            () => { // success
                ...
                HostAuthenticatorService.getIsUserSessionActive().then( ( isUserSessionActive ) => {
                    ...
                    if( !isUserSessionActive ) {
                         ...
                         */
                        var subDef = eventBus.subscribe( 'session.updated', () => {
                            if( this._debug_logHandShakeActivity ) {
                             ...
                             */
                            var startSvc = hostInteropSvc.findClientService2( hostServices.HS_CS_STARTUP_NOTIFICATION_SVC,
            ...
            () => { //failure
                if( this._debug_logHandShakeActivity ) {
                    hostSupportSvc.log( 'hostAuthenticatorService: ' +
                        'Startup defer ERROR handler.  Only resolved so should never get here.' );
                }
        ...
        this._hostingStartupTimeoutHdlr = setTimeout(
            () => {
                if( this._debug_logHandShakeActivity ) {
                    hostSupportSvc.log( 'hostAuthenticatorService: ' +
                        'Timeout function fires in hosting Auth!!' );
                }
     ...
     * @memberof hostAuthenticatorService
     *
     * @return {Promise} Promise resolved with tree/false result.
     */
    checkIfSessionAuthenticated() {
        ...
        return dms.getTCSessionInfo().then( ( soaOkResp ) => {
    ...
    postAuthInitialization() {
        ...
        if( !this._tcSessionInfoResponse || !window ) {
            ...
            if( !hostConfigSvc.isSet() ) {
                eventBus.subscribe( 'hosting.configured', () => {
                     ...
                     */
                    var discriminator = hostConfigSvc.getOption( hostConfigKeys.HOST_DISCRIMINATOR );

                    if( discriminator ) {
                        HostAuthenticatorService.getSecureToken( discriminator );
                    }
                } );
            } else {
                var discriminator = hostConfigSvc.getOption( hostConfigKeys.HOST_DISCRIMINATOR );
         ...
         */
        var startupNotificationSvc = hostInteropSvc.findClientService2(
    ...
    signOut() {
         ...
         */
        return AwPromiseService.instance.resolve(); // tell caller we are done.
    }
}
 ...
 *
 * @member hostAuthenticatorService
 * @memberof NgServices
 */
app.factory( 'hostAuthenticatorService', () => HostAuthenticatorService.instance );
