...
import ignore from 'js/sessionContext.service';
...
/**
 * string with the current Tc session discriminator - trigger initialization
 *
 * Not sure why this is marked as not read, it gets read on the signIn flow, and reset on SignOut.
 */
var _currentDiscriminator = 'AWC' + _.random( 1, 32767 );
...
function sessionInfoSuccessHandling( soaOkResp ) {
    ...
    logger.trace( 'UPW: sessionInfoSuccess - fire session.updated event with discriminator.' );
    var eventData = {
        sessionDiscriminator: _currentDiscriminator
    };
    eventBus.publish( 'session.updated', eventData );
}
...
function setLanguageDataOnScope( $scope, updateLocalStorage ) {
    $scope.$evalAsync( function() {
        ...
        localeSvc.getTextPromise( 'LoginLocale' ).then( function( textBundle ) {
            ...
            var _installedLocales = localeSvc.getInstalledLocales();
            var _installedLanguages = [];

            for( var localeKey in _localTextBundle ) {
 ...
 * @param {Object} $scope - scope
 */
export let setScope = function( $scope ) {
    ...
    $scope.setPopupHeight = function( event ) {
        var maxHeight = $( window ).innerHeight() - event.pageY - 15;
        $scope.popupStyle = {
    ...
    $scope.hideLinkPopUp = function( event ) {
        ...
        // var parent = $( event.target ).closest( '.aw-layout-popup.aw-layout-popupOverlay' )[0];
    ...
    var progressStartListener = eventBus.subscribe( 'progress.start', function() {
        $scope.$evalAsync( function() {
            $scope.networkActive = true;
        } );
    } );

    var progressStopListener = eventBus.subscribe( 'progress.end', function() {
        $scope.$evalAsync( function() {
    ...
    $scope.login = function() {
        ...
        sessionService.signIn( $scope.userName, $scope.password, $scope.group, $scope.role,
            localeSvc.getLocale(), _currentDiscriminator ).then( function( responseData ) {
...
/**
 * The UserPW authentication service.
 *
 * @class tcUserPwAuthenticator
 * @memberof NgServices
 */
app.factory( 'tcUserPwAuthenticator', () => exports );
