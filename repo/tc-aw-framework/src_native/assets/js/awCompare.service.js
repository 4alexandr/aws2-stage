...
export let getTypesAndCallSoa = function( serviceName, operationName, soaInput, modelObjects, columnProvider ) {
    ...
    return soaService.postUnchecked( serviceName, operationName, soaInput ).then( function( result ) {
        result.types = types;
        _.forEach( result.columnConfigurations[ 0 ].columnConfigurations[ 0 ].columns, function( col ) {
...
export const arrangeColumns = function( dataProvider, columnProvider, gridId, declViewModel, ctx, eventData, subPanelContext ) {
    ...
    let promise = declUtils.loadDependentModule( action.deps ).then(
...
app.factory( 'awCompareService', () => exports );
