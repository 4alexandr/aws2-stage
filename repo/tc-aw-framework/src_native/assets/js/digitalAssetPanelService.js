...
function loadObjects( uidsToLoad, uidCount, declModel ) {
    return dmSvc.loadObjects( uidsToLoad ).then( function() {
        ...
        return dmSvc.getProperties( uidsToLoad, [ 'object_string', 'item_revision_id', 'revision_list' ] ).then( function() {
            var moToDisplay = [];
            _.forEach( uidsToLoad, function( mo ) {
                // Check for dups
                for( var i = 0; i < moToDisplay.length; i++ ) {
                    if( moToDisplay[ i ].uid === mo ) {
                        return;
                    }
                }
                moToDisplay.push( {
        ...
        return expandBOM( response.output[ 0 ].bomLine ).then( function( response ) {
            ...
            if( response.output && response.output[ 0 ] ) {
                ...
                for( var i = 0; i < children.length; i++ ) {
                    var childUid = children[ i ].itemRevOfBOMLine.uid;
                    ...
                    var found = false;
                    for( var j = 0; j < uidsToLoad.length; j++ ) {
                        if( uidsToLoad[ j ] === childUid ) {
                            found = true;
                            break;
                        }
                    }
...
export let createInUseList = function( declModel, selection ) {
    ...
    // If this is an Artwork Design, set that in the panel title
    if( cmm.isInstanceOf( 'RetailProductRevision', selection.modelType ) ||
        cmm.isInstanceOf( 'Ret02DCADRevision', selection.modelType ) ) {
        // This is a RFA object, adjust the title accordingly
        declModel.dataProviders.inUseList.assetPanelTitle = declModel.i18n.imagePanelTitle;
        declModel.inUseSectionTitle = declModel.i18n.imageInUseSectionTitle;
    } else if( !cmm.isInstanceOf( 'Pka0ArtwkDesignRevision', selection.modelType ) ) {
        declModel.inUseSectionTitle = declModel.inUseSectionTitle.replace( '"{0}"', declModel.i18n.adobeFile );
        return AwPromiseService.instance.resolve( null );
    }

    if( appCtxSvc.ctx.HostedFileNameContext && appCtxSvc.ctx.HostedFileNameContext.filename ) {
        declModel.inUseSectionTitle = declModel.inUseSectionTitle.replace( '{0}', appCtxSvc.ctx.HostedFileNameContext.filename );
    } else {
        declModel.inUseSectionTitle = declModel.inUseSectionTitle.replace( '"{0}"', declModel.i18n.adobeFile );
    }
    ...
    if( cmm.isInstanceOf( 'RetailProductRevision', selection.modelType ) ) {
        return getAdobeRenderObjectUID( selection ).then( ( bomHolder ) => {
            if( bomHolder ) {
...
/**
 * DigitalAssetPanelService service utility
 *
 * @memberof NgServices
 * @member digitalAssetPanelService
 *
 * @param {object} $q - promises lib
 * @param {object} dmSvc - data management lib
 ...
 * @param {object} fireCommandProxy - the fire command hosting proxy for firing commands on the host
 *
 * @returns {object} returns this object
 */
app.factory( 'digitalAssetPanelService', () => exports );
