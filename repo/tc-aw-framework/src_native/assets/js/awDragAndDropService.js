...
const getXrtTableUid = ( dragAndDropParams ) => {
    if( dragAndDropParams.dataProvider.validSourceTypes ) {
        return dragAndDropParams.declViewModel.vmo.uid + '.' + dragAndDropParams.dataProvider.name;
    }
    return null;
};

const getValidDropTypes = ( targetVmo, isXrtTable, { dataProvider } ) => {
    if( isXrtTable ) {
        return getValidDropTypesFromDataprovider( dataProvider );
    }
 ...
 */
var _deselectAllAndPasteSourceFiles = function( targetElement, targetVMO, sourceFiles, dragAndDropParams, xrtTableUid ) {
    if( sourceFiles && sourceFiles.length > 0 ) {
        ...
        if( !validSourceTypes || _.isEmpty( validSourceTypes ) ) {
            if( cmm.isInstanceOf( 'Awp0XRTObjectSetRow', targetVMO.modelType ) ) {
                ...
                validSourceTypes = getValidDropTypes( targetVMO, xrtTableUid, dragAndDropParams );
                _pasteSourceFiles( targetVMO, sourceFiles, dragAndDropParams, validSourceTypes );
            }
        } else {
            _pasteSourceFiles( targetVMO, sourceFiles, dragAndDropParams, validSourceTypes );
 ...
 */
var _deselectAllAndPasteSourceObjects = function( targetElement, targetVMO, sourceObjects, callbackAPIs, xrtTableUid, dragAndDropParams ) {
 ...
 * @param {Object} dragAndDropParams - Framework generated drag and drop parameters.
 */
const _dropFiles = function( targetElement, sourceFiles, dragAndDropParams ) {
    ...
    if( isListOrTableContainer( dragAndDropParams ) ) {
        xrtTableUid = getXrtTableUid( dragAndDropParams );
        targetVMOs = [ dragAndDropParams.declViewModel.vmo ];
    }
    _deselectAllAndPasteSourceFiles( targetElement, targetVMOs[ 0 ], sourceFiles, dragAndDropParams, xrtTableUid );
 ...
 * @param {Object} dragAndDropParams - Framework generated drag and drop parameters.
 */
const _dropModelObjects = function( targetElement, sourceUIDs, dragAndDropParams ) {
    let targetVMOs = dragAndDropParams.targetObjects;
    let xrtTableUid = null;
    ...
    if( targetVMOs && targetVMOs.length !== 0 ) {
        _dropModelObjectsInternal( targetElement, sourceUIDs, dragAndDropParams.callbackAPIs, targetVMOs, xrtTableUid, dragAndDropParams );
    }
};
 ...
 * @param {Object} dragAndDropParams - System generated drag and drop parameters.
 */
var _dropModelObjectsInternal = function( targetElement, sourceUIDs, callbackAPIs, targetVMOs, xrtTableUid, dragAndDropParams ) {
    var sourceObjects = [];
 ...
 */
const processDragOver = function( dragAndDropParams ) {
    ...
    _processDragOverInternal( dragAndDropParams );
};
 ...
 * @param {Object} dragAndDropParams - Framework generated drag and drop parameters.
 */
var _setValidDropTypesOnTarget = function( dragAndDropParams ) {
    ...
    let declViewModel = dragAndDropParams.declViewModel;
    ...
    if( targetVMO ) {
        ...
        if( checkValidTypes( declViewModel.vmo ) ) {
            return;
        }
        let targetUid = null;
        if( dataProvider && dataProvider.validSourceTypes ) {
            sourceType2RelationMap = getValidDropTypesFromDataprovider( dataProvider );
            targetUid = getXrtTableUid( dragAndDropParams );
        } else {
            sourceType2RelationMap = pasteService.getObjectValidSourceTypes( dragAndDropParams.declViewModel.vmo );
            targetUid = dragAndDropParams.declViewModel.vmo.uid;
        }
        if( targetUid && sourceType2RelationMap ) {
            setValidTypes( sourceType2RelationMap, targetUid );
        }
    }
};
...
const createPasteInput = ( pasteRelation2SourceObjectsMap, targetVMO ) => {
    let pasteInput = [];
...
var _pasteFiles = function( pasteInput ) {
    ...
    if( !pasteFileHandler && _pasteConfig.defaultPasteFileHandler ) {
        pasteFileHandler = declUtils.getDependentModule( _pasteConfig.defaultPasteFileHandler.dep );
        if( !pasteFileHandler ) {
...
const executeDrop = ( targetVMO, callbackAPIs, pasteRelation2SourceObjectsMap ) => {
    _scheduleSelectTarget( targetVMO, callbackAPIs );
    /**
    ...
    if( keys.length > 0 ) {
        let pasteInput = createPasteInput( pasteRelation2SourceObjectsMap, targetVMO );
...
const _pasteSourceObjects = function( targetVMO, sourceObjects, callbackAPIs, validSourceTypes, dragAndDropParams ) {
    ...
    if( cmm.isInstanceOf( 'Awp0XRTObjectSetRow', targetVMO.modelType ) ) {
        adapterSvc.getAdaptedObjects( [ targetVMO ] ).then( function( adaptedObjs ) {
            executeDrop( adaptedObjs[ 0 ], callbackAPIs, pasteRelation2SourceObjectsMap );
        } );
    } else {
        executeDrop( targetVMO, callbackAPIs, pasteRelation2SourceObjectsMap );
    }
};
 ...
 */
const isValidObjectToDrop = function( dragAndDropParams, xrtTableUid ) { // eslint-disable-line complexity
...
const isListOrTableContainer = ( dragAndDropParams ) => {
    return ( !dragAndDropParams.targetObjects || !dragAndDropParams.targetObjects.length ) && dragAndDropParams.dataProvider && dragAndDropParams.declViewModel.vmo;
};
/**
 ...
 * @param {Object} dragAndDropParams - Framework generated drag and drop parameters.
 */
const _processDragOverInternal = function( dragAndDropParams ) {
    let event = dragAndDropParams.event;
    let xrtTableUid = null;
    ...
    if( isListOrTableContainer( dragAndDropParams ) ) {
        xrtTableUid = getXrtTableUid( dragAndDropParams );
        dragAndDropParams.targetObjects = [ dragAndDropParams.declViewModel.vmo ];
    }

    if( isValidObjectToDrop( dragAndDropParams, xrtTableUid ) ) {
        ...
        if( dataTransferContainsFiles( event ) ) {
            event.preventDefault();
        }
    }
};
 ...
 * @param {Object} callbackAPIs - Callback functions used for various interaction reasons.
 */
const processDrop = function( dragAndDropParams ) {
    let event = dragAndDropParams.event;
    event.preventDefault();
    ...
    if( sourceUids && sourceUids.length > 0 ) {
        _dropModelObjects( targetElement, sourceUids, dragAndDropParams );
    } else {
        var sourceFiles = event.dataTransfer.files;
        if( sourceFiles && sourceFiles.length > 0 ) {
            _dropFiles( targetElement, sourceFiles, dragAndDropParams );
        } else {
            awDragAndDropUtils._clearCachedData();
        }
    }
...
const isTargetView = ( dragAndDropParams ) => {
    if( !dragAndDropParams.targetObjects && !dragAndDropParams.dataProvider ) {
        return true;
    }
    return false;
};
...
export const dragOver = function( dragAndDropParams ) {
    if( dragAndDropParams.event ) {
        if( isTargetView( dragAndDropParams ) ) {
            return;
        }
        ...
        isGlobalHighlightPublished = false;
        processDragOver( dragAndDropParams );
    }
};

export const dragEnter = function( dragAndDropParams ) {
    if( dragAndDropParams.event ) {
        if( isTargetView( dragAndDropParams ) ) {
            return;
        }
        ...
        dragAndDropParams.event.preventDefault();
        _setValidDropTypesOnTarget( dragAndDropParams );
        /*
...
export const drop = function( dragAndDropParams ) {
    let event = dragAndDropParams.event;
    if( event ) {
        if( _debug_logEventActivity >= 2 ) {
            logger.info( 'drop: ' + '\n' + JSON.stringify( event, null, 2 ) );
