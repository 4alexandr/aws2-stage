 ...
 */
export function setViewParam( viewParam ) {
 ...
 */
export function getFitViewParam( angle ) {
 ...
 */
export function rotate( angle, align ) {
    if( align ) {
    ...
    if( canvas && ctx && img ) {
        ...
        if( msg ) {
            msg.innerHTML = "<small>View Param = { scale: " + vp.scale.toFixed( 3 ) + ", x: " +
                vp.x.toFixed( 3 ) + ", y: " + vp.y.toFixed( 3 ) + ", angle2: " + vp.angle2 + " }</small>";
        }
    }
}
...
function setVpFit( angle, to ) {
    ...
    to.scale = Math.max( Math.min( canvasRect.width / imgBboxWidth, canvasRect.height / imgBboxHeight, 1 ),
        scaleMin );
...
//==================================================
let exports;
export let refresh = draw;
export let getContainer = function() {
    return container;
};
export let getCanvas = function() {
    return canvas;
};
export let getImage = function() {
    return img;
};
...
export let setPointChangeCallback = function( callback ) {
    pointChangeCallback = callback;
};
export let setResizeCallback = function( callback ) {
    resizeCallback = callback;
};
export let setDevEnv = function( b ) {
    devEnv = b;
};
