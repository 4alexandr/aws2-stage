 ...
 */
export let getDatasetFileInfos = function( dataset, data ) {
    var fileFormat = null;
    ...
    if( !data.fileName && hostedFileNameContext ) {
        ...
        data.fileNameNoExt = awFileNameUtils.getFileNameWithoutExtension( data.fileName );
    }
    ...
    if( refInfos ) {
        ...
        for( ii = 0; ii < refInfos.length; ii++ ) {
            refInfo = refInfos[ ii ];
            if( isFileOfReferenceType( data.fileName, refInfo, false ) ) {
        ...
        if( fileFormat === null || referenceName === null ) {
            for( ii = 0; ii < refInfos.length; ii++ ) {
                refInfo = refInfos[ ii ];
                if( isFileOfReferenceType( data.fileName, refInfo, true ) ) {
                    fileFormat = refInfo.fileFormat;
                    referenceName = refInfo.referenceName;
                    break;
                }
        ...
        var ticketFileName = data.datasetFileName.dbValue;
        if( !ticketFileName || ticketFileName === '' ) {
            ticketFileName = data.fileName;
        }
