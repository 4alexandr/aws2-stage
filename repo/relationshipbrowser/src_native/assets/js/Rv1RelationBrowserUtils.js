...
export let getFilteredCounts = function( legend, rootNodes, node ) {
    ...
    var inDegrees = node.appData.inDegrees;
    var outDegrees = node.appData.outDegrees;
    ...
    for( var x = 0; x < visibleEdges.length; ++x ) {
        for( var y = 0; y < rootNodes.length; ++y ) {
            if( rootNodes[ y ].appData.id !== node.appData.id ) {
                if( visibleEdges[ x ].appData.leftId === rootNodes[ y ].appData.id ) {
                    source.push( rootNodes[ y ].appData.category );
                }

                if( visibleEdges[ x ].appData.rightId === rootNodes[ y ].appData.id ) {
                    target.push( rootNodes[ y ].appData.category );
                }
            }
        }
    }
                ...
                _.forEach( view.categoryTypes, function( type ) {
                    ...
                    _.forEach( type.categories, function( category ) {
                        ...
                        if( category.isFiltered === true ) {
                            ...
                            for( var i = 0; i < counts.inDegrees.length; ++i ) {
                                var degree = counts.inDegrees[ i ];
                            ...
                            for( var i = 0; i < counts.outDegrees.length; ++i ) {
                                var degree = counts.outDegrees[ i ];
