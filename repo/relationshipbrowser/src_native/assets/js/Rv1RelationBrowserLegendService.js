    ...
    _.each( sortedFilters, function( filterValue ) {
        ...
        var foundRange = _.find( rangeList, function( range ) {
            if( range.lowerValue && range.upperValue ) {
                var bRangeOverlap = range.lowerValue.getTime() <= filterValue.upperValue.getTime() &&
                    filterValue.lowerValue.getTime() <= range.upperValue.getTime();

                // This is only a return within the _.find() function.
                // This does not escape getSelectedDateRanges().
                return bRangeOverlap;
            }
...
export let initLegendData = function( ctx, data ) {
    if( data.legendData ) {
        ...
        if( legendViewsData.length > 0 ) {
            //Set default view if specified in appCtx of RB
            if( appCtxService.ctx.initRelationsBrowserCtx && appCtxService.ctx.initRelationsBrowserCtx.defaultActiveView ) {
                let view = '';
                for( let i = 0; i < legendViewsData.length; i++ ) {
                    if( legendViewsData[ i ].internalName === appCtxService.ctx.initRelationsBrowserCtx.defaultActiveView ) {
                        legendViewsData[ i ].expand = true;
                        view = legendViewsData[ i ].internalName;
                        break;
                    }
                }
                data.legend.defaultActiveView = view;
        ...
        eventBus.publish( 'Rv1RelationsBrowser.legendInitialized' );
    }
};
...
export let getFilterMap = function( ctx, filterList ) {
    ...
    ctx.graph.legendData.filterMap = filterMap;
...
export let updateFilterData = function( ctx, data, filterList ) {
    ...
    if( promise ) {
        promise.then( function() {
            ...
            ctx.graph.legendData.legendFilters = categories;

            // Store a cloned copy.
            ctx.graph.legendData.currentFilters = _.clone( categories );
        } );
    }
};
...
export let cacheProperties = function( ctx, data ) {
    ...
    var _filterProperties = _getFilterProperties( ctx, data.preferences.RV1_DARB_Filter_Properties );
    return tcVmoSvc.getViewModelProperties( _visibleObjects, _filterProperties );
};
...
var applyFilter = function( ctx ) {
    ...
    var filterFunction = function( nodes, edges, ports, boundaries, itemsToHide ) {
        ...
        if( itemsToHide.length > 0 ) {
            ctx.graph.legendState.hasCustomFilters = true;
        } else {
            ctx.graph.legendState.hasCustomFilters = false;
        }
...
/**
 * Rv1RelationBrowserLegendService factory
 */
...
app.factory( 'Rv1RelationBrowserLegendService', () => exports );
