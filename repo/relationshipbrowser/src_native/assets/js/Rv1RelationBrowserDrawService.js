...
var setDefaultLayoutDirection = function( graphModel, legendLayout ) {
    if( legendLayout ) {
        if( graphModel.config.layout.options.indexOf( legendLayout ) > -1 ) {
            ...
            graphModel.config.layout.defaultOption = legendLayout;
        } else {
...
export let applyGraphLayout = function( ctx, data, graphModel, newAddedNodes, edges, isInitial, performanceTimer, sortedLayoutPreferenceValue ) {
    ...
    if( isInitial ) {
        var legendLayout = _.get( ctx.graph, 'legendState.activeView.defaultLayout', null );
    }
    if( layout === graphConstants.DFLayoutTypes.SortedLayout ) {
        ...
        rbSvc.licenseCheck()
            .then(
                ...
                // else !
                function() {
                    incUpdateLayoutExpand( graphModel, legendLayout, expandDirection, seedNodes, newAddedNodes, edges.addedEdges, isInitial );
                    postApplyGraphLayout( graphModel, newAddedNodes, edges.addedEdges, isInitial, performanceTimer );
                }
            );
...
app.factory( 'Rv1RelationBrowserDrawService', () => exports );
