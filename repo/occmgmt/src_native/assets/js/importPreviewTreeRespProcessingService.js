...
let getParentFromCollapsedNodes = function( parentNodeUid ) {
    ...
    let activeContext = appCtxService.ctx.aceActiveContext;
    if( !_.isUndefined( activeContext ) 
        && !_.isUndefined( activeContext.context.vmc ) )
    {
        let loadedVMOs = appCtxService.ctx.aceActiveContext.context.vmc.loadedVMObjects;
...
let findParentNodeForTreeNode = function( collectionOfCreatedNodes, treeNode ) {
    ...
    if ( _.isUndefined( parentNode ) && appCtxService.ctx.aceActiveContext.context.vmc )
    {
        let loadedVMOs = appCtxService.ctx.aceActiveContext.context.vmc.loadedVMObjects;
...
export let populateTreeNodes = function( nodeInfos, nodeToPropsArray, createdNodes ) {
    createTree( nodeInfos, nodeToPropsArray, createdNodes, 0 );
};
...
app.factory( 'importPreviewTreeRespProcessingService', () => exports );