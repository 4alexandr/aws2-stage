...
import 'js/colorDecoratorService';

var exports = {};

var _editEventSubDef = null;
var _addObjectEvent = null;
 ...
 */
var registerEventsInMarkupMode = function( isRegister ) {
    if( isRegister ) {
        if( !_editEventSubDef || !_addObjectEvent ) {
            _editEventSubDef = eventBus.subscribe( 'editHandlerStateChange', editEventHandlerCallbackFunction );
            _addObjectEvent = eventBus.subscribe( 'addElement.elementsAdded', function() {
                updateModifiedObjects();
            } );
        }
    } else if( _editEventSubDef || _addObjectEvent ) {
        eventBus.unsubscribe( _editEventSubDef );
        eventBus.unsubscribe( _addObjectEvent );
        _editEventSubDef = null;
        _addObjectEvent = null;
    }
};
...
export let updateCtxWithShowMarkupValue = function( markupToggleState ) {
    ...
    appCtxSvc.updatePartialCtx( 'aceActiveContext.context.isMarkupEnabled', isTrue );
    registerEventsInMarkupMode( isTrue );
};
...
let isChangeEnabled = function( vmo ) {
    let pci = occmgmtUtils.getProductContextInfoForProvidedObject( vmo );
...
var editEventHandlerCallbackFunction = function( context ) {
    ...
    // We are handling only 2 , saved state we will handle in save markup story.
    if( isMarkupModeEnabled() ) {
        if( context.state === 'starting' || context.state === 'canceling' ) {
            ...
            if( context.dataSource.awb0MarkupPropertyNames && context.dataSource.awb0MarkupPropertyNames.dbValues ) {
                ...
                if( context.dataSource.viewModelCollection.loadedVMObjects ) {
                    for( var vmoIndx = 0; vmoIndx < context.dataSource.viewModelCollection.loadedVMObjects.length; vmoIndx++ ) {
                        var vmo = context.dataSource.viewModelCollection.loadedVMObjects[ vmoIndx ];
                        handleLOVPropertyOldValueOnEditAction( vmo.props, isEditStarting );
                    }
                }
            }
        } else if( context.state === 'saved' && appCtxSvc.ctx.aceActiveContext.context.activeTab.name === 'Markup' ) {
            updateModifiedObjects();
        }
    }
};
    ...
    for ( let index = 0; index < markupPropNames.length; ++index ) {
        ...
        if ( propertyObject ) {
            ...
            if ( isMarkupModeEnabled() ) {
                ...
                updateDisplayValues( propertyObject, markupPropUIValue, propertyObject.uiValue, propertyObject.dbValue );
            }
        }
    }
};
 ...
 * @param {*} data
 */
let populateSourceAndTargetUidOfNewLinesInData = function( selectedVMO, response, data ) {
    ...
    for ( let index = 0, noOfCreatedObjects = response.created.length; index < noOfCreatedObjects; ++index ) {
        ...
        if ( targetMO && targetMO.props && targetMO.props.awb0Parent && targetMO.props.awb0Parent.dbValues
            ...
            for ( let noOfAddedMarkups = addedMarkupObjects.length; innerIdx < noOfAddedMarkups; ) {
                data.targetUids.push( response.created[index] );
                data.srcUids.push( addedMarkupObjects[innerIdx].uid );
                ++innerIdx;
                break;
            }
        }
 ...
 * @param {*} selectedVMO
 */
let populatesSourceAndTargetUidOfReplacedLinesInData = function( data, selectedVMO ) {
    if ( data.eventData && data.eventData.replacedObjects ) {
        for ( let index = 0, noOfReplacedObjects = data.eventData.replacedObjects.length; index < noOfReplacedObjects; ++index ) {
            for ( let innerIdx = 0, noOfChildObjects = selectedVMO.children.length; innerIdx < noOfChildObjects; ++innerIdx ) {
                if ( selectedVMO.children[innerIdx].uid === data.eventData.replacedObjects[index] ) {
                    data.srcUids.push( selectedVMO.children[innerIdx].uid );
                    data.targetUids.push( selectedVMO.children[innerIdx].uid );
                }
            }
        }
    }
 ...
 * @param {*} data
 */
let deletePropFieldToGetUpdatedPropsFromServer = function( response, selectedVMO, data ) {
    for ( let index = 0, noOfUpdatedObjects = response.updated.length; index < noOfUpdatedObjects; ++index ) {
        for ( let innerIdx = 0, noOfChildObjects = selectedVMO.children.length; innerIdx < noOfChildObjects; ++innerIdx ) {
            if ( selectedVMO.children[innerIdx].uid === response.updated[index] && !data.srcUids.includes( selectedVMO.children[innerIdx].uid ) ) {
...
export let setSourceAndTargetElements = function( response, data ) {
    if( appCtxSvc.ctx.aceActiveContext.context.vmc ) {
        ...
        data.srcUids = [];
        data.targetUids = [];
        let selectedVMOIdx = _.findLastIndex( vmc.loadedVMObjects, function( vmo ) {
        ...
        if( selectedVMO && selectedVMO.children ) {
            ...
            if( response.created ) {
                populateSourceAndTargetUidOfNewLinesInData( selectedVMO, response, data );
            }
            //Populate Source and Target uids for replaced bomlines if any
            populatesSourceAndTargetUidOfReplacedLinesInData( data, selectedVMO );
            // Deleting the props field for updated property which will trigger SOA to update the properties.
            if( response.updated ) {
                deletePropFieldToGetUpdatedPropsFromServer( response, selectedVMO, data );
            }
        }
        return data.srcUids.length;
    }
};
 ...
 * @param {*} data
 */
export let applyMarkupPreProcessing = function( data ) {
    if( appCtxSvc.ctx.aceActiveContext.context.vmc ) {
        var vmc = appCtxSvc.ctx.aceActiveContext.context.vmc;
        data.replacedObjects = [];

        var selectedVMOIdx = _.findLastIndex( vmc.loadedVMObjects, function( vmo ) {
            return vmo.uid === appCtxSvc.ctx.selected.uid;
        } );
        ...
        if( selectedVMO && selectedVMO.children ) {
            selectedVMO.children.filter( function( vmo ) {
                if( vmo.props && vmo.props.awb0MarkupType && vmo.props.awb0MarkupType.dbValue === 8 ) {
                    data.replacedObjects.push( vmo.uid );
                }
            } );
        }
    }
...
app.factory( 'showMarkupService', () => exports );
