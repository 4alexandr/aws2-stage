...
let getPropFieldForID = function() {
    ...
    if ( !_.isUndefined( appCtxSvc.ctx.ImportBOMContext ) && !_.isUndefined( appCtxSvc.ctx.ImportBOMContext.viewModelPropertiesForHeader ) ) {
        let headerVMOMappedToIDField = _.filter(appCtxSvc.ctx.ImportBOMContext.viewModelPropertiesForHeader, function ( viewModelPropHeader ) {
...
/**
 * API takes collection fo VMOs from caller and searches for reused line in that collection. If 
...
let findSelectedObjectAndReusedAssembly = function( loadedVMOs, updatedVMOs ) {
    let selectedObjs = appCtxSvc.ctx.mselected;
 ...
 */
export let populateActionInfoMapForImportSOAInput= function( soaInput ) {
    let loadedVMOs = appCtxSvc.ctx.aceActiveContext.context.vmc.loadedVMObjects;
    if( loadedVMOs ) {
        _.forEach( loadedVMOs, function( vmo ){
            if( !_.isUndefined( vmo.props ) )
            {
                soaInput.actionInfo[ vmo.uid ] = vmo.props[localBundle.actionColumn].uiValue;
                if( vmo.__expandState )
                {
                    _.forEach( vmo.__expandState.expandedNodes, function( expandedNode ){
                        if( !_.isUndefined( expandedNode.props ) )
                        {
                            soaInput.actionInfo[ expandedNode.uid ] = expandedNode.props[localBundle.actionColumn].uiValue;
                        }
                    } );
                }
            }
...
app.factory( 'importPreviewSetActionOnLine', () => exports );