...
var _getConvertCSIDsToElementsInAceCallback = function( response ) {
    if( response.totalFound > 0 && !_.isEmpty( response.searchResults ) ) {
        var selectedUIDs = [];
        var currentSelections = appCtxSvc.getCtx( 'mselected' );

        _.forEach( response.searchResults, function( resultObj ) {
            var existInCurrentSelection = false;

            var modelObject = cdm.getObject( resultObj.uid );
 ...
 * @param {messagingService} msgSvc - Service to use.
 * @param {csidsToObjectsConverterService} csidConverterSvc - Service to use.
 * @param {objectToCSIDGeneratorService} objectToCSIDGeneratorSvc - Service to use.
 * @param {hostBaseRefService} hostBaseRefSvc - Service to use.
 * @param {hostBaseSelService} hostBaseSelSvc - Service to use.
 * @param {hostConfigService} hostConfigSvc - Service to use.
 * @param {soa_kernel_clientDataModel} cdm - Service to use.
 * @param {soa_kernel_clientMetaModel} cmm - Service to use.
 * @param {soa_dataManagementService} dms - Service to use.
 * @param {soa_preferenceService} prefSvc - Service to use.
 *
 * @returns {hostOcc_2014_10} Reference to this service's API object.
 */
app.factory( 'hostOcc_2014_10', () => exports );
