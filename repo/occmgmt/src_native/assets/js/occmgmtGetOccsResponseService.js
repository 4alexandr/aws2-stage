...
export let populateRequestPrefInfoOnOccmgmtContext = function( viewModeLoadResult, response, contextKey ) {
    ...
    if( !_.isUndefined( response.requestPref ) ) {
        viewModeLoadResult.showInEffectiveOcc = !_.isUndefined( response.requestPref.PSEShowUnconfigdEffPref ) ?
            _.isEqual( response.requestPref.PSEShowUnconfigdEffPref[ 0 ], 'true' ) :
            appCtxService.ctx.aceActiveContext.context.showInEffectiveOcc;
    }

    if( !_.isUndefined( response.requestPref ) ) {
        viewModeLoadResult.showVariantsInOcc = !_.isUndefined( response.requestPref.PSEShowUnconfigdVarPref ) ?
            _.isEqual( response.requestPref.PSEShowUnconfigdVarPref[ 0 ], 'true' ) :
            appCtxService.ctx.aceActiveContext.context.showVariantsInOcc;
    }

    if( !_.isUndefined( response.requestPref ) ) {
        viewModeLoadResult.showSuppressedOcc = !_.isUndefined( response.requestPref.PSEShowSuppressedOccsPref ) ?
            _.isEqual( response.requestPref.PSEShowSuppressedOccsPref[ 0 ], 'true' ) :
            appCtxService.ctx.aceActiveContext.context.showSuppressedOcc;
    }

    if( !_.isUndefined( response.requestPref ) && !_.isUndefined( response.requestPref.reloadDependentTabs ) && 
        _.isEqual( response.requestPref.reloadDependentTabs[ 0 ], 'false' ) ) {
        if( !_.isUndefined( contextKey ) ) {
            if( _.isUndefined( appCtxService.ctx[ contextKey ].transientRequestPref ) ) {
                appCtxService.ctx[ contextKey ].transientRequestPref={};
            }
            appCtxService.ctx[ contextKey ].transientRequestPref.reloadDependentTabs = "false";
        }
    }
