...
/**
 * take the selected bookmark object and get the tracelink within that context
 *
 * @param {Object} data - The panel's view model object
 */
export let selectBookmark = function( data ) {
    if( data.listofWorkingContext && data.listofWorkingContext.selectedLovEntries && data.listofWorkingContext.selectedLovEntries[ 0 ] && ( data.listofWorkingContext.selectedLovEntries[ 0 ].uid === '' || data.listofWorkingContext.selectedLovEntries[ 0 ].uid.length > 0 ) ) {
        eventBus.publish( 'CreateTracelink.loadExistingTracelinks' );
    }
};

/**
 * Open selected business object revision
 *
 * @param {Object} data - The panel's view model object
 * @param {Object} selectedCell - selectedCell
 */
export let filterAction = function( data ) {
    var resultArr = [];

    var filter = data.filterBox.dbValue.toUpperCase();

    for( var i = 0; i < data.arrTracelinkedItems.length; i++ ) {
        var item = data.arrTracelinkedItems[ i ];
        if( item.cellHeader1.toUpperCase().indexOf( filter ) > -1 ) {
            resultArr.push( item );
        }
    }
    data.tracelinkedItems.dbValue = resultArr;
};

/**
 * Init Current tracelinks for selected business object in ACE
 *
 * @param {Object} data - The panel's view model object
 */
export let initCurrentTracelinkDataACE = function( data ) {
    var arrItems = data.searchResults.objects;
    var arrResult = [];

    if( arrItems && arrItems.length ) {
        for( var i = 0; i < arrItems.length; i++ ) {
            var linkInfo = cdm.getObject( arrItems[ i ].uid );
            var itemP = _getTracelinkedItem( linkInfo, data );
            if( itemP ) {
                arrResult.push( itemP );
            }
        }
    }

    data.arrTracelinkedItems = arrResult;
    exports.filterAction( data );

    if( data.listofWorkingContext && data.listofWorkingContext.selectedLovEntries && data.listofWorkingContext.selectedLovEntries.length === 0 ) {
        // this will be uncommented when we enable saved bookmark tracelink filter story
        //exports.searchAvailableWorkingContext(data);
    }
};

export let  publishEventToShowExistingTraceLinks = function() {
    eventBus.publish( 'Arm0CreateTracelink.showExistingTraceLinks' );
};
 ...
 */
var _getRootElementUids = function() {
    return reqTracelinkService.getRootElementUids();
};
...
var _addInObjectList = function( data, destObjectList, otherObjectList, newObjList ) {
   var newObjs = _getTracelinkEligbleObjList(newObjList);
    ...
    for( var i = 0; i <= newObjs.length - 1; i++ ) {
        ...
        if( newObj ) {
            var isExistInOtherList = _isExistRevObjectInArray( otherObjectList.dbValue, newObj );
 ...
 * @return {Object} input data for trace link creation
 */
var _getCreateTraceLinkInput = function( data, primaryObject, secondaryObject ) {
    var objInput = {};
    ...
    var propNameValues = {};
    if( data.objCreateInfo && data.objCreateInfo.propNamesForCreate.length > 0 ) {
        var createInput = addObjectUtils.getCreateInput( data );
        var createData = createInput[ 0 ].createData;
        var dataObj = reqTracelinkService.getProperties2( data, createData.propertyNameValues );
...
export let getProviderName = function( ctx ) {
    parentData.rowProductContextFromMatrix = ctx.rowProductContextFromMatrix;
    parentData.srcObjectFromMatrix = ctx.srcObjectFromMatrix;
    ctx.rowProductContextFromMatrix = undefined;
    ctx.srcObjectFromMatrix = undefined;
...
/**
 * The method get the opened bookmark object UID into a string.
 *
 * @param {Object} data - view model object data
 * @return {String}  uid of open bookmark object
 */
var _getBookmarkObjUid = function( data ) {
    if( data.listofWorkingContext && data.listofWorkingContext.selectedLovEntries && data.listofWorkingContext.selectedLovEntries[ 0 ] && data.listofWorkingContext.selectedLovEntries[ 0 ].uid ) {
        return data.listofWorkingContext.selectedLovEntries[ 0 ].uid;
    }
    return '';
};
/**
...
export let getSearchInputExistingTracelink = function( ctx, data ) {
    var currentSelected = ctx.selected.uid;

    if( ctx.aceActiveContext && ctx.aceActiveContext.context ) {
        var tmpSeletecd = ctx.aceActiveContext.context.pwaSelectionModel.getSelection()[ 0 ];
        currentSelected = tmpSeletecd ? tmpSeletecd : currentSelected;
    }
    if( parentData.viewExistingTracelink ) {
        currentSelected = parentData.sourceObject;

        parentData.viewExistingTracelink = false;
    }
    ...
    var selectedObj = cdm.getObject(currentSelected);
    selectedObj = _getTracelinkEligbleObj(selectedObj);
    if (selectedObj) {
        currentSelected = selectedObj.uid;
    }
    ...
    if( ctx.aceActiveContext && ctx.aceActiveContext.context || parentData.rowProductContextFromMatrix ) {
        inputSearch = {
            ...
            rootElementUids: _getRootElementUids(),
            bookmarkObj: _getBookmarkObjUid( data ),
 ...
 */
export let getInputDeleteTraceLink = function( data, selectedCell ) {
    var uidProxyTracelink = selectedCell.uidProxyTracelink;
    var elementsToRemove = {};
    if( uidProxyTracelink ) {
        var traceLinkObj = cdm.getObject( uidProxyTracelink );
        var relationObject = cdm.getObject( traceLinkObj.props.awp0RelationObject.dbValues[ 0 ] );
        var relatedObject = cdm.getObject( traceLinkObj.props.awp0RelatedObject.dbValues[ 0 ] );
        var selectedObject = cdm.getObject( traceLinkObj.props.awp0SelectedObject.dbValues[ 0 ] );

        elementsToRemove = [ {
            type: relationObject.type,
            uid: relationObject.uid
        } ];

        data.refreshObjects = [ {
                uid: relatedObject.uid,
...
var _checkObjectCopiedFromClipboard = function( ctx ) {
    var clipboardObjects = ClipboardService.instance.getContents();
    if( clipboardObjects && clipboardObjects.length > 0 ) {
        ctx.createTraceLinkWorkSpace.hasObjectsInClipboard = true;
    }
};
/**
...
export let initStartItemList = function( data, ctx ) {
    ...
    parentData = data;
    modelObjectsList = [];
    data.viewExistingTracelink = false;
    data.sourceObject = [];
    ...
    if( panelContext && panelContext.sourceObject ) {
        ...
        obj1 = _getTracelinkEligbleObj(obj1);
            if (obj1) {
                ...
                data.sourceObject.push(obj1.uid);
            }
        }
        if( panelContext.viewExistingTracelink ) {
            data.viewExistingTracelink = true;
        }
    } else {
...
export let generateTraceLinkTypes = function( data ) {
    if( data.preferences.TraceReport_Default_TraceLink_Type &&
        data.preferences.TraceReport_Default_TraceLink_Type[ 0 ] ) {
        data.defaultTraceLinkType = data.preferences.TraceReport_Default_TraceLink_Type[ 0 ];
    } else {
    ...
    preferenceService.getMultiStringValues( [ 'REQ_default_tracelink_Types' ] ).then( function( prefs ) {
        var finalList = [];
        ...
        data.traceLinkTypeList = finalList;
        data.traceLinkType.dbValue = bOTypeNamesResponse[ indexSpecEle ].propInternalValue;
    } );
};
...
export let traceLinkTypeUpdated = function( data ) {
    if( data.traceLinkType.dbValue !== TRACELINK_TYPE_RULE_BASED_INTERNAL_VAL ) {
        data.fetchTracelinkProperty = true;
    } else {
        data.fetchTracelinkProperty = false;
    }
    data.selectedTraceLinkType = data.traceLinkType.dbValue;
};
...
export let addObjectToTracelinkPanel = function( panelContext ) {
    if( panelContext.sourceObject && panelContext.destObject ) {
        ...
        commandPanelService.activateCommandPanel( 'Arm0CreateTraceLinkCurrentSub', 'aw_toolsAndInfo' );
    } else {
...
export const dropCustomHighlight = ( dragAndDropParams ) => {
    ...
    exports.pasteObjectsInList( targetObject, draggedObjects );
    clearCachedDragDropData();
    return {
        preventDefault: true
    };
...
export default exports = {
    ...
    publishEventToShowExistingTraceLinks
};
/**
 ...
 */
app.factory( 'Arm0CreateTraceLink', () => exports );
