 ...
 */
export let showHtmlCompareReport = function( htmlData ) {
    ...
    if ( divs && divs.length > 0 ) {
        ...
        bodyTextDiv.innerHTML = compareUtils.addCssInContents( htmlData );
 ...
 */
export let showCompareVersionHistory = function( data ) {
    data.compareHtmlData = null;
    data.selectedFullText = null;
};
 ...
 */
export let getSelectedObjectsHtml = function( fullTextCompareDataListDataProvider, ctx, data ) {
    ...
    if ( selectedIndexArray.length === 2 ) {
        ...
        ctx.Arm0RequirementHistoryContext.selectedFullText = {
            Obj1: revFullTextVersionObjectString_1,
            Obj2: revFullTextVersionObjectString_2
        };

        showHistoryLabel( revFullTextVersionObjectString_1, revFullTextVersionObjectString_2, data );
        return [
            html_1,
            html_2
        ];
...
var showHistoryLabel = function( Obj1, Obj2, data ) {
    ...
    appCtxSvc.registerCtx( 'Arm0RequirementHistoryContextCompareToMsg', msg );
};
 ...
 * @param {Object} ctx - context
 */
export let compareSpecRevisions = function( ctx, data ) {
    if ( ctx.mselected.length === 2 ) {
        ...
        if ( specRev1 && specRev1.props && specRev1.props.object_string && specRev2 && specRev2.props && specRev2.props.object_string ) {
            ...
            showHistoryLabel( specRev1.props.object_string.displayValues[0], specRev2.props.object_string.displayValues[0], data );
        }
        eventBus.publish( 'Arm0RequirementHistory.getSpecContentData', objects );
    }
};
 ...
 */
export let preProcessSpecContents = function( htmlContents ) {
...
export let showSpecCompareReport = function( htmlData ) {
    ...
    if ( divs && divs.length > 0 ) {
        ...
        if ( historySectionElement ) {
            ...
            divs[0].innerHTML = compareUtils.addCssInContents( htmlData );
        }
    }
};
...
export let swapComparisonObjects = function( data ) {
    ...
    data.htmlContents = htmlContent;
    eventBus.publish( 'Arm0RequirementHistory.compareSpecContents' );

    var obj1 = data.dataProviders.selectedSpecRevs.selectedSpecRevs.Obj1.split( ';' );
    var rev_1_Revision = obj1[0];
    var rev_1_VersionNumber  = obj1[1];

    var obj2 = data.dataProviders.selectedSpecRevs.selectedSpecRevs.Obj2.split( ';' );
    ...
    var revFullTextVersionObjectString_2 = rev_2_Revision + ';' + '\n' + rev_2_VersionNumber;
     ...
     showHistoryLabel( revFullTextVersionObjectString_2, revFullTextVersionObjectString_1, data );
     var temp =  data.dataProviders.selectedSpecRevs.selectedSpecRevs.Obj1;
     data.dataProviders.selectedSpecRevs.selectedSpecRevs.Obj1 = data.dataProviders.selectedSpecRevs.selectedSpecRevs.Obj2;
     data.dataProviders.selectedSpecRevs.selectedSpecRevs.Obj2 = temp;
};
...
app.factory( 'Arm0CompareHistoryService', () => exports );
