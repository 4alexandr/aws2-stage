...
var AWP0_CREATE_BASELINE_COMPLETE = 'Awp0Create_Baseline_Complete';
...
export let openLineItem = function( data ) {
    if( data.eventObj.props.eventtype_id && data.object && data.object.uid ) {
        ...
        } else if( eventTypeId === AWP0_CREATE_BASELINE_COMPLETE || eventTypeId === FND0_WORD_IMPORT_COMPLETE ) {
                ...
                promise.then( function( readFileTicketsResponse ) {
                    ...
                    if( readFileTicketsResponse && readFileTicketsResponse.tickets && readFileTicketsResponse.tickets.length > 1 ) {
                        ...
                        if( ticketsArray && ticketsArray.length > 0 ) {
                            localStorage.publish( 'TraceabilityMatrixJsonFileFMSTicket', ticketsArray[ 0 ] );
                        }
                        var serviceDataForTracelinkMatrix = window.sessionStorage.getItem( 'serviceDataForTracelinkMatrix' );
                        if(serviceDataForTracelinkMatrix){
                            window.sessionStorage.removeItem( 'serviceDataForTracelinkMatrix');
                        }
                        var req_generate_matrix_payload2 = window.sessionStorage.getItem( 'req_generate_matrix_payload2' );
                        if(req_generate_matrix_payload2){
...
var openRelatedObjects = function( notificationObj ) {
    ...
    if( notificationObj.props.fnd0Subject && notificationObj.props.fnd0Subject.dbValues.length > 0 ) {
        ...
        isError = _.startsWith( subject, 'Baseline creation failed' ) || _.startsWith( subject,
            'Import specification failed' ) || _.startsWith( subject, 'Import ReqIF failed' );
    }
...
/**
 * Service to define actions on alert notification click for requirement management application
 *
 * @member requirementsNotificationService
 */

export default exports = {
    openLineItem
};
app.factory( 'requirementsNotificationService', () => exports );
