 ...
 *
 */
function changeTypeOfElement( eventData ) {
    var selectedSpecID = appCtxService.getCtx( 'selectedRequirementObjectUID' );
    var childrenOfSelectedItem = idToObjectMap[ selectedSpecID ].children;
    var element;
    ...
    if( childrenOfSelectedItem && childrenOfSelectedItem.length > 0 ) {
        ...
        if( isTypeChangeAllowed ) {
            exports.updateDataMap( selectedSpecID, eventData.sourceObject.displayName, eventData.sourceObject.internalName );
        } else {
            // 'error' handling
        }
    } else {
...
export let getSetting = function( data, ctx ) {
    ...
    var settingList = {
        Revise: {
            iconURL: app.getBaseUrlPath() + '/image/cmdUpdateAssignedRevision24.svg',
            displayName: data.i18n.revise,
            internalName: 'Revise'
        },
        Update: {
            iconURL: app.getBaseUrlPath() + '/image/cmdUpToDate24.svg',
            displayName: data.i18n.update,
            internalName: 'Update'
        },
        Delete: {
            iconURL: app.getBaseUrlPath() + '/image/cmdDelete24.svg',
            displayName: data.i18n.delete,
            internalName: 'Delete'
        },
        Add: {
            iconURL: app.getBaseUrlPath() + '/image/cmdAdd24.svg',
            displayName: data.i18n.Add,
            internalName: 'Add'
        },
        NoChange: {
            iconURL: app.getBaseUrlPath() + '/image/cmdDiscardUpdate16.svg',
            displayName: 'Discard Update',
            internalName: 'NoChange'
        },
        AcceptUpdate: {
            iconURL: app.getBaseUrlPath() + '/image/cmdUpToDate24.svg',
            displayName: 'Accept Update',
    ...
    if( selectedTargetElement && ( selectedTargetElement.classList.contains( REMOVED_CLASS )
       || selectedTargetElement.classList.contains( ADD_CLASS ) || selectedTargetElement.classList.contains( 'diff-html-changed' ) ) ) {
        ...
        switch ( reqobject.status ) {
            case 'Update':
                if( reqobject.parent ) {  // if not top line
            ...
            case 'NoChange':
                ...
                if( reqobject.isNoChangeSelected ) {
                    publishList.push( settingList.Update );
                }
                break;
            case 'Delete':
                if( reqobject.contentChange === 'Yes' ) {
                    publishList.push( settingList.Revise );
                    publishList.push( settingList.Update );
                } else {
 ...
 */
app.factory( 'Arm0ImportPreviewJsonHandlerService', () => exports );
