...
export default class ApsEffectivityValidationService extends AwBaseService {
     ...
     */
    populateDateRangeEffectivityDates( effectivityInfo, startDate, endDate, endDateOptions ) {
        ...
        if( eDateStr !== null &&
            ...
            endDateOptions.dbValue = this.UP;
        } else if( eDateStr !== null &&
            ( eDateStr.indexOf( this.EFFECTIVITY_SO_DATE ) !== -1 || eDateStr
                .indexOf( this.EFFECTIVITY_DATE_DECEMBER_25 ) !== -1 ) ) {
            endDateOptions.dbValue = this.SO;
        } else {
            endDateOptions.dbValue = this.DATE;
            endDate.dbValue = new Date( eDateStr ).getTime();
            endDate.dateApi.dateObject = dateTimeSvc.getJSDate( endDate.dbValue );
            var tempDate = dateTimeSvc.getJSDate( startDate.dbValue );
            tempDate.setDate( startDate.dateApi.dateObject.getDate() + 1 );
            tempDate.setHours( 0 );
            tempDate.setMinutes( 0 );
            tempDate.setSeconds( 0 );
            if( dateTimeSvc.compare( tempDate, endDate.dateApi.dateObject ) === 0 ) {
                endDate.dbValue = new Date( sDateStr ).getTime();
            }
        }
    }
 ...
 */
app.factory( 'apsEffectivityValidationService', () => ApsEffectivityValidationService.instance );
