...
export let pmProcessAddElementInput = function() {
    ...
    // set the addElement
    appCtxService.ctx.PartMfg.addElement = addElement;
};
...
export let setDefaultOccTypeOnElements = function( newElements ) {
    var deferred = AwPromiseService.instance.defer();
    var partMfgCtx = appCtxService.getCtx( 'PartMfg' );
    var parentItemRev = appCtxService.getCtx( 'locationContext' ).modelObject;

    occTypeSvc.loadOccTypesInfo( parentItemRev, newElements).then(
        function() {
            var inputs = [];
            _.forEach( newElements, function( newElement ) {
                var awb0UnderlyingType = newElement.props.awb0UnderlyingObjectType.dbValues[0];
...
app.factory( 'partMfgAddResourceService', () => exports );
