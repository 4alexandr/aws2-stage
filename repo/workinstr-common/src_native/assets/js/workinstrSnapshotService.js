...
import viewerProgIndicatorSvc from 'js/viewerProgressIndicatorService';
...
export let setViewerCtxData = function( viewerCtxNamespace, rootModelObjectPath ) {
    var toolsAndInfoCtx = appCtxSvc.getCtx( 'activeToolsAndInfoCommand' );
    if( !toolsAndInfoCtx || toolsAndInfoCtx && toolsAndInfoCtx.commandId !== 'Awv0ImageCapture' && toolsAndInfoCtx.commandId !== 'Awv0GeometricAnalysisMeasure' && toolsAndInfoCtx.commandId !== 'Awv0GeometricAnalysisQuery' && toolsAndInfoCtx.commandId !== 'Awv0GeometricAnalysisSection' ) {
...
export let getCloneStableId = function( modelObjectUid ) {
    ...
    if( cloneStableIDChain ) {
        cloneStableIDPropValue = cloneStableIDChain.dbValue;
...
export let getViewerView = function( viewerLoadInputParams ) {
    ...
    var viewerloadPromise = exports.loadByModelObjectInputParam( viewerLoadInputParams );
 ...
 * @param {Boolean} isViewerVisible true if viewer is visible
 */
export let updateStructureViewerVisibility = function( viewerCtxNamespace, isViewerVisible ) {
    viewerRenderSvc.updateViewerVisibility( viewerCtxNamespace, isViewerVisible );
};
...
export let getViewerLoadInputParameter = function( datasetObj, viewerWidth, viewerHeight ) {
    ...
    if( visCtx.snapshotConfiguration === 'Dynamic' ) {
        exports.createVVIFileDynamic( datasetObj, topLine ).then( function( additionalInfo ) {
        ...
        exports.createVVIFileStatic( datasetObj ).then( function( additionalInfo ) {
            viewerLoadInputParams.setAdditionalInfo( additionalInfo );
            returnPromise.resolve( viewerLoadInputParams );
        }, function( errorMsg ) {
            returnPromise.reject( errorMsg );
...
export let loadByModelObjectInputParam = function( viewerLoadInputParameters ) {
    ...
    viewerRenderSvc.updateViewerVisibility( viewerCtxName, false );
    ...
    fmsTicketPromise.then( function( fmsTkt ) {
            viewerPreferenceSvc.getViewerPreferences( viewerLoadInputParameters.isShowAll() ).then( function( viewerPreferences ) {
                ...
                viewerProgIndicatorSvc.init( viewerView );
                viewerSecurityMarkingSvc.setupSecurityMarking( viewerView );
                ...
                open3DViewPromise.then( function() {
                    ...
                    viewerRenderSvc.updateViewerVisibility( viewerCtxName, true );
 ...
 * @param {Object} viewerProgIndicatorSvc - viewerProgressIndicatorService
...
app.factory( 'workinstrSnapshotService', () => exports );
