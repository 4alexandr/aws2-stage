...
export let updateCreateButton = function( data ) {
    ...
    if( data.deliverComposed.dbValue === true || data.deliverComposed.displayValues[ 0 ].toUpperCase() === 'TRUE' ||
        data.deliverDecomposed.dbValue === true || data.deliverDecomposed.displayValues[ 0 ].toUpperCase() === 'TRUE' ) {
        if( langSelected === 'true' ) {
            isEnabled = 'true';
        }
    }

    data.createDeliveryVisible = isEnabled;
};
...
export let checkUploadsCompleteForTranslation = function( data ) {
    if( data.fmsTickets.index < data.fmsTickets.length ) {
        eventBus.publish( 'ctm1.fmsTicketGeneratedForTranslation', {} );
    } else {
        eventBus.publish( 'ctm1.fileUploadedCompleteForTranslation', {} );
    }
};
...
export default exports = {
    ...
    checkUploadsCompleteForTranslation
};
app.factory( 'Ctm1ContentMgmtTranslationService', () => exports );
