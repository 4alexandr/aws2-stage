...
export function geomWorldToScreen( geom, viewParam ) {
    ...
    if( shape === "freehand" || shape === "polyline" || shape === "polygon" ||
        shape === "curve" || shape === "closed-curve" ) {
        result.vertices = pointArrayWorldToScreen( geom.vertices, viewParam );
        if( shape === "polyline" || shape === "curve" ) {
            result.startArrow = geom.startArrow;
            result.endArrow = geom.endArrow;
        }
    } else if( shape === "circle" || shape === "ellipse" || shape === "rectangle" ) {
        result.center = pointWorldToScreen( geom.center, viewParam );
        result.major = geom.major * viewParam.scale;
        result.minor = geom.minor * viewParam.scale;
        result.angle = geom.angle + ( viewParam.angle2 ? viewParam.angle2 : 0 );
        result.cornerRadius = geom.cornerRadius;
    } else if( shape === "gdnt" ) {
        result.startPt = pointWorldToScreen( geom.startPt, viewParam );
        result.endPt = pointWorldToScreen( geom.endPt, viewParam );
    }
    return result;
}
 ...
 *
 * @param {FitPathPoint} point The point in world coord
 ...
 */
export function pointInGeom( point, geom, viewParam, extraTol ) {
    ...
    if( point.x < bbox.xmin - bTol || point.x > bbox.xmax + bTol ||
    ...
    if( geom.shape === "freehand" ) {
        for( var i = 0; i < geom.vertices.length; i++ ) {
            var dxi = point.x - geom.vertices[ i ].x;
            var dyi = point.y - geom.vertices[ i ].y;
            if( dxi * dxi + dyi * dyi < lenTol2 ) {
                return true;
            }
        }
    } else if( geom.shape === "polyline" || geom.shape === "curve" ) {
        var vertices = geom.shape === "curve" ? getCurveApproxPts( geom ) : geom.vertices;
        for( var j = 0; j < vertices.length - 1; j++ ) {
            var v0x = vertices[ j + 1 ].x - vertices[ j ].x;
            var v0y = vertices[ j + 1 ].y - vertices[ j ].y;
            var len2 = v0x * v0x + v0y * v0y;
            var len = Math.sqrt( len2 );

            var v1x = point.x - vertices[ j ].x;
            var v1y = point.y - vertices[ j ].y;
            var t = ( v0x * v1x + v0y * v1y ) / len2;

            if( 0 <= t && t <= 1 && Math.abs( v0x * v1y - v1x * v0y ) / len <= lenTol ) {
                return true;
            }
        }
    } else if( geom.shape === "polygon" || geom.shape === "closed-curve" ) {
        var vertices = geom.shape === "closed-curve" ? getCurveApproxPts( geom ) : geom.vertices;
        for( var k = 1; k < vertices.length - 2; k++ ) {
            if( pointInTriangle( point, vertices[ 0 ], vertices[ k ], vertices[ k + 1 ] ) ) {
                return true;
            }
        }
        ...
        var sin = Math.sin( geom.angle );
        var cos = Math.cos( geom.angle );

        var x = dxc * cos + dyc * sin;
        var y = -dxc * sin + dyc * cos;
        var a = geom.major + extTol;
        var b = geom.minor + extTol;
        ...
        var r = geom.major + extTol;
        if( dx0 * dx0 + dy0 * dy0 <= r * r ) {
            return true;
        }
    } else if( geom.shape === "gdnt" ) {
        return geom.bbox.xmin - extTol <= point.x && point.x <= geom.bbox.xmax + extTol &&
               geom.bbox.ymin - extTol <= point.y && point.y <= geom.bbox.ymax + extTol;
...
export function getGeomBbox( geom ) {
    ...
    if( geom.shape === "freehand" || geom.shape === "polyline" || 
        ...
        var vertices = geom.shape === "curve" || geom.shape === "closed-curve" ? 
...
export function getGeomArea( geom ) {
    ...
    if( geom.shape === "freehand" || geom.shape === "polyline" || geom.shape === "curve" ) {
        ...
        for( var j = 1; j < vertices.length; j++ ) {
            area += vertices[ j - 1 ].x * vertices[ j ].y - vertices[ j - 1 ].y * vertices[ j ].x;
        }
...
export function getGeomRect( geom, clip ) {
    ...
    if( clip ) {
        ...
        var n = !geom.startArrow ? 0 : !geom.endArrow ? geom.vertices.length - 1 :
 ...
 * @return [FitPathPoint] The array of the transformed points
 */
function pointArrayWorldToScreen( array, viewParam ) {
    var result = [];
    for( var i = 0; i < array.length; i++ ) {
        result.push( pointWorldToScreen( array[ i ], viewParam ) );
    }
    return result;
}
