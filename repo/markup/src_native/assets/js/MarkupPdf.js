...
function genOneRefImage( markup, width, height ) {
    ...
    var baseImage = pageContainer.querySelector( 'div canvas#page' + ( pageIndex + 1 ) );
...
function setVewParamFromRotation() {
    var rotation = pdfView.pagesRotation;
    if( rotation % 360 === 0 ) {
        vp.x = 0;
        vp.y = 0;
    }
    ...
    vp.angle2 = Math.PI * rotation / 180;
}
...
function addEventListeners() {
    ...
    if( pageContainer ) {
        ...
        pageContainer.addEventListener( "mouseout", mouseOutHandler );
