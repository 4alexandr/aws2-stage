 ...
 */
export let initialize = function( data, input ) {
    var element = document.getElementById( 'timeline' );
    var readOnly = false;
    var startDate = null;
    var endDate = null;
    timelineUtils.initTimelineWidget( element, input, timelineDataSource );
    var timelineConfig = getTimelineConfig( readOnly, startDate, endDate );
    var timelineZoomPreference;
    timelineZoomPreference =  data.preferences.AWC_Timeline_Zoom_Level ? data.preferences.AWC_Timeline_Zoom_Level[ 0 ] : undefined;
    //for case when only sublocation change occurs
    var ctxGanttZoomLevel = appCtx.ctx.timelineZoomLevel;
    if( !data.preferences.AWC_Timeline_Zoom_Level && ctxGanttZoomLevel ) {
        timelineZoomPreference = ctxGanttZoomLevel;
    } else if( data.preferences.AWC_Timeline_Zoom_Level ) {
        appCtx.ctx.AWC_Timeline_Zoom_Level = data.preferences.AWC_Timeline_Zoom_Level[ 0 ];
    }
    timelineUtils.setTimelineConfig( timelineConfig, timelineZoomPreference  );
    timelineUtils.assureCSSInitialization();
    //Do CSS initialization before Java-script.
    //see https://developer.yahoo.com/performance/rules.html#css_top=
...
var getTimelineConfig = function( readOnly, startDate, endDate ) {
    ...
    TimelineConfig.dragLinks = false;
