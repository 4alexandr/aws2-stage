...
export let getDeleteObjectsInput = function() {
    var objects = [];
    var selection = selectionService.getSelection().selected;

    if( selection && selection.length > 0 ) {
        for( var index = 0; index < selection.length; index++ ) {
            objects.push( selection[ index ] );
        }
    }
    appCtxService.ctx.pgp0DeletedPlanObj = objects;
...
export let getNameOfDeletedObject = function() {
    var nameOfDeletedObject = '';
    var selection = selectionService.getSelection().selected;

    if( selection && selection.length > 0 ) {
        nameOfDeletedObject = selection[ 0 ].props.object_name.uiValues[ 0 ];
    }

    return nameOfDeletedObject;
};
...
export let timelinecheckStateOfEventForDeletion = function( data ){
    var selection = appCtxService.ctx.timelineContext.selected;
    data.eventStateFlag = true;
    if( selection.props.prg0State.dbValues[ 0 ] !== 'Not Started' ) {
        data.eventStateFlag = false;
    }
};
...
export default exports = {
    ...
    populateProjectData,
    performSearch,
    getDeleteObjectsInput,
    getNameOfDeletedObject,
    checkVersionSupportForProject,
    addResponsibleUser,
    checkStateOfEventForDeletion,
    timelinecheckStateOfEventForDeletion
};
/**
 * This factory creates a service and returns exports
 *
 * @memberof NgServices
 * @member pgp0UserListService
 */
app.factory( 'pgp0UserListService', () => exports );
