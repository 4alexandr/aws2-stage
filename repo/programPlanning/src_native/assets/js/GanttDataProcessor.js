...
export default class GanttDataProcessor extends AwBaseService {
    ...
    getConfigOptions() {
        ...
        ganttConfig.branch_loading = false;
    ...
    prepareColumnsForGantt( columns, data ) {
        ...
        columns.forEach( function( col ) {
            ...
            if( colName && colName !== 'icon' && !col.hiddenFlag ) {
                ...
                column.min_width = col.displayName.length * 9;
                column.resize = true;
     ...
     * @param {Object} eventMap Event map for updated objects
     * @param {*} objectsArray Updated objects
     * @param {*} ganttColumns Columns displayed in Gantt
     */
    updateGanttObjects( eventMap, objectsArray, ganttColumns ) {
        let tasksArray = [];
        let linksArray = [];
        ...
        if( !_.isEmpty( tasksArray ) ) {
            let ganttTasks = this.getDataSource().getUpdatedTasks( eventMap, tasksArray, ganttColumns );
     ...
     */
    createNewTask( selectedTaskUID, createdObject, data ) {
        ...
        if( parentTask && ( prevSiblingUID || this.getDataSource().isPaginationCompletedForParent( parentTask.uid ) ) ) {
            let schTasksList = [];
            schTasksList.push( createdObject );
            let parentTasksList = [];
            parentTasksList.push( parentTask );
            let ganttTasks = this.getDataSource().constructGanttTasks( schTasksList, parentTasksList, data.ganttColumns, null );
    ...
    createObjectsOnGantt( createdObjects, data ) {
        ...
        createdObjects.forEach( ( createdObject ) => {
            ...
            switch ( type ) {
                ...
                {
                    let ganttTask = this.createNewTask( selectedTaskUID, createdObject, data );
                    if( ganttTask ) {
