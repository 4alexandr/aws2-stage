            ...
            if( contextObject ) {
                ...
                if( loadedObjects ) {
                    loadedObjects.forEach( function( loadedObject ) {
                        ...
                        var id = objectUID.concat( '||', status ); // use case: when duplicate object like same Risk is attached to two events is coming for two lanes , need to make id as unique.
                        ...
                        if( object ) {
                            ...
                            programRelatedObjUIDMap[ id ] = objectUID; // prepare map of key as conext object and related object UID, and value is actual uid of related object, this will be used in drag/drop and in the other actions.
...
export let initProgramBoard = function( ctx, data ) {
...
export let getValidRelationType = function( contextObject, selectedObject ) {
    if( contextObject.modelType.typeHierarchyArray.indexOf( ppConstants.OBJECT_TYPE.PROGRAM ) > -1 ) {
        if( selectedObject.modelType.typeHierarchyArray.indexOf( ppConstants.INPUT_TYPES.PRGDEL ) > -1 ) {
 ...
 *
 * @memberof NgServices
 * @member TimelineProgramBoardHelper
 */
app.factory( 'TimelineProgramBoardHelper', () => exports );
