 ...
 */
export let getVisibleNodes = function() {
    if( appCtxSvc.ctx.graph.graphModel.nodeMap ) {
        var visibleObjectsList = [];

        var nodeMap = appCtxSvc.ctx.graph.graphModel.nodeMap;
        _.forEach( nodeMap, function( value ) {
 ...
 */
export let setLabelVisibilityOnGraphItem = function( graphItem ) {
    var labelCategories = [];
    var architectureCtx = appCtxSvc.getCtx( 'architectureCtx' );
    labelCategories = architectureCtx.diagram.labelCategories;
 ...
 */
export let checkWhetherToSelectObjectInAce = function( parentUid, objectUid, isConnection ) {
    if( !parentUid ||  isConnection && (appCtxSvc.ctx.aceActiveContext.context.persistentRequestPref && !appCtxSvc.ctx.aceActiveContext.context.persistentRequestPref.includeConnections)  ) {
        return;
    }

    var openedElementUid = appCtxSvc.ctx.occmgmtContext.openedElement.uid;
    if( openedElementUid === parentUid ) {
        eventBus.publish( 'AMGraphElementAdded.updateSelectionInPWA', { objectToSelect: objectUid } );
    } else {
        eventBus.publish( 'addElement.updatePwaDisplay', { objectToSelect: objectUid } );
    }
};
...
app.factory( 'Ase0ArchitectureUtilService', () => exports );
