...
export let onGraphDragStart = function( graphModel, graphItems, dragEffect ) {
    ...
    if( aceActiveCtx && aceActiveCtx.context && aceActiveCtx.context.topElement
        && aceActiveCtx.context.topElement.modelType.typeHierarchyArray.indexOf('Uml1ModelElement') > -1) {
            //Disable drag and drop in context of project model
            return false;
    } else if( aceActiveCtx && aceActiveCtx.context.productContextInfo && aceActiveCtx.context.productContextInfo.uid ) {
        if( (_.invert( aceActiveCtx.context.elementToPCIMap ) )[ aceActiveCtx.context.productContextInfo.uid ] ) {
            ...
            if( modelObj && modelObj.modelType.typeHierarchyArray.indexOf('Uml1ModelElement') > -1 ) {
                //Disable drag and drop in context of project model diagram
                return false;
            }
        }
    }

    if( architectureCtx && !architectureCtx.hasSystemModelerLicense ) {
        // Disable drag and drop if license not present
        return false;
    }
...
var setParent = function( graphModel, dummyNode, targetGraphItem ) {
    if( dummyNode && dummyNode.getItemType() === 'Node' && targetGraphItem ) {
        ...
        if( typeof targetGraphItem.getItemType === "function" && targetGraphItem.getItemType() === 'Node' ) {
            ...
            if( graphModel.nodeMap[ targetGraphItem.modelObject.uid ] ) {
                var parentGraphNode = graphModel.nodeMap[ targetGraphItem.modelObject.uid ];
...
var createNewNode = function( graphModel, draggingGraphItem, targetGraphItem, dragDelta ) {
    ...
    var newNode = graph.createNodeWithBoundsStyleAndTag( rectNode, nodeStyle, nodeBindingData );
...
export let onGraphDrop = function( graphModel, draggingGraphItems, targetGraphItem, dragEffect, cursorLocation,
    ...
    if( !targetGraphItem ) {
        if( isDragWithinDiagram ) {
            var targetItem = {
                modelObject: appCtxSvc.ctx.occmgmtContext.openedElement
            };
            createOccurrance( graphModel, draggingGraphItems, targetItem, dragDelta, true );
        } else {
...
var toggleVisibilityOnDrop = function( graphModel, draggingGraphItems, location ) {
    ...
    _.forEach( draggingGraphItems, function( dragItem ) {
        ...
        if( !graphModel.nodeMap[ addedObject.uid ] && !graphModel.edgeMap[ addedObject.uid ] ) {
 ...
 *
 * @memberof NgServices
 * @member Ase0ArchitectureGraphDragAndDropHandler
 *
 * @param {object} cdm client data model
 * @param {object} appCtxSvc context service
 * @returns {object} Ase0ArchitectureGraphDragAndDropHandler drag and drop handler service
 */
app.factory( 'Ase0ArchitectureGraphDragAndDropHandler', () => exports );
