 ...
 */
export let processEdgeData = function( activeLegendView, graphModel, graphData, edgesArray, isRecallCase ) {
    var returnData = {
        structureEdges: []
    };
    var addedEdges = [];
            ...
            _.forEach( output.edgeData,
                function( edgeInformation ) {
                    ...
                    if( edgeInformation.edge ) {
                        ...
                        if( edgeModel ) {
                            isConnection = clientMetaModel.isInstanceOf( 'Awb0Connection', edgeModel.modelType );
                            if( !isConnection ) {
                                underlyingObject = edgeModel;
                                ...
                                var underlyingObjUid = edgeModel.props.awb0UnderlyingObject.dbValues[ 0 ];
                                underlyingObject = cdm.getObject( underlyingObjUid );
                            }
                            edge = graphModel.edgeMap[ uidToCheck ];
                        }
                    }
                    ...
                    if( !edge ) {
                        ...
                        if( isConnection ) {
                            if( graphModel.portMap ) {
                                sourcePort = graphModel.portMap[ edgeInformation.end1Element.uid ];
                                targetPort = graphModel.portMap[ edgeInformation.end2Element.uid ];
                            }
                        } else if( graphModel.nodeMap ) {
                        ...
                        if( sourcePort && targetPort ) {
                            edge = createEdgeWithStyleWithPorts( graph, sourcePort, targetPort, edgeStyle, edgeInformation );
                        } else if( sourceNode && targetNode ) {
                            if( edgeCategory && ( edgeCategory.localeCompare( 'Structure' ) === 0 ||
                                    edgeCategory.localeCompare( '' ) === 0 ) ) {
                                var returnEdgeData = processParentChildRelation( sourceNode, targetNode, graphModel, isRecallCase, edgeStyle, edgeInformation.edgeInfo );
                        ...
                        if( edge ) {
                            ...
                            edge.modelObject = edgeModel;
                            edge.edgeType = edgeType;
                            edge.appData = {
                                edgeUid: uidToCheck
                            };
                            graphModel.edgeMap[ uidToCheck ] = edge;
                            edgesArray.push( edgeModel );
                            setEdgeLabel( graph, edgeInformation.edgeInfo, edgeCategory, edge );
 ...
 * @param {Object} edge the edge object from graph
 */
var setEdgeLabel = function( graph, edgeInfo, edgeCategory, edge ) {
    if( !graph || !edgeInfo || !edge ) {
        return;
    }
 ...
 *
 */
export let updateEdgeStyle = function() {
    var graphContext = appCtxSvc.ctx.graph;
    var graph = graphContext.graphModel.graphControl.graph;
    var activeLegendView = null;
    if( graphContext ) {
        if( graphContext.legendState ) {
...
app.factory( 'Ase0ArchitectureEdgeService', () => exports );
