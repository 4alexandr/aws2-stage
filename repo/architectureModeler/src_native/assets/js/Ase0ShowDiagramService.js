 ...
 */
export let updatePanelSize = function (eventData) {
    var tree = eventData.area1;
    if (!tree || tree.getAttribute('id') !== 'diagramTabTreeViewCol') {
        return;
    }

    var prevDiagCtx = appCtxSvc.getCtx('previewDiagramCtx');
    if (!prevDiagCtx) {
        prevDiagCtx = {
            tabWidth: null
        };
        appCtxSvc.registerCtx('previewDiagramCtx', prevDiagCtx);
    }
...
/**
 * Unregister context on Diagrams tab load.
 */
export let handleDiagramsTabUnloaded = function () {
    eventBus.unsubscribe(_diagramsTabUnloadedEventListener);
    _diagramsTabUnloadedEventListener = null;
    appCtxSvc.unRegisterCtx( 'diagramsCtx' );
};
 ...
 *
 */
export let handleDiagramsTabLoad = function (data) {
    editHandlerSvc.removeEditHandler('NONE');
    var diagramsCtx = appCtxSvc.getCtx('diagramsCtx');
    if (diagramsCtx) {
        diagramsCtx.isDiagramsTabActive = true;
        appCtxSvc.updateCtx('diagramsCtx', diagramsCtx);
    } else {
        diagramsCtx = {
    ...
    if (!_diagramsTabUnloadedEventListener) {
        _diagramsTabUnloadedEventListener = eventBus.subscribe('Ase0ShowDiagram.contentUnloaded', () => {
            handleDiagramsTabUnloaded();
        }, 'Ase0ShowDiagramService');
    }
};
    ...
    licenseCheckPromise.then(
        (response) => {
            if (parseInt(response.outputVal, 10) > 0) {
                    appCtxSvc.ctx.diagramsCtx.hasAdvancedLicense = true;
            }
        }
    )
        .catch(
            (exception) => {
                ...
                appCtxSvc.ctx.diagramsCtx.hasAdvancedLicense = false;
            }
        );;
}
 ...
 */
export let updateDiagramViewOnCreate = function (toOpenDiagram, uid) {
 ...
 */
export let updateDiagramViewOnMarkupCreation = function( updatedObjects ) {
    let eventName, eventData;
    let flag = true;
...
export let openDiagram = function (selectedDiagram, navigateIn) {
    let soaInput = {
        ...
        function (response) {
            if (response.output.length > 0) {
                ...
                if (response.output[0].relationshipData[0].relationshipObjects.length > 0) {
                    ...
                    return _redirectToShowObject(owningObject, relationProxyObjectUid, navigateIn);
                }
            }
        });
};

function _redirectToShowObject(owningObject, relationProxyObjectUid, navigateIn) {
    ...
    if (navigateIn) {
        ...
        if (appCtxSvc.getCtx('occmgmtContext') && appCtxSvc.ctx.state.params.uid && appCtxSvc.ctx.state.params.uid !== owningObject.uid) {
 ...
 *
 * @memberof NgServices
 * @member Ase0ShowDiagramService
 */
app.factory('Ase0ShowDiagramService', () => exports);
