 ...
 */
export let processPortData = function( activeLegendView, graphModel, graphData, isRecallCase ) {
    var returnData = {};
    var ports = [];
    ...
    if( graphData && graphData.output ) {
        ...
        _.forEach( graphData.output, function( output ) {
            _.forEach( output.portData, function( portInformation ) {
                var portObject = cdm.getObject( portInformation.port.uid );
                var port = graphModel.portMap[ portObject.uid ];
                ...
                if( !port ) {
                    ...
                    if( graphModel.nodeMap ) {
                        portOwner = graphModel.nodeMap[ portInformation.portOwner.uid ];
                    }
                    ...
                    if( portObject && portStyle && portOwner ) {
                        port = graph.addPortAtLocationWithStyle( portOwner, portPosition, portStyle );
                    }
                    ...
                    if( port ) {
                        ...
                        setPortLabel( graph, portInformation.portInfo, portCategory, port );
                        graphModel.portMap[ portInformation.port.uid ] = port;
                        ports.push( port );
                        if( !isRecallCase ) {
                            if( portPosition ) {
 ...
 */
var setPortLabel = function( graph, portInfo, portCategory, port ) {
    if( !graph || !portInfo || !port ) {
        return;
    }
    ...
    if( isLabelVisibile ) {
        ...
        if( cmm.isInstanceOf( 'Awb0Interface', modelObject.modelType ) && !modelObject.props.ase0InterfaceName ) {
            var portDisplayedPropertyDescriptor = modelObject.modelType.propertyDescriptorsMap.ase0InterfaceName;
            modelObject.props.ase0InterfaceName = {
...
export let evaluateShowPortsCondition = function() {
    var graph = appCtxSvc.ctx.graph;
        ...
        if( selectedNodes ) {
            ...
            for( var i = 0; i < selectedNodes.length; i++ ) {
                ...
                if( node ) {
                    ...
                    if( graphModel.nodeMap && graphModel.nodeMap[ nodeUid ] ) {
                        nodeData = graphModel.nodeMap[ nodeUid ];
                        var portDegree = nodeData.portDegree;
                        var portModels = nodeData.getPorts();

                        var visiblePorts = [];
                        var hiddenPorts = [];
                        var filteredPorts = [];
                        var visibleConnectedPorts = [];
                        var hiddenConnectedPorts = [];
                            ...
                            for( var j = 0; j < portModels.length; j++ ) {
                                if( portModels[ j ].modelObject ) {
                                    if( portModels[ j ].isVisible() ) {
                                        ...
                                        if( portModels[ j ].getConnections() ) {
                                            hiddenConnectedPorts.push( portModels[ j ] );
                                        }
                                    }
                                }
                            }
                        ...
                        architectureCtx.diagram.doShowAllPortsCmd = false;
                        architectureCtx.diagram.doShowConnectedPortsCmd = false;
            ...
            architectureCtx.diagram.doShowAllPortsCmd = doShowAllPortsCmd;
            architectureCtx.diagram.doShowConnectedPortsCmd = doShowConnectedPortsCmd;
        }
    }
};

// show only connected Ports for selected Node
export let showConnectedPorts = function() {
    var graphModel = appCtxSvc.ctx.graph.graphModel;
    var graphControl = graphModel.graphControl;
    var graph = graphControl.graph;
    var architectureCtx = appCtxSvc.getCtx( 'architectureCtx' );
    if( architectureCtx && architectureCtx.diagram ) {
        var diagramSelection = architectureCtx.diagram.selection;
        if( diagramSelection ) {
            var selectedNodes = diagramSelection.nodeModels;
            for( var i = 0; i < selectedNodes.length; i++ ) {
                var node = selectedNodes[ i ];
                if( node ) {
                    var nodeData = null;
                    ...
                    if( graphModel.nodeMap ) {
                        ...
                        if( visibleUnConnectedPorts.length > 0 ) {
                            // update port map
                            for( var k = 0; k < visibleUnConnectedPorts.length; k++ ) {
                                if( graphModel.portMap ) {
                                    var portUid = visibleUnConnectedPorts[ k ].modelObject.uid;
                                    var portObj = graphModel.portMap[ portUid ];
                                    if( portObj ) {
                                        // Remove entry from PortMap
                                        delete appCtxSvc.ctx.graph.graphModel.portMap[ portUid ];
                                    }
                                }
                            }
                            var removedItems = graph.removePorts( visibleUnConnectedPorts );
                            ...
                            eventBus.publish( 'AMGraph.modelChanged', { itemsAdded: [], itemsRemoved: visibleUnConnectedPorts } );
                        }
                    }
                }
            }
...
app.factory( 'Ase0ArchitecturePortService', () => exports );
