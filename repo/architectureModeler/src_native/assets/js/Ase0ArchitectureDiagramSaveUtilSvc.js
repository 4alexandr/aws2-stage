 ...
 */
export let getOpenedDiagramInfo = function() {
    var diagramInfoMap = {};
    var diagramInfo = {};
    ...
    var autoLayoutState = getAutoLayoutState();
    diagramInfo.autoLayoutState = [ autoLayoutState.toString() ];

    //set LabelFilters
    var labelFilters = getLabelFilters();
 ...
 */
var getLabelFilters = function() {
    var labelFilters = [];
    var archContext = appCtxSvc.getCtx( 'architectureCtx' );
    var labelCategories = archContext.diagram.labelCategories;
...
/**
 * set AutoLayout in diagram Info
 * @returns {Boolean} autoLayoutState
 */
var getAutoLayoutState = function() {
    var architectureCtx = appCtxSvc.getCtx( 'architectureCtx' );
    var autoLayoutState = false;
    if( architectureCtx && architectureCtx.diagram && architectureCtx.diagram.isAutoLayoutOn ) {
        autoLayoutState = architectureCtx.diagram.isAutoLayoutOn;
    }
...
var getViewPoint = function( graphControl ) {
    if( graphControl ) {
        ...
        var viewPointStr = viewPoint.x + ':' + viewPoint.y;
        return viewPointStr;
    }
};
/**
...
var getGridOptions = function() {
    var gridOptions = [];
    var architectureCtx = appCtxSvc.getCtx( 'architectureCtx' );
    if( architectureCtx && architectureCtx.diagram && architectureCtx.diagram.gridOptions ) {
        if( architectureCtx.diagram.Ase0MajorLines ) {
            gridOptions.push( 'Ase0MajorLines' );
        }
        if( architectureCtx.diagram.Ase0MinorLines ) {
            gridOptions.push( 'Ase0MinorLines' );
        }
        if( architectureCtx.diagram.Ase0SnapToGrid ) {
...
var getFilterInfo = function() {
    var graphContext = appCtxSvc.getCtx( 'graph' );
    var legendState = graphContext.legendState;
    var objectFilter = [];
    var relationFilter = [];
    var portFilter = [];
...
app.factory( 'Ase0ArchitectureDiagramSaveUtilSvc', () => exports );
