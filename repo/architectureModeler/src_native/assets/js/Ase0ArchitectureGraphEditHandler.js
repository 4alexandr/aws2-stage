...
export let preNodeEdit = function( graphModel, node, propertyName ) {
    ...
    var promise = soaSvc.postUnchecked( 'Internal-AWS2-2012-10-DataManagement', 'loadDataForEditing', {
        inputs: [ {
            obj: node.modelObject,
    ...
    promise.then( function( response ) {
        ...
        if( response && response.outputs ) {
            var modelObject = response.outputs[ 0 ].obj;
            if( modelObject.props.is_modifiable && modelObject.props.is_modifiable.dbValues[ 0 ] === '1' ) {
                deferred.resolve();
            } else {
                deferred.reject();
            }
