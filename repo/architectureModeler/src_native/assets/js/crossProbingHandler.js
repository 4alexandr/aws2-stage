 ...
 * @return SOA input for getRelatedElements SOA.
 */
 ...
 */
var _crossProbe = function() {
    // get selections
    var mselection = appCtxSvc.getCtx( 'mselected' );
    var selectedObjects = [];
    _.forEach( mselection, function( selection ) {
        if( selection && selection.modelType ) {
            if( cmm.isInstanceOf( 'Awb0ConditionalElement', selection.modelType ) ) {
    ...
    ensurePreferenceLoaded().then( function( /* result */) {
        ...
        var productContext = appCtxSvc.getCtx( "occmgmtContext.productContextInfo" );
...
var _delayedCrossProbe = function() {
 ...
 */
export let isCrossProbingActive = function() {
    var crossProbingMode = appCtxSvc.getCtx( "occmgmtContext.CrossProbeMode" );
    if( crossProbingMode ) {
 ...
 */
export let toggleCrossProbing = function() {
    var crossProbingMode = !exports.isCrossProbingActive();

    if( !crossProbingMode ) {
        _selectedObjects = [];
    }

    // Fire Cross-Probe Event
    var eventData = {
        "crossProbingMode": crossProbingMode
    };
    eventBus.publish( "gwt.CrossProbingModeChangeEvent", eventData );

    appCtxSvc.updatePartialCtx( "occmgmtContext.CrossProbeMode", crossProbingMode );
 ...
 */
app.factory( 'crossProbingHandler', () => exports );
