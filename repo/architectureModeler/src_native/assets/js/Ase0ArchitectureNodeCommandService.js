...
var setIconImage = function( bindProperties, buttonStyleKey, state, visible ) {
    ...
    switch ( buttonStyleKey ) {
        case PARENT_DEGREE:
            bindProperties[ SHOW_PARENT_IMG ] = visible ? getButtonImage( 'Parent' ) : null;
            break;
        case CHILD_DEGREE:
            if( visible ) {
                bindProperties[ SHOW_CHILD_IMG ] = ( state === PARTIAL_STATE ) ? getButtonImage( 'ChildPartial' ) : getButtonImage( 'Child' );
            } else {
        ...
        case IN_DEGREE:
            if( visible ) {
                bindProperties[ SHOW_INCOMING_IMG ] = ( state === PARTIAL_STATE ) ? getButtonImage( 'IncomingPartial' ) : getButtonImage( 'ShowIncomingRelations' );
            } else {
        ...
        case OUT_DEGREE:
            if( visible ) {
                bindProperties[ SHOW_OUTGOING_IMG ] = ( state === PARTIAL_STATE ) ? getButtonImage( 'OutgoingPartial' ) : getButtonImage( 'ShowOutgoingRelations' );
            } else {
...
var setIconTooltip = function( selected, bindProperties, data, buttonStyleKey, state, visible, totalCount ) {
    ...
    if( visible && architectureCtx && architectureCtx.disableTileCommands ) {
        visible = false;
    }

    // populate the tooltips on the context object
    if( data && data.i18n ) {
        architectureCtx.hideParentCmdTitle = data.i18n.hideParentCmdTitle;
        architectureCtx.showParentCmdTitle = data.i18n.showParentCmdTitle;
        architectureCtx.hideAllChildren = data.i18n.hideAllChildren;
        architectureCtx.showAllChildren = data.i18n.showAllChildren;
        architectureCtx.hideChildren = data.i18n.hideChildren;
        architectureCtx.showChildren = data.i18n.showChildren;
        architectureCtx.hideAllIncomingRelations = data.i18n.hideAllIncomingRelations;
        architectureCtx.showAllIncomingRelations = data.i18n.showAllIncomingRelations;
        architectureCtx.hideIncomingRelations = data.i18n.hideIncomingRelations;
        architectureCtx.showIncomingRelations = data.i18n.showIncomingRelations;
        architectureCtx.hideAllOutgoingRelations = data.i18n.hideAllOutgoingRelations;
        architectureCtx.showAllOutgoingRelations = data.i18n.showAllOutgoingRelations;
        architectureCtx.hideOutgoingRelations = data.i18n.hideOutgoingRelations;
        architectureCtx.showOutgoingRelations = data.i18n.showOutgoingRelations;
    }
    switch ( buttonStyleKey ) {
        case PARENT_DEGREE:
            if( visible ) {
                var hideParentCmdTitle = architectureCtx.hideParentCmdTitle;
                var showParentCmdTitle = architectureCtx.showParentCmdTitle;
                bindProperties[ SHOW_PARENT_TOOLTIP ] = selected ? hideParentCmdTitle : showParentCmdTitle;
            } else {
        ...
        case CHILD_DEGREE:
            if( visible ) {
                if( state === PARTIAL_STATE ) {
                    var hideAllChildren = architectureCtx.hideAllChildren;
                    var showAllChildren = architectureCtx.showAllChildren;
                    bindProperties[ SHOW_CHILDREN_TOOLTIP ] = selected ? hideAllChildren : showAllChildren.replace( '{0}', totalCount );
                } else {
                    var hideChildren = architectureCtx.hideChildren;
                    var showChildren = architectureCtx.showChildren;
                    bindProperties[ SHOW_CHILDREN_TOOLTIP ] = selected ? hideChildren : showChildren;
                }
            } else {
        ...
        case IN_DEGREE:
            if( visible ) {
                if( state === PARTIAL_STATE ) {
                    var hideAllIncomingRelations = architectureCtx.hideAllIncomingRelations;
                    var showAllIncomingRelations = architectureCtx.showAllIncomingRelations;
                    bindProperties[ SHOW_INCOMING_TOOLTIP ] = selected ? hideAllIncomingRelations : showAllIncomingRelations.replace( '{0}', totalCount );
                } else {
                    var hideIncomingRelations = architectureCtx.hideIncomingRelations;
                    var showIncomingRelations = architectureCtx.showIncomingRelations;
                    bindProperties[ SHOW_INCOMING_TOOLTIP ] = selected ? hideIncomingRelations : showIncomingRelations;
                }
            } else {
        ...
        case OUT_DEGREE:
            if( visible ) {
                if( state === PARTIAL_STATE ) {
                    var hideAllOutgoingRelations = architectureCtx.hideAllOutgoingRelations;
                    var showAllOutgoingRelations = architectureCtx.showAllOutgoingRelations;
                    bindProperties[ SHOW_OUTGOING_TOOLTIP ] = selected ? hideAllOutgoingRelations : showAllOutgoingRelations.replace( '{0}', totalCount );
                } else {
                    var hideOutgoingRelations = architectureCtx.hideOutgoingRelations;
                    var showOutgoingRelations = architectureCtx.showOutgoingRelations;
                    bindProperties[ SHOW_OUTGOING_TOOLTIP ] = selected ? hideOutgoingRelations : showOutgoingRelations;
                }
            } else {
...
var updateInOutDegreeAttributesOnNode = function( node, bindData, data, direction ) {

    var state = EXPAND_STATE;
    var selected = false;
    var allVisibleRelations = direction === "IN" ? archGraphLegendManager.getVisibleRelationTypes( node.incomingRelations ).length : archGraphLegendManager.getVisibleRelationTypes( node.outgoingRelations ).length;
 ...
 */
export let updateGraphInfoOnNodes = function( affectedNodeList, graphModel, data ) {
    _.forOwn( affectedNodeList, function( node ) {
        ...
        bindData = updateInOutDegreeAttributesOnNode( node, bindData, data, "IN" );
        bindData = updateInOutDegreeAttributesOnNode( node, bindData, data, "OUT" );
...
export let validateForUpdateGraphNodeDegree = function( graphModel ) {
    if( graphModel && graphModel.nodeMap ) {
        var nodes = Object.values( graphModel.nodeMap );
        var affectedNodeList = [];
        _.forEach( nodes, function( node ) {
            if( node.isVisible() && !node.isFiltered() ) {
...
app.factory( 'Ase0ArchitectureNodeCommandService', () => exports );
