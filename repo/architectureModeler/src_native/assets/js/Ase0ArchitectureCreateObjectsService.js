 ...
 */
export let createObject = function( data ) {
    var createInput = addObjectUtils.getCreateInput( data );
    var panelContext = appCtxSvc.ctx.panelContext;
    if( panelContext ) {
 ...
 */
export let edgeCreatedCompletionAction = function( graphDataResponse, eventData, data ) {
    ...
    try {
        ...
        if( edgeObjectInfo && cmm.isInstanceOf( 'Awb0Element', edgeObjectInfo.edge.modelType ) ) {
            ...
            utilSvc.checkWhetherToSelectObjectInAce( edgeObjectInfo.edge.props.awb0Parent.dbValues[ 0 ],
                edgeObjectInfo.edge.uid, true );
        }

        if( edge && edgeObjectInfo ) {
            var edgeCategory = eventData.categoryType;

            var affectedNodeList = updateEdgeWithNewObject( graphModel, edge, legendState, edgeObjectInfo, edgeCategory );
        ...
        _createElementQueue.shift();
        resetAcePwa( graphDataResponse );
    }
};

var resetAcePwa = function( graphDataResponse ) {
    ...
    if( clientIdStr === 'CreatePort' ) {
        ...
        var includeInterfaces = _.get(appCtxSvc, "ctx.aceActiveContext.context.persistentRequestPref.includeInterfaces", false);
        doSkipReset = includeInterfaces ? false : true;
    }
...
export let reconnectCompletionAction = function( data, graphData ) {
    var graphContext = appCtxSvc.getCtx( 'graph' );
    var graphModel = null;
    var legendState = null;
    ...
    try {
        ...
        _.forEach( graphData.output, function( output ) {
            ...
            _.forEach( output.edgeData, function( edgeInformation ) {
                var srcNode = graphModel.nodeMap[ edgeInformation.end1Element.props.awb0Parent.dbValues[ 0 ] ];
                var tarNode = graphModel.nodeMap[ edgeInformation.end2Element.props.awb0Parent.dbValues[ 0 ] ];
                if( srcNode && srcNode.isVisible() && tarNode && tarNode.isVisible() ) {
                    var categoryToAdd = '';
                    var relationObjectType = edgeInformation.edgeInfo.edgeType[ 0 ];

                    var srcNodeType = srcNode.nodeType;
                    var tarNodeType = tarNode.nodeType;
 ...
 */
var updateEdgeWithNewObject = function( graphModel, edge, legendState, newEdgeObject, edgeCategory ) {
    ...
    if( graphModel.graphControl && graphModel.graphControl.graph ) {
        ...
        utilSvc.setLabelVisibilityOnGraphItem( edge );
        ...
        if( edge.modelObject ) {
            ...
            if( srcNode && srcNode.isVisible() && tarNode && tarNode.isVisible() ) {
                ...
                var categoryToAdd = '';
                var relationObject = edge.modelObject;
        ...
        if( !graphModel.edgeMap[ uidToCheck ] ) {
            edge.appData = {
                edgeUid: uidToCheck
            };
            graphModel.edgeMap[ uidToCheck ] = edge;
            responseEdgeData = [];
            output = {};
            output.edgeData = [];
 ...
 */
var updatePortWithNewObject = function( graphModel, port, legendState, newPortObject, node ) {
    if( graphModel.graphControl && graphModel.graphControl.graph ) {
        var graph = graphModel.graphControl.graph;
        var scopeFilter;
        ...
        var portLabel = _.get( newPortObject, 'port.props.ase0InterfaceName.dbValues[0]' );
        if( !portLabel ) {
            portLabel = _.get( newPortObject, 'portInfo.displayProperties[0]', '' );
        }
        graph.setLabel( port, portLabel );
        utilSvc.setLabelVisibilityOnGraphItem( port );
        graphModel.graphControl.graph.updateOnItemsAdded( [ port ] );

        if( !graphModel.portMap[ newPortObject.port.uid ] ) {
            graphModel.portMap[ newPortObject.port.uid ] = port;
 ...
 * @param {object} graphDataResponse -  SOA execution response
 */
export let portCreatedCompletionAction = function( graphDataResponse ) {
    var graphContext = appCtxSvc.getCtx( 'graph' );
    var graphModel = null;
    var legendState = null;
    ...
    try {
        ...
        if( port && portObjectInfo && portObjectInfo.portOwner ) {
            ...
            updatePortWithNewObject( graphModel, port, legendState, portObjectInfo, ownerObject );
...
app.factory( 'Ase0ArchitectureCreateObjectsService', () => exports );
