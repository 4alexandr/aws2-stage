...
export let updatePanelSizes = function( eventData ) {
    var area1 = eventData.area1;
    var area2 = eventData.area2;

    // If ACE slider is moved update the max-width
    if( area1.className && area1.className.includes( 'primaryWorkarea' ) ) {
        handleAceSlider( area1, area2 );
        return null;
    }

    // Find the Requirements table
    var reqTable = null;
    var tables = area1.parentElement.getElementsByTagName( SPLM_TABLE_ELEMENT );
    for( var x = 0; x < tables.length; ++x ) {
        var gridid = tables[ x ].getAttribute( 'gridid' );
        if( gridid && gridid === 'RequirementsTable' || gridid === 'RequirementsTree' ) {
            reqTable = tables[ x ];
            break;
        }
    }
    ...
    // find the doc viewer
    var viewer = null;
    var presenters = area2.getElementsByTagName( 'form' );
    if( presenters.length > 0 ) {
        _.forEach( presenters, function( form ) {
            var type = form.getAttribute( 'panel-id' );
            if( type === 'Arm0RequirementDocumentation' ) {
                viewer = form;
            }
        } );
    }

    // determine if doc viewer is visible
    var viewerVis = false;
    if( viewer ) {
        viewerVis = true;
        if( viewer.parentElement.parentElement.className.indexOf( 'hidden' ) > -1 ) {
            viewerVis = false;
        }
    }

    // find the attributes table
    var attTable = findAttributesTable( area1, area2 );

    // determine if attribute table is visible
    var attTableVis = false;
    if( attTable ) {
        attTableVis = true;

        var table = attTable.parentElement.parentElement.parentElement;
        if( table.className.indexOf( 'hidden' ) > -1 ) { attTableVis = false; }
    }
    return defineTableHeights( area1, reqTable, viewerVis, viewer, attTableVis, attTable );
};
...
app.factory( 'Ase0SystemReqUtilService', () => exports );
