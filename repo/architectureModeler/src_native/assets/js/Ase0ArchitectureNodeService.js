...
export let processNodeData = function( activeLegendView, graphModel, graphData, data, isRecallCase ) {
    ...
    if( graphData && graphData.output ) {
        ...
        _.forEach( nodesInfo.nodes, function( nodeData ) {
            var node = graphModel.nodeMap[ nodeData.id ];

            //Node data to be bind in SVG template
            if( !node ) {
                var nodeObject = cdm.getObject( nodeData.id );

                var archCtx = appCtxSvc.getCtx( 'architectureCtx' );

                if( nodeObject.props.awb0IsStructModifiable && !_.isEmpty( archCtx.revisedUid ) ) {
                    var underlyingObjectUid = _.get( nodeObject, 'props.awb0UnderlyingObject.dbValues[0]', undefined );
                    if( underlyingObjectUid && underlyingObjectUid === archCtx.revisedUid ) {
                        archCtx.revisedUid = '';
                        appCtxSvc.updateCtx( 'architectureCtx', archCtx );
                        delete nodeObject.props.awb0IsStructModifiable;
                    }
                }
                ...
                node = graph.createNodeWithBoundsStyleAndTag( nodeData.position, template, bindData );
                ...
                updateNodeMap( nodeObject, data.archPageData.nodes );
            } else {
                if( !node.isVisible() ) {
...
/** Maintain the model object Vs graphItem map
 *  @param {Object} nodesObject Node object
 *  @param {Array} nodesArray Node array
 */
var updateNodeMap = function( nodesObject, nodesArray ) {
    if( !_.includes( nodesArray, nodesObject ) ) {
...
app.factory( 'Ase0ArchitectureNodeService', () => exports );
