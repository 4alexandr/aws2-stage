...
const through = require( 'through2' ); // returning a stream
...
const start = function( argv ) {
    ...
    gulp.src( srcDir )
        .pipe( tap( file => {
            logger.debug( 'Converting ' + util.shortenPath( file.path ) );
        } ) )
        .pipe( toJs() )
        .pipe( chmod( 0o777 ) )
        .pipe( minimize() )
        ...
        .on( 'end', () => {
            logger.success( 'Finished converting templates from XML to JSON in ' + util.shortenPath( outDir ) );
        } );
