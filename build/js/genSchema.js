...
const util = require( 'afx/build/js/util' );

/**
 * Generate the schema json
 *
 * @param {String} assetsDir - assets directory
 * @param {String[]} requiredSoa - required SOA roots
 * @param {Boolean|null} forceJavaScript - force javascript
 * @return {Promise} promise
 */
module.exports.createSchema = function( assetsDir, requiredSoa, forceJavaScript ) {
    ...
    let outputPath = `${assetsDir}/config/schema.js`;
    let output;
    return util.stream2Promise( gulp.src( `${process.cwd()}/src/soa/json/*_template.json` )
        .pipe( tap( file => {
            try {
                templates[ file.relative.split( '_' )[ 0 ] ] = JSON.parse( file.contents.toString() );
            } catch ( err ) {
                logger.severe( `Error parsing ${file.path}\n${err}` );
            }
        } ) )
    ).then( () => {
        return util.stream2Promise( gulp.src( `${process.cwd()}/out/soa/json/*_template.json` )
            .pipe( tap( file => {
                try {
                    // any new templates in the out directory will overwrite the ones in the src directory (they should have the same name)
                    templates[ file.relative.split( '_' )[ 0 ] ] = JSON.parse( file.contents.toString() );
                } catch ( err ) {
                    logger.severe( `Error parsing ${file.path}\n${err}` );
                }
        ...
        output = JSON.stringify( { schema: schema }, null, 2 ).replace( /IModelObject/g, 'ModelObj' );
        return util.writeFile( `${outputPath}on`, output );
    } ).then( () => {
        if( forceJavaScript ) {
            output = `define(['js/configurationService'],function(cfgSvc){cfgSvc.add('schema',${output});});`;
            return util.writeFile( outputPath, output );
        }
    } ).then( () => {
        return logger.debug( ' ... generated ' + util.shortenPath( outputPath ) + stopwatch.end(), MSG_PREFIX );
    } );
};
